		...
		t.Run(wc.label, func(t *testing.T) {
			...
			func() {
				...
				for _, sz := range sizes {
					p0, p1, p2 := splitTriple48LockedBatch(superTestFixedData(sz), wc.bp)
	...
	for _, tc := range interlock48X4Cases(t) {
		...
		t.Run(tc.label, func(t *testing.T) {
			...
			func() {
				...
				for _, sz := range sizes {
					p0, p1, p2 := splitTriple48LockedBatch(framed[sz], scalar)
			...
			for _, a := range interlockArms() {
				...
				t.Run(a.name, func(t *testing.T) {
					...
					for _, sz := range sizes {
						x4Calls.Store(0)
						p0, p1, p2 := splitTriple48LockedBatch(framed[sz], armed)
						if x4Calls.Load() == 0 {
						...
						x4Calls.Store(0)
						out := interleaveTriple48LockedBatch(p0, p1, p2, armed)
						if x4Calls.Load() == 0 {
