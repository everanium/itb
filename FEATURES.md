...
- **Triple Ouroboros Bit-Soup Mode.** Opt-in plaintext bit-permutation split (`SetBitSoup(1)`) that re-permutes the 7-bit data fields across the three Triple Ouroboros snakes at sub-byte granularity instead of the default byte-level split. Provides the strongest architectural barrier against split-fixing attacks (each snake sees a non-contiguous bit-permuted slice of the plaintext, so a partial seed compromise of one snake reveals fewer adjacent bits) at **no performance trade-off** — encrypt/decrypt throughput remains within benchmark noise of the byte-level split because both modes route through the same `processChunk{256,512}` SIMD kernel with the same allocation profile. Recommended for any deployment where the additional barrier is acceptable security collateral; benchmarks across all PRF-grade primitives in [BENCH3.md](BENCH3.md) confirm the parity.
