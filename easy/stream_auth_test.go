...
// / key_bits / mode combination. mode = 1 selects [easy.New3] (Single
...
// TestEasyStreamAuthFlagPreservedTripleTinyPayload confirms the
// per-chunk finalFlag round-trips intact at chunk_size = 1 plaintext
// byte under Triple Ouroboros. The recovered flag must match the
// encoder-set value across both finalFlag = false and finalFlag = true
// regardless of how tightly the 48-bit interlock overlay compresses
// the single data byte.
func TestEasyStreamAuthFlagPreservedTripleTinyPayload(t *testing.T) {
	plaintext := []byte{0x42}
	streamID := streamAuthZeroID()
	...
	for _, ps := range streamAuthSpec {
		for _, finalFlag := range []bool{false, true} {
			...
			name := fmt.Sprintf("%s_w%d_TripleTiny_%s", ps.name, ps.width, flagName)
			t.Run(name, func(t *testing.T) {
				...
				if !bytes.Equal(pt, plaintext) {
					t.Errorf("plaintext mismatch: got %x, want %x", pt, plaintext)
				}
				if recoveredFinal != finalFlag {
					t.Errorf("flag mismatch: encoded %v, recovered %v",
