...
// default constructor (areion512 / 1024 / hmac-blake3, Triple mode).
...
func TestSmokeExportImport(t *testing.T) {
	...
	if prim != "areion512" || kb != 1024 || mode != 3 || mac != "hmac-blake3" {
		t.Errorf("PeekConfig: got (%q, %d, %d, %q), want (areion512, 1024, 3, hmac-blake3)",
			prim, kb, mode, mac)
	}

	receiver := easy.New3("areion512", 1024, "hmac-blake3")
...
func TestSmokeImportRejection(t *testing.T) {
	sender := easy.New3("areion512", 1024)
	defer sender.Close()
	blob := sender.Export()

	// primitive mismatch — receiver expects blake3.
	receiver := easy.New3("blake3", 1024)
	defer receiver.Close()
...
// TestSmokeSeedCountEight verifies that a fresh Triple-Ouroboros
// encryptor always carries eight seed slots (noise + lockSeed +
// 3 data + 3 start).
func TestSmokeSeedCountEight(t *testing.T) {
	enc := easy.New3()
	defer enc.Close()
...
func TestSmokeNonceBitsTwoEncryptorsIsolated(t *testing.T) {
	a := easy.New3("blake3", 1024, "kmac256")
	defer a.Close()
	b := easy.New3("blake3", 1024, "kmac256")
	defer b.Close()
...
// TestSmokeImportRestoresFullConfig verifies that the per-instance
// configuration knobs (NonceBits, BarrierFill) round-trip through
// Export / Import without a manual mirror call on the receiver
// side. The blob carries the values that were explicitly set by
// the sender; missing fields indicate sender inheritance and leave
// the receiver's cfg untouched.
func TestSmokeImportRestoresFullConfig(t *testing.T) {
	src := easy.New3("blake3", 1024, "kmac256")
	defer src.Close()
	...
	dst := easy.New3("blake3", 1024, "kmac256")
	defer dst.Close()
...
func TestSmokeImportBarrierFillReceiverPriority(t *testing.T) {
	...
	// blob value.
	dst2 := easy.New3("blake3", 1024, "kmac256")
	defer dst2.Close()
