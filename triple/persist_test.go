...
func TestNonceBitsBarrierFillIsolation(t *testing.T) {
	...
	// for the same plaintext. Two components scale with the nonce width:
	// the wire header carries the main nonce (16 vs 64 bytes), and the
	// three interlocked lanes together carry nonceLen bytes of interlock
	// nonce as per-lane fragments. At this plaintext size the container
	// sits on the MinPixels floor, which absorbs the lane growth
	// entirely, so only the header component reaches the wire — hence a
	// bound of one nonce-width difference rather than two.
