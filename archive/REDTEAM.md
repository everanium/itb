...
- [Proof 10](PROOFS.md#proof-10-guaranteed-drbg-residue-no-perfect-fill) — guaranteed CSPRNG residue (fill minimum)
...
**One-off 63 MB KL floor probe.** A standalone test pair encrypts one plaintext at the ITB data-size limit and runs a chunked single-threaded Phase 2b on it — used to measure how close per-pixel KL gets to its theoretical floor at maximum sample size per hash:
...
2. **PRF seed recovery.** Same reason — PRF primitives have no GF(2)-linear structure that brute-force compound-key recovery could exploit, by the PRF assumption itself. They would require a PRF-break, out of scope by assumption.
...
- **At BF=1** (`tmp/results/single_bf1/04_phase2b.log`): **8 / 90 cells flagged**, across FNV-1a, MD5, Areion-SoEM-512, AES-CMAC, SipHash-2-4, ChaCha20. No single primitive dominates the flag list.
- **At BF=32** (`tmp/results/single_bf32/04_phase2b.log`): **10 / 90 cells flagged**, distributed across a different set of primitives (Areion-SoEM-256, Areion-SoEM-512, BLAKE2b-512, BLAKE2s appear more; ChaCha20 does not appear).
...
Attacker forces the sender to encrypt two plaintexts `p_1 ≠ p_2` under the SAME `(noiseSeed, dataSeed, startSeed)` and the SAME nonce. The attacker knows both plaintexts (Full KPA) — either because they are public protocol payloads or because they leaked via another channel. The attacker does NOT know `startPixel` or the per-pixel configuration map `(noisePos, rotation, channelXOR)`. The chain assumes the default encoding mode (`SetLockSoup(0)`; on Single Ouroboros also `SetBitSoup(0)` since the two flags are coupled symmetrically there) — engaging the Lock Soup overlay applies a per-chunk PRF-keyed bit-permutation before COBS and the barrier, so Layer 1's `(noisePos, rotation)` constraint matching loses its anchor on the colliding pair and the chain exits with no signal regardless of the Layer-2 / Layer-3 stages. The empirical matrix below is therefore conditional on the user not having enabled the overlay (see [ITB.md § 8 condition 6](ITB.md#8-nonce-reuse-only-if-every-condition-holds) and [SECURITY.md §20](SECURITY.md#20-encoding-mode-security-summary)). Under [`SCIENCE.md` §2.5](SCIENCE.md#25-nonce-reuse-analysis)'s locality claim, the expected damage is a two-time pad on these 2 – 3 colliding messages only; seeds remain secret under PRF non-invertibility and future messages (fresh nonce) are unaffected. Under an invertible / linear primitive, the same claim does not hold — seed retention is no longer guaranteed.
...
2. **Under a PRF primitive (Areion-SoEM / BLAKE2 / BLAKE3 / AES-CMAC / SipHash-2-4 / ChaCha20): just the partially broken hash-output stream, useless.** The reconstructed `dataHash` stream is statistically indistinguishable from uniform random (188/188 on NIST STS). Inverting it to recover `dataSeed` requires breaking the PRF — out of scope by assumption. `startSeed` and `noiseSeed` inversion hit the same wall. The attack surface closes on this output.
...
- **All-10-primitives Full KPA documentation run**: not performed. Demasking is primitive-independent at the recovery step (observation 1 above), so adding BLAKE2b, BLAKE2s, SipHash, etc..., would produce six near-identical tables. The Full KPA validation table in the main Phase 2d writeup already covers four widths (FNV-1a, MD5, BLAKE2b-512, BLAKE3) confirming the demasker formula works across all ChainHash widths.
...
1. **PRF + carry-chain primitives are related-seed secure under ITB wrapping.** Tested matrix covers every 1024-bit ChainHash primitive in the suite; none of the 10 non-GF(2)-linear primitives (PRF-grade + MD5) shows a differential on `data` or `start` above the df=255 random band.
...
This section establishes the empirical attack cost for the weakest invertible-but-non-linear primitive in the matrix (FNV-1a) at ITB's architectural minimum configuration (`keyBits = 512`, 4 rounds). It does NOT claim production ITB is vulnerable: FNV-1a is marked `Fully broken` in the [Hash matrix](#hash-matrix) and is an unexported lab control with no public API path through `NewSeed{128,256,512}`. Users who wire a PRF-grade primitive from the [Hash matrix](#hash-matrix) are not exposed to SAT-based seed recovery — under each primitive's PRF assumption, any successful SAT inversion would constitute a PRF distinguisher and is therefore ruled out by definition; published SAT cryptanalysis reaches only reduced-round variants in isolation, consistent with the assumption. ITB's per-pixel envelope compounds multiplicatively on top of this primitive-level PRF-hardness. The Phase 2g empirical figure is therefore a lower bound on attacker cost against PRF-grade primitives, not a measurement of them. Per-round cost scaling from 4 → 8 → 16 ChainHash rounds (corresponding to `keyBits = 512 → 1024 → 2048`) is analytical with wide error bars — see [Phase 2a](#phase-2a--chainhash-analysis-and-the-three-layer-defense-structure).

**Solver-choice robustness.** The ~8 h figure is not an artefact of a generic SAT encoding. A structure-aware T-function solver — exploiting that the FNV-1a lo-lane is a carry-up-only T-function in the seed bits — collapses the isolated ChainHash inversion (channel-decode given the barrier) to under a second, but does not reduce the barrier itself: the per-pixel `noise_pos` / rotation joint search stays dominant and is primitive-independent. Injecting that solver into a SAT search as a Z3 user-propagator theory oracle ran slower than native Bitwuzla, not faster — Z3's CDCL over the barrier clauses is less efficient than Bitwuzla's native bitvector reasoning, and the per-conflict consistency proof carries non-linear, so no GF(2) shortcut applies. The barrier also compounds multiplicatively: the ~90-bit per-pixel ambiguity does not decompose into independent SAT sub-problems that could be solved separately to sidestep it. The structure-aware solver wins the ChainHash sub-problem, not the barrier.
...
| **Triple Interlocked Bit Soup** (`SetBitSoup(1)` + `SetLockSoup(1)`, Insane Interlocked Mode) | Per-snake decoded content is bit-permuted garbage AND the 24-bit chunk permutation is per-chunk PRF-keyed from a 2^33 balanced-partition mask space (not the public Bit Soup permutation) — no shared algebraic structure couples chunks across, so every crib chunk multiplies attacker enumeration by ~2^33 without contributing constraints solvable jointly. The joint SAT instance is information-theoretically under-determined regardless of crib density | Triple Bit Soup lower bound × 2^33 (single crib chunk floor) ≈ **10¹⁸ core-hours** (~3 × 10¹² years on 289 cores) — beyond cosmic time scales. Realistic protocol cribs supply c=2–4 chunks, raising the floor by an additional 2^33 per chunk (~10²⁷ core-hours at c=2, ~10³⁶ at c=3). Under-determination: no SAT path applies — solver throughput, including any hypothetical PRF-inversion shortcut, does not convert an under-determined instance into a determined one |
| **Single Interlocked Bit Soup** (`SetLockSoup(1)` or `SetBitSoup(1)`, Insane Interlocked Mode on Single Ouroboros) | The 24-bit chunk permutation is per-chunk PRF-keyed from a 2^64 full-S₂₄ permutation space (Lehmer-code unrank of 24! addressed by 64 bits of per-chunk PRF output) — no shared algebraic structure couples chunks across, so every crib chunk multiplies attacker enumeration by ~2^64 without contributing constraints solvable jointly. The 2^64-per-chunk floor compensates for Single's absence of the 3-way snake split that gives Triple its public-encoding barrier. The SAT instance is information-theoretically under-determined regardless of crib density | Single FNV-1a baseline × 2^64 (single crib chunk floor) ≈ **2^77 core-hours** (~10²² years on 289 cores at the empirical 8 h Single SAT anchor). Realistic protocol cribs supply c=2–4 chunks, raising the floor by an additional 2^64 per chunk — beyond any meaningful brute-force frame at any chunk count. Under-determination: no SAT path applies; the per-chunk permutation is unobservable without the noiseSeed |
...
**Optional Lock Seed (orthogonal layer).** `AttachLockSeed` on the native path / `Encryptor.SetLockSeed(1)` on the Easy Mode path installs a dedicated lockSeed for the bit-permutation derivation channel. The per-chunk PRF closure for the Lock Soup overlay then captures both the lockSeed's Components AND its PRF primitive (independent of the noiseSeed within the same native hash width), so a structural shortcut against the noiseSeed primitive cannot leak into the lock-channel derivation, and vice versa. The dominant 2³³-per-chunk (Triple Interlocked Bit Soup) and 2⁶⁴-per-chunk (Single Interlocked Bit Soup) Lock Soup floors in the table above are unaffected — the lockSeed layer adds algorithm-diversity defence-in-depth on top of the keying-material isolation already counted, rather than altering the per-chunk floor figures.
...
### Defensive reserve: Lock Soup (Insane Interlocked Mode, opt-in overlay)

`SetLockSoup(1)` is the keyed-bit-permutation overlay on top of bit soup; setting it automatically engages `SetBitSoup(1)` (see [ITB3.md § Lock Soup](ITB3.md#lock-soup-insane-interlocked-mode-opt-in-overlay-on-bit-soup) for the Triple-side accessible explanation, [ITB.md § Lock Soup](ITB.md#15-lock-soup-insane-interlocked-mode-opt-in-overlay-on-bit-soup) for Single).

On Triple Ouroboros, the overlay replaces the fixed public 24-bit chunk permutation of plain Bit Soup with a per-chunk balanced 8-of-24 mask triple drawn from a 2^33 mask space (combinatorial unrank of `C(24,8) × C(16,8)` partitions), derived deterministically per chunk from the noiseSeed and nonce via a single PRF call.

On Single Ouroboros, `SetBitSoup(1)` and `SetLockSoup(1)` are coupled symmetrically — either flag activates the Lock Soup overlay (the two flags engage the same code path on Single, vice versa). The Single overlay replaces the 24-bit chunk identity with a per-chunk full S₂₄ permutation drawn from a 2^64 space (Lehmer-code unrank of 24! addressed by 64 bits of per-chunk PRF output). The 2^64-per-chunk floor compensates for Single's absence of the 3-way snake split that gives Triple its public-encoding barrier.

Lock Soup strengthens the instance-formulation barrier from the public-encoding layer (Bit Soup) to the keyed-encoding layer. Under any KPA — Partial, Full, or even hypothetical adversaries with full plaintext-ciphertext pairs — every crib chunk multiplies attacker enumeration by ~2^33 (Triple) or ~2^64 (Single) without contributing constraints that couple chunks across, because the per-chunk mask / permutation is independently keyed and unobservable without the noiseSeed. SAT cryptanalysis ceases to be a computational-hardness problem and becomes an instance-formulation impossibility: there is no fixed bit-position-to-lane mapping for the solver to anchor on. Improvements in solver throughput, including any hypothetical PRF-inversion shortcut, do not affect the under-determination — the instance has no unique solution to be discovered faster.

Performance cost is approximately 2×–7× slower than plain Bit Soup depending on platform; the BMI2 PEXT/PDEP path on x86 (Haswell+, Excavator+/Zen 1+) on Triple, AVX-512 VBMI VPERMB + VPMOVM2B + VPTESTMB path on x86 (Ice Lake / Tiger Lake / Rocket Lake / Sapphire Rapids+, Zen 4 / Zen 5) on Single, sit near the lower bound; pure-Go fallbacks near the upper. The trade-off is acceptable only where the architectural barrier is the deployment priority — long-term archive encryption, high-value off-line ciphertexts, defense-in-depth layering above already-strong PRF guarantees. Default `SetLockSoup(0)` leaves Bit Soup behaviour unchanged. Both flags must agree across the encrypt and decrypt sides of the channel; mismatched modes produce wrong-seed-style garbage with no error oracle (plausible-decryption invariant preserved). Ciphertext wire format is identical across all three modes (Byte Level / Bit Soup / Lock Soup).

`SetLockBatch(1)` is the performance variant of Lock Soup: the per-chunk overlays for a whole group of chunks are derived from a single primitive call instead of one call per chunk, raising throughput several-fold (on x86 hosts with AVX-512 — F plus VPOPCNTDQ for the Single permutation — through lane-parallel AVX-512 kernels; other hosts take a scalar fallback, so the lift is largest on x86 and modest elsewhere, with AArch64 limited to hash-call batching as Go's assembler carries no SVE2). The guarantee is **PRF-conditional**: the batched lanes are independently keyed only for PRF-grade primitives, so Lock Batch is recommended in every case **under the PRF assumption** with no security loss for PRF-grade hashes. The default one-mask-per-chunk Lock Soup retains the stronger claim that holds even for the below-spec primitives of the [Hash matrix](#hash-matrix), so Lock Batch is not unconditionally as strong as default Lock Soup. The flag is symmetric — batched and non-batched per-chunk masks differ, so it must be set identically on the encrypt and decrypt sides — and inert unless Lock Soup is engaged.

An optional Lock Seed (`AttachLockSeed` on the native path, `Encryptor.SetLockSeed(1)` on the Easy Mode path) routes the bit-permutation derivation channel through a dedicated seed slot whose Components AND PRF primitive are independent of the noise-injection channel within the same native hash width — algorithm-diversity defence-in-depth on top of the per-chunk PRF-keyed permutation. A structural shortcut against the noiseSeed primitive cannot leak into the lock-channel derivation, and vice versa.
...
| [Proof 10](PROOFS.md#proof-10-guaranteed-drbg-residue-no-perfect-fill) (CSPRNG-fill residue) | ✅ Phase 3b — 188/188 pass for all 12 primitives at both BF=1 and BF=32 on typical runs; fill dominates the stream in both regimes to within Proof 10's guaranteed minimum |
