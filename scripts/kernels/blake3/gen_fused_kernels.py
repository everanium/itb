#!/usr/bin/env python3
"""Emit the BLAKE3 fused ChainHash cascade kernels for hashes/internal/blake3asm.

One file per (shape, lanes, tier). Width 256 only (BLAKE3 keyed mode:
32-byte key as the initial chaining value, four component words per
cascade group, 32-byte output, 32-bit state words); shapes 13 / 20 / 36 /
68; lanes x4 (four data lanes over one shared component slice, one dword
lane per pixel), x8 (eight lanes, the widened per-pixel hooks and the
...
Cascade evaluated per lane (see blake3asm_fused.go):
    h = 0
    for each component group g (4 words):
        seed = g ^ h
        h = BLAKE3-keyed(key, data ⊕ seed)
    out = h
which is Seed256.ChainHash over the parent package's keyed BLAKE3
closure: data zero-padded to at least the 32-byte seed-injection width
with the seed words XORed over the first 32 bytes, hashed by BLAKE3 in
keyed mode with the fixed key as the chaining value. The input is one
chunk: the 13-, 20- and 36-byte shapes are one compression (block length
32 / 32 / 36, flags KEYED_HASH | CHUNK_START | CHUNK_END | ROOT); the
68-byte shape spans two blocks (64 bytes under KEYED_HASH | CHUNK_START,
then 4 bytes under KEYED_HASH | CHUNK_END | ROOT with the first block's
chaining value as the state). The output is the first eight words of the
root compression, v[i] ⊕ v[i+8]. The message words are the seed-injected
words and the data-only words; the data words are staged once per call
from the lane pointers into the frame, and each cascade round rebuilds
the seed-injected words as D ⊕ component ⊕ h from the staged data dword,
the component dword broadcast — each 64-bit component and output word
straddles two 32-bit message words, low half first — and the previous
round's output: one VPTERNLOGD per word on the EVEX tier. The state is
re-initialised each round from the key words (broadcast from the key
pointer) and a per-kernel table holding IV[0..3], the zero counter, the
block length and the flags of the block.

Register plans:
    avx512 x4   X0..X15 the compression state v[0..15], X16..X31 the
                message words m[0..15]; the seed-word rebuild is
                VPTERNLOGD with an embedded-broadcast memory operand and
                the fold v ⊕ v' is VPXORD, so no scratch register is
                needed
    avx512 x8   the same plan on YMM registers (eight lanes): Y0..Y15 the
                state, Y16..Y31 the message words, 32 of 32; the shape-68
                kernel (544-byte frame) fits the NOSPLIT budget
    ...
    gpr amd64   v[0..15] in AX, BX, CX, DX, SI, DI, BP, R8..R11, R12..R15
                ...
                component / key / output pointers in the frame; R9 /
                R10 / R11 / R12 are the loop-top temps while only
                h = v[0..7] is live
...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "..", "hashes", "internal", "blake3asm")
SHAPES = [13, 20, 36, 68]
...
CHUNK_START, CHUNK_END, ROOT, KEYED_HASH = 1, 2, 8, 16


def schedule():
    """The seven message schedules: the identity, then the permutation
    applied to the previous schedule."""
    s = list(range(16))
    out = [s]
    for _ in range(6):
        s = [s[p] for p in PERM]
...
def layout(n):
    ...
    words — ('seed', k, d0, size), ('data', d0, size) or ('zero',) — the
    block length and the flags. A seed word k (0..7) is the k-th dword of
    the message: data bytes [d0, d0+size), component dword k (the low
    half of component k/2 for even k, the high half for odd k) and output
    dword k."""
    total = max(n, INJECT)
    ...
    for b in range(nblocks):
        ...
        for w in range(16):
            ...
            if off >= total:
                ...
                size = max(0, min(off + 4, n) - off)
                if off < INJECT:
                    words.append(("seed", off // 4, off, size))
                elif size > 0:
                    words.append(("data", off, size))
                else:
                    words.append(("zero",))
        flags = KEYED_HASH
        if b == 0:
        ...
        blocks.append({"words": words, "blen": min(total - BLOCK * b, BLOCK), "flags": flags})
    return blocks


def init_table(block):
    """The state words v[8..15] a compression of this block starts from:
    IV[0..3], the zero counter, the block length and the flags. v[0..7]
    is the chaining value — the key for the first block, the previous
    block's output for the second."""
    return list(IV[:4]) + [0, 0, block["blen"], block["flags"]]
...
// {tier_desc} {what} for BLAKE3 at the
...
def tables(blocks, x8=False):
    ...
    for b, block in enumerate(blocks):
        ...
        lines.append(f"GLOBL tab{b}<>(SB), RODATA|NOPTR, $32")
        lines.append("")
    if x8:
        for i in range(8):
...
def evex_macros(p):
    ...
    for gi, (a, b, c, d) in enumerate(G_ORDER):
        rows.append(f"\tBLAKE3_G({p}{a}, {p}{b}, {p}{c}, {p}{d}, s{2 * gi}, s{2 * gi + 1})")
    r = "#define BLAKE3_ROUND(" + ", ".join(f"s{i}" for i in range(16)) + ") \\\n" + "; \\\n".join(rows)
    return g + "\n\n" + r + "\n"


def evex_rounds(p):
    return [f"\tBLAKE3_ROUND({', '.join(f'{p}{16 + s}' for s in sigma)})" for sigma in ROUNDS]
...
def evex_kernel(n, x8=False, fill=False):
    ...
    fn = f"blake3FusedChain{n}x{lanes}Avx512Asm"
    ...
    for b, block in enumerate(blocks):
        ...
        lines.append("")
        for i in range(8):
            lines.append(f"\tVPXORD {p}{8 + i}, {p}{i}, {p}{i}")
    lines.append("")
...
def neon_kernel(n):
    ...
    fn = f"blake3FusedChain{n}x4NeonAsm"
    ...
    for b, block in enumerate(blocks):
        ...
        emit("")
        v = regs.v
        # Fold in place: h_i = v_i ⊕ v_{i+8}, then move the rotated b
        # words back to V4..V7.
        for i in range(8):
...
// {tier_desc} fused ChainHash cascade kernel for BLAKE3 at the
...
def gpr_kernel_amd64(n):
    ...
    fn = f"blake3FusedChain{n}x1GprAsm"
    ...
    # data-only words) into the frame and the data-only words of block 0
    # into their m slots.
    ...
    for b, block in enumerate(blocks):
        ...
        L.append("")
        if b == 0:
            # The chaining value is the key (the key pointer through
            # v[10]'s register, dead at the loop top).
            L.append(f"\tMOVQ {keyp}(SP), R11")
            L += [f"\tMOVL {4 * k}(R11), {R[k]}" for k in range(8)]
        else:
            L += load_words(b)
        for k in range(8):
            if R[8 + k] is None:
                ...
                L.append(f"\tMOVL $0x{tab[k]:08x}, {R[8 + k]}")
        L.append("")
        for sigma in ROUNDS:
            for gi, (a, bb, c, d) in enumerate(G_ORDER):
...
def gpr_kernel_arm64(n):
    ...
    for b, block in enumerate(blocks):
        L.append("")
        if b == 0:
            L += [f"\tMOVWU {4 * k}(R0), {R[k]}" for k in range(8)]
        else:
            L += load_words(b)
        L.append(f"\tMOVD $tab{b}<>(SB), R25")
        for k in range(8):
            L.append(f"\tMOVWU {4 * k}(R25), {R[8 + k]}")
        L.append("")
        for sigma in ROUNDS:
            for gi, (a, bb, c, d) in enumerate(G_ORDER):
...
def render_all():
    files = {}
    for n in SHAPES:
        files[f"blake3_fusedchain256_{n}x4_avx512_amd64.s"] = evex_kernel(n) + "\n"
        files[f"blake3_fusedchain256_{n}x4_avx2_amd64.s"] = avx2_kernel(n) + "\n"
        files[f"blake3_fusedchain256_{n}x4_neon_arm64.s"] = neon_kernel(n) + "\n"
        files[f"blake3_fusedchain256_{n}x1_gpr_amd64.s"] = gpr_kernel_amd64(n)
