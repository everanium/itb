...
own source (the initialisation vector, the block lengths and the flags
are folded into per-kernel read-only tables the generator computes), and
must reproduce the committed `.s` files
...
```
python3 scripts/kernels/blake3/gen_fused_kernels.py --check
```

Run the generator without `--check` to (re)write its files; the output
directory is resolved relative to the script
(`../../../hashes/internal/blake3asm/`). After any generator change, run
the `--check` form and confirm `git diff --stat hashes/internal/blake3asm/`
...
(see `hashes/internal/blake3asm/blake3asm_fused.go`):
`blake3_fusedchain256_<shape>x4_<tier>_amd64.s` (tiers avx512 / avx2),
`blake3_fusedchain256_<shape>x4_neon_arm64.s`, and the eight-lane YMM
kernels of the avx512 tier: `blake3_fusedchain256_{20,36,68}x8_avx512_amd64.s`,
the per-pixel kernels at the nonce-buf shapes, and the Interlocked
Barrier fill kernel `blake3_fusedchain256_13x8_avx512_amd64.s` (the
...
`blake3_fusedchain256_<shape>x1_gpr_{amd64,arm64}.s`, the
...
Per cascade round the kernel runs keyed BLAKE3 per lane over
`data ⊕ seed` — data zero-padded to at least the 32-byte seed-injection
width with the seed words XORed over the first 32 bytes, hashed with the
32-byte fixed key as the initial chaining value — with the seed of the
round being the component group XOR the previous round's output. The
input is one chunk: the 13-, 20- and 36-byte shapes are one compression
(block length 32 / 32 / 36 under `KEYED_HASH | CHUNK_START | CHUNK_END |
ROOT`); the 68-byte shape spans two blocks (64 bytes under `KEYED_HASH |
CHUNK_START`, then 4 bytes under `KEYED_HASH | CHUNK_END | ROOT` with the
first block's output as the chaining value). The message words split
into the seed-injected words and the data-only words; every 64-bit
component and output word straddles two 32-bit message words, low half
first. The data words are staged once per call into the frame, and the
seed-injected words are rebuilt each round from the staged data dword,
the component dword broadcast and the previous output — one `VPTERNLOGD`
per word on the EVEX tier, a broadcast and two `VPXOR` on the AVX2 tier,
a `VDUP` and two `VEOR` on NEON. The state is re-initialised each round
from the key words (broadcast from the key pointer) and a read-only
table that carries `IV[0..3]`, the zero counter, the block length and
the flags of the block; the output of a compression is `v ⊕ v'`, the
first eight words of the root output. The module docstring of the
generator lists the register plan of every tier.

## Register budget

The 32-bit-lane state of BLAKE3 fills 16 registers at one dword lane
...
`hashes/internal/blake3asm/blake3asm_fused.go` and is pinned to the
pure-Go reference there (`ScalarFusedChain256`, built on the
keyed mode of `github.com/zeebo/blake3`) by the in-package parity tests
(`go test ./hashes/internal/blake3asm/`, every tier by direct call plus
`ITB_FORCE_HASH_TIER` / `ITB_FORCE_INTERLOCK_PRF_FILL_TIER` probes), by
the `hashes` package's known-answer vectors of the cascade
(`blake3_cascade_kat_test.go`, produced under `-tags noitbasm`), and by
...
(`hashes/internal/blake3asm/blake3asm_entropy*_test.go`, every tier by
