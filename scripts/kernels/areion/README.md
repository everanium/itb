...
`areion_fusedchain{256,512}_<shape>x{1,4}_neon_arm64.s`, and — at width
256 only — `areion_fusedchain256_13x8_avx512_amd64.s`, the eight-lane
batch-16 Interlocked Barrier fill kernel: two four-lane ZMM groups whose
cascade rounds are interleaved instruction by instruction, so four
independent `VAESENC` chains are in flight per permutation instead of the
two a single lane group exposes, with the eight 13-byte fill blocks
synthesised in-register from `groupIdxBase`. Width 512 has no dedicated
fill kernel: its batch-16 hook covers four groups per call, exactly the
four-lane kernel over Go-synthesised blocks, and so do the YMM / XMM /
NEON tiers of width 256.
...
are data-invariant across rounds and are staged once per call; the ZMM
tier keeps them and both keys in registers, the YMM tier reads the blocks
back as memory operands (and, at width 512, keeps k2 in the frame), the
XMM and NEON tiers reload the blocks from the frame per chunk. The module
docstring of the generator lists the register plan of every tier.
