#!/usr/bin/env bash
# HARNESS.md § 4.1 row 1 (t1ha1_64le) — Axis B ITB-wrapped bias audit.
#
# Single-primitive driver that generates ITB corpora under known_ascii
# plaintext mode with t1ha1 wrapped into ChainHash128 (via the parallel
# two-lane adapter t1ha1Hash128 defined in harness_test.go), then
...
# surface is the chainhashes/t1ha1.py mirror, already parity-validated
...
#   RESULTS_TAG default: "harness_bias_audit_t1ha1"
#   TIMEOUT_S   default: 600     (per-cell Go generator timeout)
#
# Output:
#   tmp/attack/t1ha1stress/corpus/size_<N>_ascii/          — generated corpora
#   tmp/attack/t1ha1stress/<RESULTS_TAG>/matrix_summary.jsonl
#   tmp/attack/t1ha1stress/<RESULTS_TAG>/matrix.log
#   tmp/attack/t1ha1stress/<RESULTS_TAG>/probe_t1ha1_<N>_ascii.log

set -euo pipefail

SIZES=${SIZES:-"524288 1048576"}
PROBE_SIZE=${PROBE_SIZE:-"auto"}
RESULTS_TAG=${RESULTS_TAG:-"harness_bias_audit_t1ha1"}
TIMEOUT_S=${TIMEOUT_S:-600}

PROJ_DIR=$(cd "$(dirname "$0")/../.." && pwd)
cd "$PROJ_DIR"

CORPUS_ROOT="tmp/attack/t1ha1stress/corpus"
RESULTS_ROOT="tmp/attack/t1ha1stress/${RESULTS_TAG}"
...
echo "==========================================================================="
echo "  primitive    : t1ha1_64le (harness track; not in Phase 2a matrix)"
echo "  sizes        : $SIZES"
...
for size in $SIZES; do
    ...
    cell_log="${RESULTS_ROOT}/probe_t1ha1_${size}_ascii.log"
    ...
    ITB_HARNESS_T1HA1_MODE=known_ascii \
    ITB_HARNESS_T1HA1_SIZE="$size" \
    ITB_HARNESS_T1HA1_OUT="$cell_dir" \
    go test -tags redteam -run TestRedTeamHarnessGenerateT1ha1NonceReuse \
    ...
    # Reuse the existing raw_mode_bias_probe.py; point it at chainhashes.t1ha1
    # (Python mirror bit-exact-verified against the Go t1ha1_64le reference
...
row = {
    "primitive": "t1ha1",
    "hash_display": meta.get("hash_display", "t1ha1_64le"),
