#!/usr/bin/env python3
"""Axis A lab bias harness for mx3_hash (HARNESS.md § 3.1).

Runs SMHasher-subset statistical bias tests on the **native 64-bit
mx3_hash output** — which equals the hLo projection that ITB's encoding
path observes when mx3 is wrapped via the parallel two-lane 128-bit
adapter (lo lane = mx3_hash(data, seed_lo)). hHi is constructed by a
separate mx3_hash call with seed_hi and is discarded at the ITB encoding
boundary; the quantity that actually reaches ChainHash composition is
what this harness measures.

Tests:
  * Avalanche bias across key sizes. Note: Mx3's documented SMHasher
    weakness is NOT avalanche (which passes) but PerlinNoise — 2 × 10¹² ×
    over-expected collisions on coordinate-structured inputs (rurban/smhasher
    mx3.txt). This harness measures avalanche as a NO-SIGNAL baseline;
    PerlinNoise is not reproduced here (it requires a different corpus
    structure). The bias that matters for ITB is covered by Axis A'
    (structural-input bias under ITB-realistic methodology) and Axis B
    (ITB-wrapped ciphertext surface absorption).
...
from chainhashes.mx3 import mx3_hash  # noqa: E402

OUTPUT_BITS = 64  # mx3_hash native output width
MASK64 = (1 << 64) - 1
...
def momentchi2_test(n_samples: int, rng_seed: int) -> dict[str, Any]:
    """Chi-square test on the byte distribution of mx3_hash output.
...
def sparse_keyset_test(n_bits_set_max: int, key_bytes: int,
    ...
    ≤ 4 bits set. mx3_hash's sparse behaviour is not individually
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Axis A lab bias on mx3_hash hLo projection")
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        ...
        "published_reference": {
            "source": "rurban/smhasher mx3.txt",
            "avalanche": ("clean baseline — Mx3's documented SMHasher "
