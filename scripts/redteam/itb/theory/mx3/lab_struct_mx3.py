#!/usr/bin/env python3
"""Axis A' — structural-input bias on mx3_hash hLo projection.

Companion to lab_bias_mx3.py. Differs in methodology per the ITB-attack-
...
three plaintext kinds (ascii already covered by lab_bias_mx3.py random
avalanche). JSON is the more sensitive detector on the hypothesis that
mx3's 32-byte block loop + 1..31-byte tail handler responds to
...
Usage:
  python3 scripts/redteam/itb/theory/mx3/lab_struct_mx3.py \\
      ...
      --json-report ~/scratch/redteam/mx3stress/axis_a_struct_smoke.json
...
def struct_bias_test(n_instances: int, instance_size: int,
    ...
    for i in range(n_instances):
        ...
        hashes[i] = mx3_hash(instance, fixed_seed)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Axis A' structural-input bias on mx3_hash")
