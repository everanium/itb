#!/usr/bin/env python3
"""Axis A lab bias harness for seahash64 (HARNESS.md § 3.1).

Runs SMHasher-subset statistical bias tests on the **native 64-bit
seahash64 output** — which equals the hLo projection that ITB's encoding
path observes when seahash is wrapped via the parallel two-lane 128-bit
adapter (lo lane = seahash64(data, seed_lo)). hHi is constructed by a
separate seahash64 call with seed_hi and is discarded at the ITB encoding
boundary; the quantity that actually reaches ChainHash composition is
what this harness measures.

Tests:
  * Avalanche bias across key sizes. Note: SeaHash's documented SMHasher
    ...
    seahash.txt). This harness measures avalanche as a NO-SIGNAL baseline;
...
OUTPUT_BITS = 64  # seahash64 native output width
MASK64 = (1 << 64) - 1
...
def momentchi2_test(n_samples: int, rng_seed: int) -> dict[str, Any]:
    """Chi-square test on the byte distribution of seahash64 output.
...
def sparse_keyset_test(n_bits_set_max: int, key_bytes: int,
    ...
    ≤ 4 bits set. seahash64's sparse behaviour is not individually
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Axis A lab bias on seahash64 hLo projection")
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        ...
        "published_reference": {
            "source": "rurban/smhasher seahash.txt",
            "avalanche": ("clean baseline — SeaHash's documented SMHasher "
