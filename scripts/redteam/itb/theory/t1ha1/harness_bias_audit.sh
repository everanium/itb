#!/usr/bin/env bash
# HARNESS.md § 5.1 (t1ha1_64le) — Axis B ITB-wrapped bias audit.
#
# Single-primitive driver that generates ITB corpora under known_ascii
# plaintext mode with t1ha1 wrapped into ChainHash128 (via the parallel
# two-lane adapter t1ha1Hash128 defined in harness_test.go), then runs
# the raw-mode bias probe at scripts/redteam/itb/theory/_common/raw_mode_bias_probe.py
# against each corpus.
#
# Corpus generation uses a dedicated Go test entry point —
# TestRedTeamHarnessGenerateT1ha1NonceReuse in harness_shelf_test.go —
# which delegates to the same runNonceReuse128 body as the sibling
# harness drivers, so encryption, cell.meta.json schema,
# config.truth.json, and summary.json are identical across all four
# shelf primitives. The chainhashes/t1ha1.py Python mirror is parity-
# validated against the Go reference by
# scripts/redteam/itb/theory/_common/chainhashes/_parity_test.py.
#
# Usage:
#   bash scripts/redteam/itb/theory/t1ha1/harness_bias_audit.sh
#
# Env overrides:
#   SIZES                          default: "524288 1048576"
#                                  (512 KB + 1 MB, matching the shelf
#                                  baseline; add 4194304 for a 4 MB
#                                  stress cell.)
#   PROBE_SIZE                     default: "auto"  (scales with corpus size)
#   RESULTS_TAG                    default: "harness_bias_audit_t1ha1"
#   TIMEOUT_S                      default: 600     (per-cell Go generator timeout)
#   HARNESS_BIAS_AUDIT_OUTPUT_DIR  default: ${HOME}/scratch/redteam/harness_bias_audit_t1ha1
#                                  (corpus + results tree; per the project guidelines
#                                  working-tree layout — no tmp/ in repo)
#
# Output:
#   ${OUTPUT_DIR}/corpus/size_<N>_ascii/          — generated corpora
#   ${OUTPUT_DIR}/<RESULTS_TAG>/matrix_summary.jsonl
#   ${OUTPUT_DIR}/<RESULTS_TAG>/matrix.log
#   ${OUTPUT_DIR}/<RESULTS_TAG>/probe_t1ha1_<N>_ascii.log

set -euo pipefail

SIZES=${SIZES:-"524288 1048576"}
PROBE_SIZE=${PROBE_SIZE:-"auto"}
RESULTS_TAG=${RESULTS_TAG:-"harness_bias_audit_t1ha1"}
TIMEOUT_S=${TIMEOUT_S:-600}
OUTPUT_DIR=${HARNESS_BIAS_AUDIT_OUTPUT_DIR:-"${HOME}/scratch/redteam/harness_bias_audit_t1ha1"}

PROJ_DIR=$(cd "$(dirname "$0")/../../../../.." && pwd)
cd "$PROJ_DIR"

CORPUS_ROOT="${OUTPUT_DIR}/corpus"
RESULTS_ROOT="${OUTPUT_DIR}/${RESULTS_TAG}"
mkdir -p "$CORPUS_ROOT" "$RESULTS_ROOT"

MATRIX_SUMMARY="${RESULTS_ROOT}/matrix_summary.jsonl"
DRIVER_LOG="${RESULTS_ROOT}/matrix.log"
: > "$MATRIX_SUMMARY"
: > "$DRIVER_LOG"

echo "==========================================================================="
echo "t1ha1 harness bias-neutralization audit (HARNESS.md § 4.1, Axis B)"
echo "==========================================================================="
echo "  primitive    : t1ha1_64le (shelf harness track)"
echo "  sizes        : $SIZES"
echo "  mode         : known_ascii (Full KPA; strongest per-byte bit-7=0 bias)"
echo "  probe size   : $PROBE_SIZE"
echo "  results tag  : $RESULTS_TAG"
echo "  matrix sum   : $MATRIX_SUMMARY"
echo "  driver log   : $DRIVER_LOG"
echo "==========================================================================="
echo

for size in $SIZES; do
    cell_dir="${CORPUS_ROOT}/size_${size}_ascii"
    cell_log="${RESULTS_ROOT}/probe_t1ha1_${size}_ascii.log"

    echo "[${size}B ascii] generating corpus → ${cell_dir}"
    echo "[${size}B ascii] generating corpus → ${cell_dir}" >> "$DRIVER_LOG"

    # Go corpus generator — shelf harness entry point. cell.meta.json + ct_*.bin
    # emitted with the same schema as the sibling shelf corpora.
    ITB_HARNESS_T1HA1_MODE=known_ascii \
    ITB_HARNESS_T1HA1_SIZE="$size" \
    ITB_HARNESS_T1HA1_OUT="$cell_dir" \
    go test -tags redteam -run TestRedTeamHarnessGenerateT1ha1NonceReuse \
        -count=1 -v -timeout "${TIMEOUT_S}s" >> "$DRIVER_LOG" 2>&1

    echo "[${size}B ascii] running bias probe → ${cell_log}"
    echo "[${size}B ascii] running bias probe → ${cell_log}" >> "$DRIVER_LOG"

    # Invoke the raw-mode bias probe; point it at chainhashes.t1ha1 (Python
    # mirror bit-exact-verified against the Go t1ha1_64le reference by
    # scripts/redteam/itb/theory/_common/chainhashes/_parity_test.py).
    python3 scripts/redteam/itb/theory/_common/raw_mode_bias_probe.py \
        --cell-dir "$cell_dir" \
        --hash-module chainhashes.t1ha1 \
        --probe-size "$PROBE_SIZE" \
        --top-n 5 > "$cell_log" 2>&1

    # Parse the probe output into a single-row matrix summary. Uses a heredoc
    # Python invocation to keep everything in one file; no new dependencies.
    python3 - "$cell_log" "$size" "$cell_dir" >> "$MATRIX_SUMMARY" <<'PY'
import json
import re
import sys
from pathlib import Path

log_path, size_str, cell_dir = sys.argv[1], sys.argv[2], sys.argv[3]
size = int(size_str)
text = Path(log_path).read_text()

def _find(pattern, group=1, cast=None):
    m = re.search(pattern, text)
    if not m:
        return None
    v = m.group(group)
    return cast(v) if cast else v

# Read meta for nonce + total_pixels so the summary row cross-references cleanly.
meta = json.loads(Path(cell_dir + "/cell.meta.json").read_text())

row = {
    "primitive": "t1ha1",
    "hash_display": meta.get("hash_display", "t1ha1_64le"),
    "hash_width": meta.get("hash_width", 128),
    "size_bytes": size,
    "format": "ascii",
    "total_pixels": meta.get("total_pixels"),
    "start_pixel": meta.get("start_pixel"),
    "min_conflict_pct": _find(r"min:\s+([\d.]+)%", cast=float),
    "p01_conflict_pct": _find(r"p01:\s+([\d.]+)%", cast=float),
    "p05_conflict_pct": _find(r"p05:\s+([\d.]+)%", cast=float),
    "median_conflict_pct": _find(r"median:\s+([\d.]+)%", cast=float),
    "p95_conflict_pct": _find(r"p95:\s+([\d.]+)%", cast=float),
    "p99_conflict_pct": _find(r"p99:\s+([\d.]+)%", cast=float),
    "max_conflict_pct": _find(r"max:\s+([\d.]+)%", cast=float),
    "true_shift_rank": _find(r"TRUE shift \d+: rank (\d+)/(\d+)", cast=int),
    "total_shifts": _find(r"TRUE shift \d+: rank \d+/(\d+)", cast=int),
    "plateau_size": _find(r"plateau \(≤ same conflict\) = (\d+) shift", cast=int),
    "pred_min_bits_pct": _find(
        r"at min-conflict shift \d+:\s*\n\s*bits matched\s*:\s*\d+/\d+ = ([\d.]+)%",
        cast=float),
    "pred_true_bits_pct": _find(
        r"at TRUE shift \d+  \(lab-only\):\s*\n\s*bits matched\s*:\s*\d+/\d+ = ([\d.]+)%",
        cast=float),
}
print(json.dumps(row))
PY

    echo "[${size}B ascii] done"
    echo >> "$DRIVER_LOG"
done

echo
echo "==========================================================================="
echo "Done."
echo "  Matrix summary: $MATRIX_SUMMARY"
echo "  Driver log    : $DRIVER_LOG"
echo "==========================================================================="
echo
echo "Summary rows:"
cat "$MATRIX_SUMMARY"
