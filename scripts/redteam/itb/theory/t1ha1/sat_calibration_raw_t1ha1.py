...
# -*- coding: utf-8 -*-
"""
Phase 0 calibration: raw t1ha1 ChainHash lo-lane SAT inversion cost.
...
constraints on symbolic seed components, and measures wall-clock. Answers:
    ...
    "Can Z3 recover an N-round t1ha1 ChainHash seed (lo+hi lanes) from K
...
Usage:
    python3 sat_calibration_raw_t1ha1.py [--rounds 1]
                                       [--obs 16]
                                       [--timeout-sec 86400]
                                       [--seed-rng 42]
                                       [--json-report PATH]
...
from t1ha1_chain_lo_concrete import (  # type: ignore
    MASK64,
    t1ha1_chain_lo_concrete,
    t1ha1_chain_lo_z3,
)
...
def _generate_synthetic_instance(
    ...
    # Flat list [s0_lo, s0_hi, s1_lo, s1_hi, ...] — matches t1ha1_chain_lo_concrete API.
    seed_components = [rng.getrandbits(64) for _ in range(2 * rounds)]
    observations: List[Observation] = []
    for _ in range(num_obs):
        data = bytes(rng.getrandbits(8) for _ in range(20))
        target = t1ha1_chain_lo_concrete(seed_components, data, rounds)
    ...
    if status == "sat" and recovered_lo and recovered_hi:
        ...
        if holdout_rng is not None and holdout_count > 0:
            ...
            for _ in range(holdout_count):
                ...
                truth_target = t1ha1_chain_lo_concrete(
                    list(ground_truth), data, rounds,
                )
                recovered_target = t1ha1_chain_lo_concrete(
...
# only and never enters the SipHash-1-3 SipRound encoding (`t1ha1_chain_lo_z3`
# in `t1ha1_chain_lo_concrete.py`). The F/G/H/I boolean lattice and round
# function constants are untouched.
# ============================================================================


def _solve_t1ha1_cube_worker(worker_args: dict) -> dict:
    """Top-level (picklable) worker for cube-and-conquer dispatch.

    Builds the t1ha1 ChainHash z3 formula identically to `_run_cell`,
    ...
    _common_theory_dir = _Path(worker_args["common_theory_dir_str"])
    if str(_this_dir) not in _sys.path:
        _sys.path.insert(0, str(_this_dir))
    if str(_common_theory_dir) not in _sys.path:
        _sys.path.insert(0, str(_common_theory_dir))

    import z3 as _z3
    from t1ha1_chain_lo_concrete import (  # type: ignore
        MASK64 as _MASK64,
        t1ha1_chain_lo_z3 as _t1ha1_chain_lo_z3,
    )
...
def _run_cell_cnc(
    ...
    _this_dir = Path(__file__).resolve().parent
    _common_theory_dir = _this_dir.parent / "_common"
    ...
    jobs = [
        {
            ...
            "common_theory_dir_str": str(_common_theory_dir),
        }
    ...
    with ProcessPoolExecutor(max_workers=workers, mp_context=ctx) as pool:
        futures = {
            pool.submit(_solve_t1ha1_cube_worker, job): job["cube_id"]
            for job in jobs
        }
...
def main() -> int:
    ...
    ap.add_argument(
        "--seed-rng", type=int, default=0x54_31_48_41_43_41_4C,  # "T1HACAL"
        help="Python-level random seed for reproducibility",
    )
    ap.add_argument(
        "--json-report",
        default="tmp/attack/t1ha1stress/phase0_raw_sat_calibration.json",
    ...
    print(
        f"Phase 0 raw t1ha1 SAT calibration: "
