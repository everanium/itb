#!/usr/bin/env python3
"""Axis A' — structural-input bias on t1ha1_64le hLo projection.

Companion to lab_bias_t1ha1.py. Differs in methodology per the ITB-attack-
...
t1ha1's 32-byte block loop + 1..31-byte tail handler responds to
...
def struct_bias_test(n_instances: int, instance_size: int,
    ...
    for i in range(n_instances):
        ...
        hashes[i] = t1ha1_64le(instance, fixed_seed)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Axis A' structural-input bias on t1ha1_64le")
