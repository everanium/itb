...
Concessions (logged at startup, see the design notes):
...
def _build_cell_context(
    ...
    ci_nonce_hex = cell_info.get("main_nonce_hex") or cell_info["nonce_hex"]
    nonce = bytes.fromhex(ci_nonce_hex)
    ...
    if start_pixel_override is not None:
        ...
        # startPixel concession (disclosed, see the design notes):
...
def solve(
    ...
    for c in cells:
        for o in c.observations:
            ...
            # encoded via optimisation #1 from the design notes:
    ...
    if solver_backend == "bitwuzla":
        ...
        # parameter which applies to CDCL only). See the design notes
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="SMT backend. z3 (default) uses z3-solver Python bindings "
             ...
             "the design notes for why this matters on this "
             "formula.",
    )
    ap.add_argument(
    ...
    if args.brute_force_start_pixel:
        ...
        # Default report path respects the project guidelines working-tree layout: writes
        ...
        default_out_dir = Path(os.environ.get(
            ...
            str(Path.home() / "scratch" / "redteam" / "sat_harness_4round"),
        ))
        summary_path = Path(args.json_report or (default_out_dir / "phase3b_bruteforce.json"))
    ...
    if args.cube_and_conquer:
        ...
        # Same output-dir convention as the brute-force branch above.
        default_out_dir = Path(os.environ.get(
            "SAT_HARNESS_4ROUND_OUTPUT_DIR",
            str(Path.home() / "scratch" / "redteam" / "sat_harness_4round"),
        ))
        summary_path = Path(
            args.json_report or (default_out_dir / "phase3b_cc.json")
        )
        summary_path.parent.mkdir(parents=True, exist_ok=True)
        summary_path.write_text(json.dumps({
