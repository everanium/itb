#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Axis C SAT calibration: raw MurmurHash3 ChainHash lo-lane seed recovery.
...
the symbolic lo-lane seed components, and measures wall-clock. It settles, for
the pre-screen's `inv = ?` contrast primitive, the question the pre-screen
explicitly defers:

    "MurmurHash3 reads clean on every avalanche / differential / degree column
     and the screen declares no cheap structural inverse (inv = ?). Does the
     lo-lane seed fall to a SAT/SMT solver anyway at modest observation count,
     or does it resist — the outcome the pre-screen could not call?"
...
Unlike splitmix64 (a designed bijection, SAT-broken at rounds = 1 in seconds)
MurmurHash3 carries no documented solver-exploitable shortcut, so neither a
fast recovery nor a timeout is the structurally guaranteed outcome — that is
exactly why it is worth measuring. MurmurHash3 is a research pre-screen
primitive, not wired into ITB; a recovery here is not an attack on a deployed
primitive.

Usage:
    python3 sat_calibration_raw_murmur3.py [--rounds 2] [--obs 16]
...
_PHASE2_THEORY_DIR = _THIS_DIR.parent / "_common"
for _p in (_THIS_DIR, _PHASE2_THEORY_DIR):
    if str(_p) not in sys.path:
        sys.path.insert(0, str(_p))

from murmur3_chain_lo_concrete import (  # type: ignore
    MASK64,
    murmur3_chain_lo_concrete,
    murmur3_chain_lo_z3,
)
...
def _generate_synthetic_instance(rounds: int, num_obs: int, rng: random.Random):
    ...
    for _ in range(num_obs):
        ...
        target = murmur3_chain_lo_concrete(seed_components, data, rounds)
...
def _run_cell(rounds: int, observations: Sequence[Observation],
    ...
    for obs in observations:
        expr = murmur3_chain_lo_z3(z3, lo_syms, hi_syms, obs.data_bytes, rounds)
    ...
    if status == "sat" and recovered_lo:
        ...
        recovered_valid = all(
            murmur3_chain_lo_concrete(rec_components, obs.data_bytes, rounds) == obs.target_hlo
            for obs in observations)
        holdout_total = holdout_count
        for _ in range(holdout_count):
            data = bytes(holdout_rng.getrandbits(8) for _ in range(20))
            truth_t = murmur3_chain_lo_concrete(list(ground_truth), data, rounds)
            rec_t = murmur3_chain_lo_concrete(rec_components, data, rounds)
            if truth_t == rec_t:
                holdout_pass += 1
...
def main() -> int:
    ...
    print(f"Axis C raw murmur3 SAT calibration: rounds={rounds_list} "
