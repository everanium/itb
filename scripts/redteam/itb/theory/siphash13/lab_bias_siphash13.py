#!/usr/bin/env python3
"""Axis A lab bias harness for siphash13_hash (HARNESS.md § 3.1).

Runs SMHasher-subset statistical bias tests on the **native 64-bit
siphash13_hash output** — which equals the hLo projection that ITB's encoding
path observes when siphash13 is wrapped via the parallel two-lane 128-bit
adapter (lo lane = siphash13_hash(data, seed_lo)). hHi is constructed by a
separate siphash13_hash call with seed_hi and is discarded at the ITB encoding
boundary; the quantity that actually reaches ChainHash composition is
what this harness measures.

Tests:
  * Avalanche bias across key sizes. SipHash-1-3 is the reduced-round
    boundary case in HARNESS.md § 4.1 row 7: full SipHash-2-4 is a
    designed PRF with ~0.2 % worst-case avalanche bias on rurban's
    SMHasher table; the 1-3 reduced-round variant elevates that to
    ~0.9 % per published measurements. This harness reproduces the
    avalanche measurement on the hLo projection and quantifies how
    much of that elevated bias propagates through ChainHash composition
    to the ITB-observable surface (Axis B captures the absorption side).
...
PROJ = HERE.parents[4]
sys.path.insert(0, str(PROJ / "scripts" / "redteam" / "itb" / "theory" / "_common"))

from chainhashes.siphash13 import siphash13_hash  # noqa: E402

OUTPUT_BITS = 64  # siphash13_hash native output width
MASK64 = (1 << 64) - 1
...
def momentchi2_test(n_samples: int, rng_seed: int) -> dict[str, Any]:
    """Chi-square test on the byte distribution of siphash13_hash output.
...
def sparse_keyset_test(n_bits_set_max: int, key_bytes: int,
    ...
    ≤ 4 bits set. siphash13_hash's sparse behaviour is not individually
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Axis A lab bias on siphash13_hash hLo projection")
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        "primitive": "siphash13_hash",
        "axis": "A",
        "scope": "hLo projection on raw primitive (native 64-bit output)",
        "published_reference": {
            "source": "rurban/smhasher siphash.txt",
            "avalanche": ("clean baseline — SipHash-1-3's documented SMHasher "
                          "weakness is PerlinNoise catastrophic (2e12 × "
                          "over-expected collisions on coordinate-"
                          "structured input), not avalanche. This Axis A "
                          "test establishes a NO-SIGNAL baseline; the "
                          "architecturally relevant check lives in Axis B"),
        },
