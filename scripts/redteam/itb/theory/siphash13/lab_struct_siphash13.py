#!/usr/bin/env python3
"""Axis A' — structural-input bias on siphash13_hash hLo projection.

Companion to lab_bias_siphash13.py. Differs in methodology per the ITB-attack-
...
three plaintext kinds (ascii already covered by lab_bias_siphash13.py random
avalanche). JSON is the more sensitive detector on the hypothesis that
SipHash-1-3's 8-byte word compression with reduced 1-round mixing per
word responds to fixed-offset repetition through its limited per-block
diffusion; HTML serves as a complementary check on variable-length
structural patterns.
...
def struct_bias_test(n_instances: int, instance_size: int,
    ...
    for i in range(n_instances):
        ...
        hashes[i] = siphash13_hash(instance, fixed_seed)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Axis A' structural-input bias on siphash13_hash")
