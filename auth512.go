...
// [EncryptAuthenticated512]: threads cfg through every Cfg-aware
// accessor in the Single Ouroboros authenticated pipeline. Body
// otherwise identical to EncryptAuthenticated512, including the MAC-
...
func EncryptAuthenticated512Cfg(cfg *Config, noiseSeed, dataSeed, startSeed *Seed512, data []byte, macFunc MACFunc) ([]byte, error) {
	...
	process512Cfg(cfg, noiseSeed, dataSeed, startSeed, nonce, container, width, height, full, true, 0)
...
// DecryptAuthenticated512Cfg is the Cfg variant of
...
func EncryptAuthenticated3x512Cfg(cfg *Config, noiseSeed, dataSeed1, dataSeed2, dataSeed3, startSeed1, startSeed2, startSeed3 *Seed512, data []byte, macFunc MACFunc) ([]byte, error) {
	...
	// part2 COBS length increased by tagSize for container sizing
	cobsLens := [3]int{len(encs[0]), len(encs[1]), len(encs[2]) + tagSize}
	width, height := containerSizeAuth3_512Cfg(cfg, noiseSeed, dataSeed1, dataSeed2, dataSeed3, startSeed1, startSeed2, startSeed3, cobsLens)
	totalPixels := width * height
	third := totalPixels / 3
	thirdPixels2 := totalPixels - 2*third
	...
	go func() {
		process512Cfg(cfg, noiseSeed, dataSeed3, startSeed3, nonce, container[offset2:totalPixels*Channels], thirdPixels2, 1, full2, true, perThird)
		wg.Done()
	}()
	wg.Wait()
...
// with integrity protection using 512-bit hash seeds.
//
// Pipeline: data → COBS encode → build payload [COBS][0x00][fill]
...
// DecryptStreamAuthenticated512 decrypts and verifies a single
// Streaming AEAD chunk using 512-bit hash seeds. Returns the recovered
...
// chunk using Triple Ouroboros (512-bit variant). Streaming-binding
// components mirror [EncryptStreamAuthenticated512]: streamID,
...
// DecryptStreamAuthenticated3x512 decrypts and verifies a single
// Streaming AEAD chunk using Triple Ouroboros (512-bit variant).
...
// [EncryptStreamAuthenticated512]: threads cfg through every Cfg-aware
...
// DecryptStreamAuthenticated512Cfg is the Cfg variant of
// [DecryptStreamAuthenticated512].
