...
Parallax does **not** strengthen any individual cipher. Plaintext is split into fixed-size segments and each segment is XORed under one counter-mode keystream chosen from a user-configured palette by a per-message keyed schedule. The **security floor is the weakest cipher in the palette** — every byte is encrypted under exactly one slot's keystream, and the adversary's task on that byte is no harder than breaking that one cipher in isolation. The package's value is **vendor and jurisdictional diversity**: a deployed pipeline can avoid baking in a single foreign algorithm by mixing regionally-certified primitives across slots so that no one certifier owns the keystream layer.

The defense-in-depth contribution is **multiplicative only in the branch where the inner ITB barrier breaks**. While the barrier holds, parallax adds no measurable confidentiality margin; the inner construction is already what the wire-side adversary must defeat first. Parallax is best understood as a transparent envelope: a way to dilute exposure to a single primitive's catastrophic break, not a cryptographic upgrade over a strong inner layer.
...
- **Defense-in-depth scope.** The multiplicative DiD value of parallax materialises **only** in the counterfactual branch where the inner ITB barrier breaks. While the barrier holds, the encryption is already protected at the inner layer and parallax contributes no measurable additional confidentiality margin.
...
- Not a cryptographic upgrade over its strongest palette member. The encryption floor is the weakest palette cipher. The package's contribution is diversity / regulatory posture and a multiplicative defense-in-depth term that activates only in the counterfactual branch where the inner ITB barrier breaks.
