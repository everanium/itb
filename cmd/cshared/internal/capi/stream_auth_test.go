...
// streaming path across the shipped PRF primitives at width 1024,
// using the bound MAC closure in the constructed encryptor.
func TestEasyEncryptStreamAuthRoundtrip(t *testing.T) {
	...
	primitives := []string{"siphash24", "blake3", "blake2b512"}
...
func TestEasyDecryptStreamAuthTamperRejection(t *testing.T) {
	...
	id, st := NewEasy("blake3", 1024, "hmac-blake3", 3)
	if st != StatusOK {
		t.Fatalf("NewEasy: %v", st)
	}
	defer FreeEasy(id)
