...
- **Quantum adversaries (Grover search on the seed space)** — not empirically testable; the oracle-degradation argument is documented in [SCIENCE.md § 2.11.3 — Grover Oracle Degradation](SCIENCE.md#2113-grover-oracle-degradation). In short: Core ITB and MAC + Silent Drop expose no verification predicate, so Grover has no `f(key) → {0, 1}` to run against the superposition — the `√N` speedup does not apply without an oracle, and joint `noiseSeed + dataSeed` brute force at `keyBits = 1024` costs `~2^2056` classical / `~2^1028` Grover. Under MAC + Reveal the oracle exists but each query requires `O(P)` hash evaluations (full decryption), placing the work factor at `~2^1033` classical / `~2^516` Grover — both beyond foreseeable quantum capability. The deployment-specific evaluation for MAC + Reveal protocols is out of this document's scope for the same reason as the CCA bullet above.
