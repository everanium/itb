...
- **Byte-level Triple** (`SetBitSoup(0)`, default): every third plaintext byte goes to a different snake.
- **Bit-soup Triple** (`SetBitSoup(1)` via `ITB_BITSOUP=1`): every third plaintext bit goes to a different snake; each snake's payload is a bit-permuted garbage byte-stream.
...
1. **Triple Ouroboros already dilutes input-driven bias below the probe's detection threshold.** Under Single Ouroboros the same probe surfaces CRC128 at min ≈ 47 % on 512 KB ASCII (see table rows above). Under Triple Ouroboros at byte-level the min settles at 48.1 – 48.3 % — the 3-way partition reduces the per-shift bias weighting to ≈ 1/3 because only one snake's region aligns with the probe's `const(p)` computation at any shift hypothesis, and the other 2/3 of pixels contribute ~50 % random conflict regardless of shift. The resulting weighted min sits about halfway between the 47 % Single-mode signature and the 50 % PRF-baseline — a natural consequence of the partition-unknown property, achieved without any bit-soup assistance.
2. **CRC128's residual probe signature is mask-schedule-driven, not plaintext-driven.** The probe measures alignment with `ChainHash(p_le || nonce, seed=0)` which is a property of CRC128's own GF(2)-linear algebra over the nonce + pixel-index input space, independent of whatever was encrypted. Bit-soup flattens the per-snake **plaintext input** to uniform bit-permuted garbage but does not alter the mask schedule — so this bias source persists unchanged. Bit-soup's architectural contribution is at the SAT-instance-formulation layer (insufficient crib coverage per snake under Partial KPA + realistic protocol traffic), not at the bias-probe layer.

Net interpretation: the byte-level Triple Ouroboros 3-way partition is the load-bearing defence at the attacker-realistic bias-probe scope, and bit-soup does not measurably add to it for this specific measurement. FNV-1a shows the same pattern for the same reason plus the carry-chain nonlinearity that already held min close to the noise floor (48.69 % byte-level Single per the Phase 2a extension baseline). Neither primitive is attacker-distinguishable via this probe from a Triple-Ouroboros corpus — bit-soup does not change that conclusion empirically.

Reproduction:

```bash
# Byte-level Triple arm (default mode).
bash scripts/redteam/bias_audit_matrix_triple.sh

# Bit-soup Triple arm (SetBitSoup(1) applied via the package TestMain
