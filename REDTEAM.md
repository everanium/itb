...
`SetLockSoup(1)` is an additional opt-in overlay on top of `SetBitSoup(1)` (see [ITB3.md § Lock Soup](ITB3.md#lock-soup-insane-interlocked-mode-opt-in-overlay-on-bit-soup)). The fixed public 24-bit chunk permutation of plain Bit Soup is replaced by a per-chunk balanced 8-of-24 mask triple drawn from a 2^33 mask space (combinatorial unrank of `C(24,8) × C(16,8)` partitions), derived deterministically per chunk from the noiseSeed and nonce via a single PRF call.

Lock Soup strengthens the instance-formulation barrier from the public-encoding layer (Bit Soup) to the keyed-encoding layer. Under any KPA — Partial, Full, or even hypothetical adversaries with full plaintext-ciphertext pairs — every crib chunk multiplies attacker enumeration by ~2^33 without contributing constraints that couple chunks across, because the per-chunk mask is independently keyed and unobservable without the noiseSeed. SAT cryptanalysis ceases to be a computational-hardness problem and becomes an instance-formulation impossibility: there is no fixed bit-position-to-lane mapping for the solver to anchor on. Improvements in solver throughput, including any hypothetical PRF-inversion shortcut, do not affect the under-determination — the instance has no unique solution to be discovered faster.

Performance cost is approximately 2×–7× slower than plain Bit Soup depending on platform; the BMI2 PEXT/PDEP path on x86 (Haswell+, Excavator+/Zen 1+) sits near the lower bound, the pure-Go fallback near the upper. The trade-off is acceptable only where the architectural barrier is the deployment priority — long-term archive encryption, high-value off-line ciphertexts, defense-in-depth layering above already-strong PRF guarantees. Requires `SetBitSoup(1)`; setting `SetLockSoup(1)` while Bit Soup is off is a silent no-op. Default `SetLockSoup(0)` leaves Bit Soup behaviour unchanged. Both flags must agree across the encrypt and decrypt sides of the channel; mismatched modes produce wrong-seed-style garbage with no error oracle (plausible-decryption invariant preserved). Ciphertext wire format is identical across all three modes (Byte Level / Bit Soup / Lock Soup).
