...
| **Nonce reuse** | forced fixed nonce (lab) | not closed — usage precondition | Part 1 masks repeat under a reused nonce; keystream-reuse structure persists |
...
Nonce reuse is **not reachable through the shipped API** — the nonce is drawn from `crypto/rand` per call. Forced via a test-only override, it is the one condition that surfaces signal. Under a reused nonce with the same seeds, Part 1's per-chunk masks repeat, the noise positions and startPixels repeat, and the nonce-deterministic container structure aligns across the colliding pair (Probe 2: ~5.4× the independent-stream floor). This is recorded, not closed: **nonce reuse is a usage precondition the barrier does not architecturally remove.** Even so the correlation stays far below a plaintext-recovery channel — the nonce-independent CSPRNG tail fill and the plaintext-dependent bytes dominate — and a full demask of the colliding pair additionally requires Full KPA and yields only the barrier-permuted snake images, not plaintext order (which stays behind the ≈ 2^70.20-per-chunk mask space). The mandatory internal nonce is API discipline, not a barrier guarantee. Pre-v0.3.0 demasker / NIST-STS PRF-separation detail is in [REDTEAM-v0.2.md § Phase 2d — Nonce-Reuse](archive/REDTEAM-v0.2.md).
