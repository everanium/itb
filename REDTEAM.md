...
Realistic attacker lacks per-snake `startPixel` knowledge — obtaining it requires sender-side or recipient-side machine access, which is outside the standard threat model. Extrapolating the 8 h Phase 2g empirical anchor to Triple Ouroboros without disclosed `startPixel`:

| Mode | Per-snake validation | Wall-clock projection (289-core commodity pool) |
|---|---|---|
| **Triple Byte Level** (`SetBitSoup(0)`, shipped default) | Printable-ASCII / partial-schema on every-3rd-byte subsequence — independent per-snake validation works | **~8 h — 2 days** (depends on whether the corpus is packed-dense-crib 4 KB → ~96 sp per snake, or expanded-sparse-crib 12 KB → ~537 sp per snake); ≈ same order of magnitude as Single Ouroboros |
| **Triple Bit Soup** (`SetBitSoup(1)`) | Per-snake decoded content is bit-permuted garbage — no readable structure → per-snake validation denied → joint SAT across all three snakes forced | **Decades to millennia** (96³ – 537³ `startPixel` triples × 10-100× Single per coupled-seed joint instance, no empirical anchor for coupled-seed FNV-1a SAT under non-linear wrapping); adding primitive-identity enumeration (deployment-level hardening, 12³) + CSPRNG-fill observation poisoning (each hypothesis has ~30 % bogus-constraint rate) pushes into the **10¹⁴ – 10¹⁸ core-hour region** where no publicly known SAT tooling is applicable |

Under the 4 KB dense-crib interpretation: per-snake SAT needs ≥ 13 crib pixels for 2.85× overdet (Phase 2g's ratio). Phase 2g's 4 KB corpus supplies 4 cribs × 3 pixels = 12 crib pixels under Single; byte-level Triple split dilutes this to ~4 per-snake crib pixels (0.88× overdet, **under-determined** → per-snake SAT does not converge on the Phase 2g corpus). A corpus packing ~12 cribs in 4 KB or ~13 cribs in a 12 KB expansion restores per-snake overdet to the convergent region. Both variants project to the same ~8 h – 2 days band on a 289-core pool because the per-snake solve cost stays at the Phase 2g baseline; only the `startPixel` enumeration range varies with the corpus size.

Under Triple Bit Soup, the joint SAT instance is compounded beyond simple enumeration. The circular length-prefix dependency (length unknown until all three snakes jointly solved, interleave alignment unknown until length known) denies partial-verification signal — each hypothesis is either fully correct or rejected with no gradient for guided search. Per-instance joint SAT cost for 3 coupled 256-bit seeds under FNV-1a carry-chain multiplication has no published baseline; the 10-100× Single estimate is analytical with wide error bars. The **minor arithmetic** for Scenario C joint enumeration — 884 736 combinations × 80 h per instance = 70.8 million core-hours — finishes in ~3.3 days on a 884 736-core cluster (one core per combination) and in ~28 years on 289 cores; expanded to realistic 537³ per-snake ranges it becomes ~6 300 years on 289 cores. Empirical verification of either Triple case is out of scope — the required compute for Triple Bit Soup in particular exceeds any budget available for this project.
