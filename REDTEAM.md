...
| **Related-nonce** | 1-bit nonce Δ, same seeds (lab) | diffused; χ² inside df=255 uniform band | nonce perturbs all 8 seed-derivation slots simultaneously — supersets lockSeed Δ |
...
**SAT-based `lockSeed` recovery is unmeasured, predicted bounded.** Unlike the fresh-nonce regime, the nonce-reuse constraint is formulable: `C1 ⊕ C2` cancels the per-pixel additive mask and hands a Full KPA attacker the known `M1 ⊕ M2`, so each per-chunk observation is a genuine constraint on the interlock permutation with a true satisfying model. Recovery is nonetheless dominated by the `rankToMaskTriple48` combinadic unrank keyed by the `lockSeed` chain, not by the FNV multiply chain a structure-aware solver accelerates: a single 48-bit chunk observation admits ≈ 2^57.80 mask preimages, and collapsing that ambiguity requires coupling many chunks through the shared `lockSeed` chain against the SAT-hostile unrank arithmetic. A symbolic-`lockSeed` SAT harness was not run this cycle; the empirical closure rests on the attacker-realistic demasker null and the mask-oracle upper bound, and the SAT avenue is left as an open, predicted-bounded item rather than a measured null.
...
**Empirical re-verification — related-nonce axis** — `redteam_related_nonce_test.go`. The nonce is a shared input to four distinct derivation paths covering the 8 shipped seed roles: `noiseSeed` gates per-pixel `noisePos` through `blockHash(pixIdx || nonce)`; each `dataSeed_i` gates per-pixel `dataRotation` and `channelXOR` through the same `pixIdx || nonce` buffer; each `startSeed_i` gates the per-snake `startPixel` through `ChainHash(0x02 || nonce)`; and `lockSeed` gates the Interlocked Barrier's per-chunk overlay key through `deriveInterLockSeed(nonce)`. A 1-bit nonce Δ therefore perturbs all 8 seed-derived outputs simultaneously — a fundamentally different structural perturbation from a single-seed Δ. The lockSeed derivation slot is one of eight nonce consumers, not a bottleneck; **nonce Δ supersets lockSeed Δ** by additionally perturbing the other seven derivation slots.
