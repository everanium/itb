...
- **Hash identity** known; **seed components / rotation / noisePos / dataRotation never disclosed**.
...
- **Below-spec primitive injected** in [Phase 2a extension (CRC128)](#phase-2a-extension--empirical-mixed-algebra-stress-test-via-crc128-nonce-reuse): `crc128` is a fully GF(2)-linear stress control, rejected by every `NewSeed*` public API. Runs only via `redteam_lab_test.go`.
