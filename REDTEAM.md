...
Under Triple Bit Soup the joint SAT instance compounds further: per-snake decoded bytes are bit-permuted garbage, removing the per-snake content constraints that byte-level Triple retains inside each joint hypothesis. The circular length-prefix dependency (length unknown until all three snakes jointly solved, interleave alignment unknown until length known) denies partial-verification signal — each hypothesis is either fully correct or rejected with no gradient for guided search. Per-instance joint SAT cost for 3 coupled 256-bit seeds under FNV-1a carry-chain multiplication has no published baseline; the 10–100× Single estimate is analytical with wide error bars. 884 736 combinations × 80 h per instance = 70.8 million core-hours finishes in ~3.3 days on a 884 736-core cluster (one core per combination) and in **~28 years on 289 cores**; expanded to 537³ per-snake ranges it becomes ~6 300 years on 289 cores. Adding primitive-identity enumeration (12³ if deployers use mixed-primitive hardening) + CSPRNG-fill observation poisoning (~30 % bogus-constraint rate per joint hypothesis) pushes the realistic region into **10¹⁴ – 10¹⁸ core-hours**, beyond any publicly known SAT tooling. Empirical verification of either Triple case is out of scope — the required compute for Triple Bit Soup in particular exceeds any budget available for this project.
