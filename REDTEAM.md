...
Suppose every precondition lines up — the attacker forced a nonce-reuse event, knows the byte-level plaintext format, holds the two distinct templates, has the sequence-number field offsets, and the demasker converged on the true `startPixel` with 80 %+ coverage. What do they actually walk away with? Less than "the seeds", and less than the phrase "reconstructed dataHash stream" might suggest:

1. **Plaintext of the 2 – 3 colliding messages.** The two-time pad on the XOR of colliding ciphertexts reveals the unknown value regions of both messages to within the attacker's known-mask precision. This is the direct gain — but it is **local to the nonce-reuse event**, not a key compromise, and it is already the baseline assumption of the [`SCIENCE.md` §2.5](SCIENCE.md#25-nonce-reuse-analysis) locality claim.

2. **Under a PRF primitive (BLAKE3 / MD5 / BLAKE2b / etc.): nothing beyond #1.** The reconstructed `dataHash` stream is statistically indistinguishable from uniform random (188/188 on NIST STS). Inverting it to recover `dataSeed` requires breaking the PRF — out of scope by assumption. `startSeed` and `noiseSeed` inversion hit the same wall.

3. **Under FNV-1a (the only invertible primitive in the hash matrix): one SAT instance per seed, not three free seeds.** The reconstructed `dataHash` stream exposes the ChainHash-wrapped FNV-1a output under a controlled pixel-index probe — which gives a SAT solver real leverage. But:
