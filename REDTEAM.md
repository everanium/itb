...
- **[Phase 2f](#phase-2f--direct-crib-kpa-against-gf2-linear-primitives) partial-recovery figures (75 % byte / 60 % full-pixel) are specific to printable-ASCII plaintexts** (HTML, JSON). The entropy-scaling model `1 − (1 − (|A|/128)^7)^8` places binary / compressed plaintexts at the ~12.5 % floor and narrow-alphabet text at ~82 %. Generalising the attack's practical effectiveness across plaintext classes requires per-format measurement, not extrapolation from the HTML/JSON anchor alone.
- **[Phase 2f](#phase-2f--direct-crib-kpa-against-gf2-linear-primitives) requires a public schema crib in the target plaintext.** The attack uses a 37-byte JSON-schema prefix on corpus A and a 28-byte HTML-schema prefix on corpus B — both publicly-known format tokens. Formats without fixed publicly-known prefixes (randomised headers, pre-compressed payloads, encrypted tunnels carrying arbitrary data) do not admit the Crib KPA step of the attack chain.
