...
The barrier fill in the Triple pipeline runs the shipped `aesitb128` primitive at r = 1, on the 13-byte one-block shape, with a secret ChainHash-derived 128-bit key — the exact regime where the standalone table's ≈ 2²⁰ lo-lane recovery and the one-pair inversion succeed **if** the primitive's output were visible. It is not. `fillRanks` writes exactly `(lo, hi)` of one `Hash(0x03 ‖ LE64(groupIdx) ‖ 0⁴, lockLo, lockHi)` call per group at width 128 (measured empirically — one factor per group, one 128-by-30 divmod on the resulting pair through `fillLockMasksTriple48Super`, producing one `(idx0, idx1)` → one mask triple → one 6-byte chunk); at widths 256 / 512 the wider output is sliced into 128-bit pairs, each pair feeding one chunk under the same divmod. The full 128-bit output is therefore consumed **monolithically** per 128-bit slice, not sliced into 48-bit pieces the lo-lane accumulator could ride.

The 128-bit key `(lockLo, lockHi)` consumed by the hot-loop calls is not the lockSeed itself — it is the full-cascade output `ChainHash128(0x04 ‖ interlock nonce)` on the lockSeed (`seed128.go` `deriveInterLockSeed`), computed once per container on both encrypt and decrypt and captured by the fill closures at build time (`interlock48.go` `buildLockBatchPRF48_*`). Two cascade depths therefore compose across the two stages: the setup runs the primitive at the shipped cascade depth (r = 4 / 8 / 16 at `keyBits` 512 / 1024 / 2048, the regime [§3.10.2](#3102-dissolution-mode--cost-versus-cascade-depth) places above dissolution), and every hot-loop call runs the primitive at r = 1 under the derived key. The two input domains are disjoint by the leading tag byte — `0x04` at setup, `0x03` in the hot loop — so no fill input coincides with a setup input, and the setup output never reaches the wire, only its role as the hot-loop key.
