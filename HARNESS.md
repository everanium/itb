...
| One-pair inversion | 1000 trials; 1 query + 4 verification queries | **not on the wire** — surface (A) consumes `lo(h_r)` only; the ≈ 2²⁰ row below is the hypothetical partial lo-lane inverse | **1000 / 1000 full 128-bit seed blocks recovered** (lab grant only) |
| Structured lo-lane recovery ≈ 2²⁰ (one-block shape, 15 chosen Λ-sets over plaintext bytes 0..14) | 3840 chosen texts, 20 trials | **20 / 20** K = fixedKey ⊕ seed recovered (raw-primitive fact about the one-block shape; the engine is out of regime at surface (A) shapes 20 / 36 / 68 — 0 / 0 at every position at r = 1 and r = 4, `keyrecover_r1_2p20.py --shape-probe`, 3 trials each) | not applicable (lo-lane engine) |
...
| NIST STS 2.1.2, 15-test battery (Frequency, BlockFrequency, CumulativeSums, Runs, LongestRun, Rank, FFT, NonOverlappingTemplate, OverlappingTemplate, Universal, ApproximateEntropy, RandomExcursions, RandomExcursionsVariant, Serial, LinearComplexity) | 10⁷ bits × 10 substreams × 3 trials × {13, 20, 36, 68}-byte inner-Barrier input at r ∈ {1, 2, 3, 4, 5, 6, 7, 8, 12, 16} (120 cells) | 119 / 120 cells at 15 / 15; single flag at r = 5 shape = 13 trial = 2 (LongestRun proportion 7 / 10, uniformity p = 0.534 — statistical fluctuation at α = 0.01, expected ~1–2 flags under FWER at 120 × 15) | same |
...
| Shipped 20 B (T = 4), r = 2, 3 Λ-sets — classical 4-round κ-byte engine, `keyrecover_r2_20byte.py` | floor 0 / 1000 (1000 trials × 8 sets, `keyrecover_kbyte_go --model realistic --trials 1000`) | floor (κ peel needs the hi lane) | **recovers 1000 / 1000** (768 chosen texts, ≈ 2¹³·⁶ κ guess-sums) | recovers | Full-state lab grant is the sole load-bearing grant (idx-only sets suffice under it; fails on lo lane under either nonce model). |
...
| Key recovery, full-state lab grant | 1 known text ≈ 2⁰ (one-pair inversion) | one-block lab shape 256 chosen texts ≈ 2²⁵ (`keyrecover_r2.py`); shipped 20 B shape 768 chosen texts ≈ 2¹³·⁶ κ guess-sums (`keyrecover_r2_20byte.py`); shipped 36 B shape 2³² texts on the order-4 diagonal set at 22 s / set (three-hedge cell — full-state grant, chosen nonce, below shipped depth r = 4) | fails at every engine (0 / 1000) | fails at every engine (0 / 1000 at r = 4, 20 B via `keyrecover_r2_20byte.py`; extended to r ∈ {8, 16} at 0 / 1000 via `keyrecover_kbyte_go`) | Lab reference; every recovery sits behind at least one grant the wire does not admit. |
...
python3 keyrecover_r2.py --trials 1000
...
python3 keyrecover_r2.py --trials 1000
python3 keyrecover_r2_20byte.py --trials 1000
python3 keyrecover_r2_20byte.py --model realistic --trials 20 --sets 8

# Standalone lo-lane KEY-RECOVERY at r = 1, one-block lab shape: ≈ 2^20 structured Square-style
# recovery of K = fixedKey ^ seed from 15 chosen Λ-sets over plaintext bytes 0..14 (20 / 20). Splits
...
python3 keyrecover_r1_2p20.py --trials 20
...
# NIST STS 2.1.2 15-test battery, ChainHash<AES-ITB-128> lo-lane output, 10^7 bits × 10 substreams
# × 3 trials × {13, 20, 36, 68}-byte inner-Barrier input × r ∈ {1, 2, 3, 4, 5, 6, 7, 8, 12, 16}
# (120 cells, ~1 hour). Cell outputs land under $HOME/scratch/redteam/aesitb128_nist_sts/;
# `nist_sts_aggregate.py` collates per-cell finalAnalysisReport.txt into summary.md + summary.json.
# Requires /usr/bin/nist-sts (Arch package nist-sts) plus templates cached under
# $HOME/.local/share/nist-sts/templates/ on first run.
bash nist_sts/nist_sts_sweep.sh
python3 nist_sts/nist_sts_aggregate.py --root ~/scratch/redteam/aesitb128_nist_sts \
    --out-md ~/scratch/redteam/aesitb128_nist_sts/summary.md \
    --out-json ~/scratch/redteam/aesitb128_nist_sts/summary.json
