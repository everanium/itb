...
// shipped names only.
//
// Names includes inner-PRF-only entries (Class = ClassNPRF) — the return
...
// ClassPRFCounter), in canonical order. Inner-PRF-only entries (ClassNPRF)
...
// ClassNPRF when name is not a shipped primitive.
