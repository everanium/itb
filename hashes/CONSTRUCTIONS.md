...
- **Interlocked Barrier fill.** Defence = the ChainHash cascade with feed-forward at fill depth `r = 1 + keyBits / width` **plus** the **divmod-then-combinadic-unrank** absorption of the full `(lo, hi)` output into a 16-of-48 mask triple. Both stages are load-bearing; neither alone suffices, and the primitive's `(lo, hi)` never surfaces to the wire — it exits only as the mask triple.
