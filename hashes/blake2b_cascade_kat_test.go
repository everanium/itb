...
// hook at the 13-byte shape across a group index base that carries
// through byte 7 inside the batch. A seed with every hook and the same
// seed on the arms alone are both checked, so the table pins the wire as
// well as every hook against the noitbasm reference.
