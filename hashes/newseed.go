...
// newseed.go — name-keyed seed constructors. Each builds a seed of the
// named registry primitive with every performance hook the primitive
// offers, in the sequence the triple package's seed builders and the C
// ABI seed constructors run: the (single, batched) arms from the
// width's Make<W>Pair, keyBits random components from the itb
// constructor, then the fused ChainHash cascade hooks and the batch-16
// Interlocked Barrier fill hook from the width's attach helpers. The
...
// restore through [itb.SeedFromComponents128] with the arms of
// [Make128Pair] and the hooks of [AttachFused128] /
// [AttachInterlockBatch16].
//
