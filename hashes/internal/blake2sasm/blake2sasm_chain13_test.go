...
// TestBlake2s256ChainAbsorb13x4 verifies the Interlocked Barrier PRF
// fill kernel matches the BLAKE2s-256 reference at the 13-byte shape.
func TestBlake2s256ChainAbsorb13x4(t *testing.T) {
	runChainAbsorb256Test(t, "Blake2s256ChainAbsorb13x4", 13, Blake2s256ChainAbsorb13x4)
}
...
// TestBlake2s256ChainAbsorb13x4_BitSensitivity flips each of the 104
// bits of the 13-byte input (per lane) and requires that lane's
// digest to change.
func TestBlake2s256ChainAbsorb13x4_BitSensitivity(t *testing.T) {
	if !HasAVX512Fused {
	...
	run := func(d [][]byte) [4][8]uint32 {
		...
		var out [4][8]uint32
		Blake2s256ChainAbsorb13x4(&Blake2sIV256Param, &b2key, &seeds, &dp, &out)
		return out
	}
	baseOut := run(base)
	for lane := 0; lane < 4; lane++ {
...
func benchB2s13(b *testing.B, fused bool) {
	if !HasAVX512Fused {
	...
	if fused {
		for i := 0; i < b.N; i++ {
			Blake2s256ChainAbsorb13x4(&Blake2sIV256Param, &b2key, &seeds, &ptrs, &out)
		}
	} else {
		for i := 0; i < b.N; i++ {
			for lane := 0; lane < 4; lane++ {
...
func BenchmarkBlake2s256ChainAbsorb13x4_Reference(b *testing.B) { benchB2s13(b, false) }
func BenchmarkBlake2s256ChainAbsorb13x4_Fused(b *testing.B)     { benchB2s13(b, true) }
