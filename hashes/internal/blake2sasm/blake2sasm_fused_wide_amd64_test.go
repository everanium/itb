...
var kernels256x8 = map[int]x8fn256{20: wrap8_256(blake2sFusedChain20x8Avx512Asm), 36: wrap8_256(blake2sFusedChain36x8Avx512Asm), 68: wrap8_256(blake2sFusedChain68x8Avx512Asm)}

func fill256x8Kernel(k *[32]byte, c []uint64, base uint64, o *[8][4]uint64) {
	blake2sFusedChain13x8Avx512Asm(k, &c[0], len(c)/4, base, o)
}
		...
		b.Run(fmt.Sprintf("fill256/2x4/groups%d", g), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				for h := 0; h < 2; h++ {
					...
					blake2sFusedChain13x4Avx512Asm(key256, &c256[0], g, &ptrs, out8x256Half(&o8, h))
				}
			}
		})
	}
