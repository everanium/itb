...
// AVX2 counterpart of blake2b_chain256_68_amd64.s. Single compression
// block per lane (buf is 100 bytes ≤ 128), t = 100 = key_len(32) +
// data_len(68). Seed injection covers buf[32:64] = m[4..7]; data words
// m[8..12] carry no seed XOR. Layout and register schedule identical to
// the 20-byte AVX2 kernel.
...
// 8-byte data word, no seed XOR (m[8..11] of the 68-byte shape).
#define BUILD_DATA_Q(data_off, slot) \
	MOVQ data_off(R8),  R12; MOVQ R12, slot+0(SP); \
	MOVQ data_off(R9),  R13; MOVQ R13, slot+8(SP); \
	MOVQ data_off(R10), R14; MOVQ R14, slot+16(SP); \
	MOVQ data_off(R11), DI;  MOVQ DI,  slot+24(SP)

// 4-byte data tail zero-extended, no seed XOR (m[12] of the 68-byte shape).
...
TEXT ·blake2b256ChainAbsorb68x4Avx2Asm(SB), NOSPLIT, $608-40
	MOVQ h0+0(FP),        AX
	MOVQ b2key+8(FP),     BX
	MOVQ seeds+16(FP),    CX
	...
	BUILD_DATA_Q(32, M(8))
	BUILD_DATA_Q(40, M(9))
	BUILD_DATA_Q(48, M(10))
	BUILD_DATA_Q(56, M(11))
	BUILD_DEXT4_Q(64, M(12))
	BUILD_BCST_Q(0,  M(0))
	BUILD_BCST_Q(8,  M(1))
	BUILD_BCST_Q(16, M(2))
	BUILD_BCST_Q(24, M(3))
