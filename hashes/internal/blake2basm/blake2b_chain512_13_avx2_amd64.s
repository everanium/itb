...
// AVX2 YMM-batched fused chain-absorb kernel for BLAKE2b-512 with
// 13-byte per-lane data input (the Interlocked Barrier PRF fill shape).
// AVX2 counterpart of blake2b_chain512_13_amd64.s. The 512-bit kernels
// carry a 64-byte key prefix (m[0..7]) and 8 seed components (per-lane
// stride 64 bytes), and emit the full 64-byte digest (8 words per
// lane). One compression with t=128, f=^0 (final). Layout, rotate
// synthesis, and the v[12]/v[15] register-spill schedule are identical
// to the -256 AVX2 kernels.
//
// Per-lane absorb construction (matches hashes.BLAKE2b512 bit-exactly):
//
//	buf[0:64]   = b2key ; buf[64:84] = data[lane] ; buf[84:128] = zero
//	then buf[64+i*8 : 72+i*8] ^= seeds[lane][i] for i in 0..7
//
// Register allocation: see blake2b_chain256_20_avx2_amd64.s (the seed
// stride is 64 not 32, and the fold/store cover all 8 words).
...
// Seed stride 64 (seeds *[4][8]uint64).
...
// 5-byte data tail zero-extended to a qword, XOR seed (m[9] of 13-byte).
#define LOAD5(base, off, dst) \
	MOVL off(base), dst; \
	MOVBLZX off+4(base), R12; \
	SHLQ $32, R12; \
	ORQ R12, dst

#define BUILD_D5XS_Q(data_off, seed_idx, slot) \
	LOAD5(R8,  data_off, R13); XORQ seed_idx*8+0*64(CX), R13; MOVQ R13, slot+0(SP); \
	LOAD5(R9,  data_off, R13); XORQ seed_idx*8+1*64(CX), R13; MOVQ R13, slot+8(SP); \
	LOAD5(R10, data_off, R13); XORQ seed_idx*8+2*64(CX), R13; MOVQ R13, slot+16(SP); \
	LOAD5(R11, data_off, R13); XORQ seed_idx*8+3*64(CX), R13; MOVQ R13, slot+24(SP)

#define BUILD_DEXT4XS_Q(data_off, seed_idx, slot) \
	MOVL data_off(R8),  R12; XORQ seed_idx*8+0*64(CX), R12; MOVQ R12, slot+0(SP); \
	MOVL data_off(R9),  R13; XORQ seed_idx*8+1*64(CX), R13; MOVQ R13, slot+8(SP); \
	MOVL data_off(R10), R14; XORQ seed_idx*8+2*64(CX), R14; MOVQ R14, slot+16(SP); \
	MOVL data_off(R11), DI;  XORQ seed_idx*8+3*64(CX), DI;  MOVQ DI,  slot+24(SP)

#define BUILD_SEED_Q(seed_idx, slot) \
	MOVQ seed_idx*8+0*64(CX), R12; MOVQ R12, slot+0(SP); \
	MOVQ seed_idx*8+1*64(CX), R13; MOVQ R13, slot+8(SP); \
	MOVQ seed_idx*8+2*64(CX), R14; MOVQ R14, slot+16(SP); \
	MOVQ seed_idx*8+3*64(CX), DI;  MOVQ DI,  slot+24(SP)
...
TEXT ·blake2b512ChainAbsorb13x4Avx2Asm(SB), NOSPLIT, $608-40
	MOVQ h0+0(FP),        AX
	MOVQ b2key+8(FP),     BX
	MOVQ seeds+16(FP),    CX
	...
	// out[k] = h0[k]^v[k]^v[k+8], k in 0..7. v15 reloaded from slot.
