...
// init applies ITB_FORCE_HASH_TIER to the BLAKE2b fused chain-absorb
...
// with a stderr note. BLAKE2b has no AES-based arm, so an aesni force
...
func init() {
	...
	case "avx2":
		if !cpu.X86.HasAVX2 {
			forcetier.Warnf("blake2basm: avx2 tier needs AVX2; keeping auto-dispatch")
			return
		}
