...
func applyHashTier() {
	...
	case "neon", "sve2", "sve":
		if !cpu.ARM64.HasASIMD {
			forcetier.Warnf("blake2basm: %s tier needs Advanced SIMD; keeping auto-dispatch", forcetier.HashTier())
			return
		}
	...
	default:
		forcetier.Warnf("blake2basm: %s tier is amd64-only; keeping auto-dispatch", forcetier.HashTier())
	}
}

// applyInterlockPRFFillTier applies ITB_FORCE_INTERLOCK_PRF_FILL_TIER to
// the batch-16 fill flag (HasNEONX16), accepting "neon" and "scalar";
...
func applyInterlockPRFFillTier() {
	...
	case "neon":
		if !cpu.ARM64.HasASIMD {
			forcetier.Warnf("blake2basm: neon batch-16 tier needs Advanced SIMD; keeping auto-dispatch")
			return
		}
		HasNEONX16 = true
	case "gpr":
	...
	case "scalar":
		...
		HasGPRX16 = false
	default:
		forcetier.Warnf("blake2basm: %s batch-16 tier is amd64-only; keeping auto-dispatch", forcetier.InterlockPRFFillTier())
	}
}
