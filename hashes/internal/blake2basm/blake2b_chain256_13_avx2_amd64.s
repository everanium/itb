...
// Lift 2). AVX2 counterpart of blake2b_chain256_13_amd64.s. Layout,
// rotate synthesis, and register schedule are identical to the 20-byte
// AVX2 kernel; only the message build differs — max(data_len,32)=32 so
// t=64 exactly as the 20-byte case, one final-block compression.
//
// Per-lane absorb construction (matches hashes.BLAKE2b256 bit-exactly):
//
//	buf[0:32]  = b2key ; buf[32:45] = data[lane] ; buf[45:64] = zero
//	then buf[32+i*8 : 40+i*8] ^= seeds[lane][i] for i in 0..3
//
...
// BUILD_D5XS_Q — 5-byte data tail (4-byte + 1-byte) zero-extended to a
// qword, XOR seed. Used for m[5] of the 13-byte shape (data[8:13]).
...
#define BUILD_D5XS_Q(data_off, seed_idx, slot) \
	LOAD5(R8,  data_off, R13); XORQ seed_idx*8+0*32(CX), R13; MOVQ R13, slot+0(SP); \
	LOAD5(R9,  data_off, R13); XORQ seed_idx*8+1*32(CX), R13; MOVQ R13, slot+8(SP); \
	LOAD5(R10, data_off, R13); XORQ seed_idx*8+2*32(CX), R13; MOVQ R13, slot+16(SP); \
	LOAD5(R11, data_off, R13); XORQ seed_idx*8+3*32(CX), R13; MOVQ R13, slot+24(SP)
...
TEXT ·blake2b256ChainAbsorb13x4Avx2Asm(SB), NOSPLIT, $608-40
	MOVQ h0+0(FP),        AX
	MOVQ b2key+8(FP),     BX
	MOVQ seeds+16(FP),    CX
