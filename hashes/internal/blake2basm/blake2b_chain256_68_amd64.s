...
// ZMM-batched fused chain-absorb kernel for BLAKE2b-256 with 68-byte
// per-lane data input (the ITB SetNonceBits(512) buf shape).
//
// Unlike the BLAKE2b-512 68-byte kernel (which spans two compressions
// because the key+data buffer is 132 bytes > 128), the BLAKE2b-256
// 68-byte buf is only 100 bytes (32 key + 68 data) — fits in a single
// 128-byte BLAKE2b block, so this kernel runs only one compression.
//
// Per-lane buffer construction (matches the public hashes.BLAKE2b256
// closure bit-exactly):
//
//	buf[0:32]   = b2key                (shared across all 4 lanes)
//	buf[32:100] = data[lane]           (per-lane, 68 bytes)
//	then for i in 0..3:
//	  buf[32+i*8 : 40+i*8] ^= seeds[lane][i]   (LE)
//
// One compression with t=100 (= 32 key + 68 data), f=^0 (final block).
//
//	blake2b256ChainAbsorb68x4Asm(
...
// EMIT_M_FROM_DATA — 8-byte data load with no seed XOR. Used for m
// positions whose 8-byte slot lies entirely past the seed-injection
// region [32:64] of buf.
#define EMIT_M_FROM_DATA(data_off, y_dst) \
	MOVQ data_off(R8),  R12; \
	MOVQ data_off(R9),  R13; \
	MOVQ data_off(R10), R14; \
	MOVQ data_off(R11), DI; \
...
// EMIT_M_FROM_DATAEXT4_NOSEED — 4-byte zero-extended data load (no seed XOR).
...
TEXT ·blake2b256ChainAbsorb68x4Asm(SB), NOSPLIT, $0-40
	MOVQ h0+0(FP),       AX
	MOVQ b2key+8(FP),    BX
	MOVQ seeds+16(FP),   CX
	...
	// m[4..12] first (uses X16/X17 scratch — m[0..3] come last).
	//
	// m[4..7] = data[lane][0..32] ⊕ seeds[lane][0..3] (seed-injection
	//          region of buf [32:64]).
	EMIT_M_FROM_DATAXSEED(0,  0, Y20)
	EMIT_M_FROM_DATAXSEED(8,  1, Y21)
	EMIT_M_FROM_DATAXSEED(16, 2, Y22)
	EMIT_M_FROM_DATAXSEED(24, 3, Y23)
	// m[8..11] = data[lane][32..64]. Past seed-injection region — no XOR.
	EMIT_M_FROM_DATA(32, Y24)
	EMIT_M_FROM_DATA(40, Y25)
	EMIT_M_FROM_DATA(48, Y26)
	EMIT_M_FROM_DATA(56, Y27)
	// m[12] = (data[lane][64:68] || zero[0:4]).
	EMIT_M_FROM_DATAEXT4_NOSEED(64, Y28)
	...
	// m[13..15] = 0 (zero pad region of the 128-byte block after byte 100).
	VPXORQ Z29, Z29, Z29
	VPXORQ Z30, Z30, Z30
	VPXORQ Z31, Z31, Z31
