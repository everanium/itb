...
// aescmacasm_entropy_test.go — the input-entropy differential audit
...
// cascade (ScalarFusedChain / scalarFusedX16) at the baseline and after
// every flip. The helpers drive the single-, four-, eight- and
...
func auditSchedule(in *kernelaudit.Inputs) *Schedule {
	var key [16]byte
	copy(key[:], in.Key)
...
func refLanes(in *kernelaudit.Inputs) [][2]uint64 {
	...
	for l := range in.Lanes {
		out[l][0], out[l][1] = ScalarFusedChain(s, in.Comps, in.Lanes[l])
	}
	return out
}
...
func refFill(in *kernelaudit.Inputs) [][2]uint64 {
	var out [16][2]uint64
	scalarFusedX16(auditSchedule(in), in.Comps, in.Base, &out)
	return out[:]
}
...
func auditX4(t *testing.T, label string, n int, f fusedX4Fn) {
	...
	for _, pairs := range auditPairCounts {
		cfg := kernelaudit.Config{Label: fmt.Sprintf("%s x4 %s pairs=%d", label, shapeName(n), pairs), KeyBytes: 16, Words: 2 * pairs, Lanes: 4, N: n}
		reportAudit(t, kernelaudit.DifferentialRef(cfg, func(in *kernelaudit.Inputs) [][2]uint64 {
			...
			var out [4][2]uint64
			f(auditSchedule(in), in.Comps, &ptrs, &out)
			return out[:]
		}, refLanes))
	}
}
...
func auditX1(t *testing.T, label string, n int, f fusedX1Fn) {
	t.Helper()
	for _, pairs := range auditPairCounts {
		cfg := kernelaudit.Config{Label: fmt.Sprintf("%s x1 %s pairs=%d", label, shapeName(n), pairs), KeyBytes: 16, Words: 2 * pairs, Lanes: 1, N: n}
		reportAudit(t, kernelaudit.DifferentialRef(cfg, func(in *kernelaudit.Inputs) [][2]uint64 {
			var out [2]uint64
			f(auditSchedule(in), in.Comps, &in.Lanes[0][0], &out)
			return [][2]uint64{out}
		}, refLanes))
	}
}
...
func auditX8(t *testing.T, label string, n int, f fusedX8Fn) {
	t.Helper()
	for _, pairs := range auditPairCounts {
		cfg := kernelaudit.Config{Label: fmt.Sprintf("%s x8 %s pairs=%d", label, shapeName(n), pairs), KeyBytes: 16, Words: 2 * pairs, Lanes: 8, N: n}
		reportAudit(t, kernelaudit.DifferentialRef(cfg, func(in *kernelaudit.Inputs) [][2]uint64 {
			var ptrs [8]*byte
			for l := range ptrs {
				ptrs[l] = &in.Lanes[l][0]
...
func auditX16(t *testing.T, label string, f fusedX16Fn) {
	t.Helper()
	for _, pairs := range auditPairCounts {
		cfg := kernelaudit.Config{Label: fmt.Sprintf("%s x16 pairs=%d", label, pairs), KeyBytes: 16, Words: 2 * pairs, Lanes: 16, Base: true}
		reportAudit(t, kernelaudit.DifferentialRef(cfg, func(in *kernelaudit.Inputs) [][2]uint64 {
			var out [16][2]uint64
			f(auditSchedule(in), in.Comps, in.Base, &out)
			return out[:]
		}, refFill))
	}
}
