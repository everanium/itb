package aescmacasm

// ExpandKeyAES128 produces the 176-byte AES-128 forward round-key
// schedule (11 × 16-byte round keys K0..K10) from a caller-supplied
// 16-byte master key, following FIPS 197 §5.2 verbatim.
//
// Used by the AES-CMAC Pair factory: each Pair invocation expands
// the fixed key once and passes a *[176]byte to the ZMM-batched
// chain-absorb kernels. The kernels broadcast each round key to all
// 4 lanes via VBROADCASTI32X4 at function entry; the actual VAESENC
// per-pixel work consumes the broadcast keys without re-touching
// the schedule buffer.
//
// The expansion is bit-identical to the round keys produced
// internally by crypto/aes.NewCipher; the parity test in
// aescmacasm_chain_test.go cross-checks the kernel output against a
// scalar reference that calls cipher.Block.Encrypt, indirectly
// validating the schedule.
func ExpandKeyAES128(key [16]byte) [176]byte {
	var w [44]uint32
	for i := 0; i < 4; i++ {
		w[i] = uint32(key[i*4]) |
			uint32(key[i*4+1])<<8 |
			uint32(key[i*4+2])<<16 |
			uint32(key[i*4+3])<<24
	}
	for i := 4; i < 44; i++ {
		temp := w[i-1]
		if i%4 == 0 {
			temp = subWord(rotWord(temp)) ^ rcon[i/4]
		}
		w[i] = w[i-4] ^ temp
	}
	var out [176]byte
	for i := 0; i < 44; i++ {
		out[i*4+0] = byte(w[i])
		out[i*4+1] = byte(w[i] >> 8)
		out[i*4+2] = byte(w[i] >> 16)
		out[i*4+3] = byte(w[i] >> 24)
	}
	return out
}

// rcon holds the AES round constants placed in the low byte of each
// uint32 (the high three bytes are always zero per FIPS 197 §5.2).
// Index 0 is unused; indices 1..10 cover round keys K1..K10.
var rcon = [11]uint32{
	0x00,
	0x01, 0x02, 0x04, 0x08,
	0x10, 0x20, 0x40, 0x80,
	0x1b, 0x36,
}

// rotWord rotates a 4-byte word left by 1 byte (= 8 bits in the
// little-endian encoding used by the schedule).
func rotWord(w uint32) uint32 {
	return (w >> 8) | (w << 24)
}

// subWord applies the AES forward S-box byte-wise to a 4-byte word.
func subWord(w uint32) uint32 {
	return uint32(sbox[byte(w)]) |
		uint32(sbox[byte(w>>8)])<<8 |
		uint32(sbox[byte(w>>16)])<<16 |
		uint32(sbox[byte(w>>24)])<<24
}

// sbox is the AES forward S-box (FIPS 197 §5.1.1, Table 4). Used
// only by the key-schedule expansion above; the per-pixel VAESENC
// path in the kernels uses the hardware S-box embedded in the AES
// round instructions.
var sbox = [256]byte{
	,
}
