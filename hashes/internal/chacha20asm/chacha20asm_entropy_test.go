...
// and the scalar fill reference) at the baseline and after every flip.
// The helpers drive the single-, four- and eight-lane evaluators and
// the fill hook through the audit; the per-architecture files apply
// them to every kernel by direct call, and this file applies them to
// the public dispatchers under the current dispatch state.
...
// ref256 evaluates the pure-Go cascade on every lane.
...
// refFill256x8 evaluates the pure-Go fill cascade.
...
// auditDispatchers runs the audit over every public dispatcher and the
// fill hook under the current dispatch state.
