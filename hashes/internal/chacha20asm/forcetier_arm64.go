...
func applyFusedHashTier() {
	...
	case "neon", "sve2", "sve":
		if !cpu.ARM64.HasASIMD {
			forcetier.Warnf("chacha20asm: %s tier needs Advanced SIMD; keeping auto-dispatch", forcetier.HashTier())
			return
		}
	...
	default:
		forcetier.Warnf("chacha20asm: %s tier is amd64-only; keeping auto-dispatch", forcetier.HashTier())
	}
}
...
func applyFusedInterlockPRFFillTier() {
	...
	case "neon":
		if !cpu.ARM64.HasASIMD {
			forcetier.Warnf("chacha20asm: neon batch-16 tier needs Advanced SIMD; keeping auto-dispatch")
			return
		}
		HasNEONX16 = true
	case "scalar":
		HasNEONX16 = false
	default:
		forcetier.Warnf("chacha20asm: %s batch-16 tier is amd64-only; keeping auto-dispatch", forcetier.InterlockPRFFillTier())
	}
}
