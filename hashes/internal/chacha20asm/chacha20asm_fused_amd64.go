...
// Fused-cascade tier flags of the ChaCha20 kernels. At most one flag of
...
func Fused256Chain13x4(fixedKey *[32]byte, components []uint64, dataPtrs *[4]*byte, out *[4][4]uint64) {
	...
	switch {
	case FusedHasAVX512:
		chacha20FusedChain13x4Avx512Asm(fixedKey, c, ng, dataPtrs, out)
	case FusedHasAVX2:
		chacha20FusedChain13x4Avx2Asm(fixedKey, c, ng, dataPtrs, out)
	default:
		if !FusedHasGPR {
			scalarFused256X4(fixedKey, components, dataPtrs, 13, out)
			return
		...
		for l := range dataPtrs {
			chacha20FusedChain13x1GprAsm(fixedKey, c, ng, dataPtrs[l], &out[l])
		}
	}
}
...
func Fused256Chain20x4(fixedKey *[32]byte, components []uint64, dataPtrs *[4]*byte, out *[4][4]uint64) {
	...
	switch {
	case FusedHasAVX512:
		chacha20FusedChain20x4Avx512Asm(fixedKey, c, ng, dataPtrs, out)
	case FusedHasAVX2:
		chacha20FusedChain20x4Avx2Asm(fixedKey, c, ng, dataPtrs, out)
	default:
		if !FusedHasGPR {
			scalarFused256X4(fixedKey, components, dataPtrs, 20, out)
			return
		...
		for l := range dataPtrs {
			chacha20FusedChain20x1GprAsm(fixedKey, c, ng, dataPtrs[l], &out[l])
		}
	}
}
...
func Fused256Chain36x4(fixedKey *[32]byte, components []uint64, dataPtrs *[4]*byte, out *[4][4]uint64) {
	...
	switch {
	case FusedHasAVX512:
		chacha20FusedChain36x4Avx512Asm(fixedKey, c, ng, dataPtrs, out)
	case FusedHasAVX2:
		chacha20FusedChain36x4Avx2Asm(fixedKey, c, ng, dataPtrs, out)
	default:
		if !FusedHasGPR {
			scalarFused256X4(fixedKey, components, dataPtrs, 36, out)
			return
		...
		for l := range dataPtrs {
			chacha20FusedChain36x1GprAsm(fixedKey, c, ng, dataPtrs[l], &out[l])
		}
	}
}
...
func Fused256Chain68x4(fixedKey *[32]byte, components []uint64, dataPtrs *[4]*byte, out *[4][4]uint64) {
	...
	switch {
	case FusedHasAVX512:
		chacha20FusedChain68x4Avx512Asm(fixedKey, c, ng, dataPtrs, out)
	case FusedHasAVX2:
		chacha20FusedChain68x4Avx2Asm(fixedKey, c, ng, dataPtrs, out)
	default:
		if !FusedHasGPR {
			scalarFused256X4(fixedKey, components, dataPtrs, 68, out)
			return
		...
		for l := range dataPtrs {
			chacha20FusedChain68x1GprAsm(fixedKey, c, ng, dataPtrs[l], &out[l])
		}
	}
}
...
func Fused256Fill13x8(fixedKey *[32]byte, components []uint64, groupIdxBase uint64, out *[8][4]uint64) {
	...
	switch {
	case HasAVX512X16:
		chacha20FusedChain13x8Avx512Asm(fixedKey, c, ng, groupIdxBase, out)
	case HasAVX2X16:
		var blocks [4][13]byte
		for h := 0; h < 2; h++ {
			ptrs := fillPtrs4(&blocks, groupIdxBase+uint64(4*h))
			chacha20FusedChain13x4Avx2Asm(fixedKey, c, ng, &ptrs, out8x256Half(out, h))
		}
	default:
		if !HasGPRX16 {
			scalarFill256X8(fixedKey, components, groupIdxBase, out)
		...
		for i := range out {
			...
			chacha20FusedChain13x1GprAsm(fixedKey, c, ng, &block[0], &out[i])
		}
	}
}
...
func Fused256Chain20x8(fixedKey *[32]byte, components []uint64, dataPtrs *[8]*byte, out *[8][4]uint64) {
	if FusedX8Active() && validComponents256(components) {
		chacha20FusedChain20x8Avx512Asm(fixedKey, &components[0], len(components)/4, dataPtrs, out)
		return
	}
...
func Fused256Chain36x8(fixedKey *[32]byte, components []uint64, dataPtrs *[8]*byte, out *[8][4]uint64) {
	if FusedX8Active() && validComponents256(components) {
		chacha20FusedChain36x8Avx512Asm(fixedKey, &components[0], len(components)/4, dataPtrs, out)
		return
	}
...
func Fused256Chain68x8(fixedKey *[32]byte, components []uint64, dataPtrs *[8]*byte, out *[8][4]uint64) {
	if FusedX8Active() && validComponents256(components) {
		chacha20FusedChain68x8Avx512Asm(fixedKey, &components[0], len(components)/4, dataPtrs, out)
		return
	}
...
// Kernels (chacha20_fusedchain256_<shape>x4_<tier>_amd64.s, the eight-lane
// per-pixel and fill kernels chacha20_fusedchain256_<shape>x8_avx512_amd64.s
// and the single-lane kernels chacha20_fusedchain256_<shape>x1_gpr_amd64.s).
//
//go:noescape
...
//
//go:noescape
func chacha20FusedChain13x4Avx2Asm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[4]*byte, out *[4][4]uint64)

//
//go:noescape
func chacha20FusedChain20x4Avx512Asm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[4]*byte, out *[4][4]uint64)
...
//
//go:noescape
func chacha20FusedChain36x4Avx512Asm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[4]*byte, out *[4][4]uint64)

//
//go:noescape
func chacha20FusedChain36x4Avx2Asm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[4]*byte, out *[4][4]uint64)

//
//go:noescape
func chacha20FusedChain68x4Avx512Asm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[4]*byte, out *[4][4]uint64)

//
//go:noescape
func chacha20FusedChain68x4Avx2Asm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[4]*byte, out *[4][4]uint64)
...
//
//go:noescape
func chacha20FusedChain20x8Avx512Asm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[8]*byte, out *[8][4]uint64)

//
//go:noescape
func chacha20FusedChain36x8Avx512Asm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[8]*byte, out *[8][4]uint64)
...
//
//go:noescape
func chacha20FusedChain36x1GprAsm(fixedKey *[32]byte, comps *uint64, nGroups int, data *byte, out *[4]uint64)

//
//go:noescape
func chacha20FusedChain68x1GprAsm(fixedKey *[32]byte, comps *uint64, nGroups int, data *byte, out *[4]uint64)
