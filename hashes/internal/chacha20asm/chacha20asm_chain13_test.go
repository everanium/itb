...
// TestChaCha20256ChainAbsorb13x4 verifies the Interlocked Barrier PRF
// fill kernel matches the ChaCha20-256 reference at the 13-byte shape.
func TestChaCha20256ChainAbsorb13x4(t *testing.T) {
	runChainAbsorb256Test(t, "ChaCha20256ChainAbsorb13x4", 13, ChaCha20256ChainAbsorb13x4)
}
...
// TestChaCha20256ChainAbsorb13x4_BitSensitivity flips each of the 104
// bits of the 13-byte input (per lane) and requires that lane's
// digest to change.
func TestChaCha20256ChainAbsorb13x4_BitSensitivity(t *testing.T) {
	if !HasAVX512Fused {
	...
	run := func(d [][]byte) [4][4]uint64 {
		...
		var out [4][4]uint64
		ChaCha20256ChainAbsorb13x4(&key, &seeds, &dp, &out)
		return out
	}
	baseOut := run(base)
	for lane := 0; lane < 4; lane++ {
...
func benchChacha13(b *testing.B, fused bool) {
	if !HasAVX512Fused {
	...
	if fused {
		for i := 0; i < b.N; i++ {
			ChaCha20256ChainAbsorb13x4(&key, &seeds, &ptrs, &out)
		}
	} else {
		for i := 0; i < b.N; i++ {
			for lane := 0; lane < 4; lane++ {
...
func BenchmarkChaCha20256ChainAbsorb13x4_Reference(b *testing.B) { benchChacha13(b, false) }
func BenchmarkChaCha20256ChainAbsorb13x4_Fused(b *testing.B)     { benchChacha13(b, true) }
