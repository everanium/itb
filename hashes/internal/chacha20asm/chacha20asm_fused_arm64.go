...
func Fused256Chain13x4(fixedKey *[32]byte, components []uint64, dataPtrs *[4]*byte, out *[4][4]uint64) {
	...
	switch {
	case FusedHasNEON:
		chacha20FusedChain13x4NeonAsm(fixedKey, c, ng, dataPtrs, out)
	case FusedHasGPR:
...
func Fused256Chain20x4(fixedKey *[32]byte, components []uint64, dataPtrs *[4]*byte, out *[4][4]uint64) {
	...
	switch {
	case FusedHasNEON:
		chacha20FusedChain20x4NeonAsm(fixedKey, c, ng, dataPtrs, out)
	case FusedHasGPR:
		for l := range dataPtrs {
...
func Fused256Chain36x4(fixedKey *[32]byte, components []uint64, dataPtrs *[4]*byte, out *[4][4]uint64) {
	...
	switch {
	case FusedHasNEON:
		chacha20FusedChain36x4NeonAsm(fixedKey, c, ng, dataPtrs, out)
	case FusedHasGPR:
		for l := range dataPtrs {
...
func Fused256Chain68x4(fixedKey *[32]byte, components []uint64, dataPtrs *[4]*byte, out *[4][4]uint64) {
	...
	switch {
	case FusedHasNEON:
		chacha20FusedChain68x4NeonAsm(fixedKey, c, ng, dataPtrs, out)
	case FusedHasGPR:
		for l := range dataPtrs {
...
func Fused256Fill13x8(fixedKey *[32]byte, components []uint64, groupIdxBase uint64, out *[8][4]uint64) {
	...
	for h := 0; h < 2; h++ {
		...
		chacha20FusedChain13x4NeonAsm(fixedKey, c, ng, &ptrs, out8x256Half(out, h))
	}
}
...
// Kernels (chacha20_fusedchain256_<shape>x4_neon_arm64.s and the
// single-lane kernels chacha20_fusedchain256_<shape>x1_gpr_arm64.s).
//
//go:noescape
...
//
//go:noescape
func chacha20FusedChain20x4NeonAsm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[4]*byte, out *[4][4]uint64)

//
//go:noescape
func chacha20FusedChain36x4NeonAsm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[4]*byte, out *[4][4]uint64)

//
//go:noescape
func chacha20FusedChain68x4NeonAsm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[4]*byte, out *[4][4]uint64)
