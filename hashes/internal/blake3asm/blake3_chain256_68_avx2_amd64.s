...
// AVX2 XMM-batched fused two-block chain-absorb kernel for BLAKE3-256
// with 68-byte per-lane data input (the ITB SetNonceBits(512) buf
// shape). AVX2 counterpart of blake3_chain256_68_amd64.s. Two 64-byte
// BLAKE3 blocks per lane:
//
//	Block 1 (block_len=64, flags=0x11 = KEYED_HASH | CHUNK_START):
//	    m[0..7]  = data[0:32] ⊕ seed ; m[8..15] = data[32:64]
//	    cv1[k]   = v[k] ⊕ v[k+8]  (k 0..7), left in X0..X7.
//	Block 2 (block_len=4,  flags=0x1A = KEYED_HASH | CHUNK_END | ROOT):
//	    v[0..7]  = cv1 ; m[0] = data[64:68] ; m[1..15] = 0
//	    out[k]   = v[k] ⊕ v[k+8].
//
// Unlike the BLAKE2 two-block kernels, NO cv1 spill is needed: BLAKE3's
// fold is v[k] ⊕ v[k+8] alone (no cv1 term), so cv1 simply stays in
// X0..X7 as block 2's initial v[0..7]. The four lanes stay
// lane-parallel in XMM across both blocks; message words live in stack
// slots, v[15] (flags) in a stack slot with the v[12] excursion.
//
// Register allocation: see blake3_chain256_20_avx2_amd64.s.
...
TEXT ·blake3256ChainAbsorb68x4Avx2Asm(SB), NOSPLIT, $288-32
	MOVQ key+0(FP),       AX
	MOVQ seeds+8(FP),     CX
	...
	// Block 1 messages: m0..7 = data[0:32]^seed; m8..15 = data[32:64].
	DXS_LO( 0, 0, M(0))
	DXS_HI( 4, 0, M(1))
	DXS_LO( 8, 1, M(2))
	DXS_HI(12, 1, M(3))
	...
	// cv1[k] = v[k] ⊕ v[k+8] → X0..X7 (v15 flags from slot).
	VPXOR X8,  X0, X0
	VPXOR X9,  X1, X1
	VPXOR X10, X2, X2
	VPXOR X11, X3, X3
	...
	// Block 2 messages: m0 = data[64:68], m1..15 = 0.
	DATA_D(64, M(0))
	ZERO_SLOT(M(1))
	ZERO_SLOT(M(2))
	ZERO_SLOT(M(3))
