...
func Fused256Chain13x4(fixedKey *[32]byte, components []uint64, dataPtrs *[4]*byte, out *[4][4]uint64) {
	if FusedHasNEON && validComponents256(components) {
		blake3FusedChain13x4NeonAsm(fixedKey, &components[0], len(components)/4, dataPtrs, out)
		return
	}
...
func Fused256Chain20x4(fixedKey *[32]byte, components []uint64, dataPtrs *[4]*byte, out *[4][4]uint64) {
	if FusedHasNEON && validComponents256(components) {
		blake3FusedChain20x4NeonAsm(fixedKey, &components[0], len(components)/4, dataPtrs, out)
		return
	}
...
func Fused256Chain36x4(fixedKey *[32]byte, components []uint64, dataPtrs *[4]*byte, out *[4][4]uint64) {
	if FusedHasNEON && validComponents256(components) {
		blake3FusedChain36x4NeonAsm(fixedKey, &components[0], len(components)/4, dataPtrs, out)
		return
	}
...
func Fused256Chain68x4(fixedKey *[32]byte, components []uint64, dataPtrs *[4]*byte, out *[4][4]uint64) {
	if FusedHasNEON && validComponents256(components) {
		blake3FusedChain68x4NeonAsm(fixedKey, &components[0], len(components)/4, dataPtrs, out)
		return
	}
...
func Fused256Fill13x8(fixedKey *[32]byte, components []uint64, groupIdxBase uint64, out *[8][4]uint64) {
	...
	for h := 0; h < 2; h++ {
		...
		blake3FusedChain13x4NeonAsm(fixedKey, c, ng, &ptrs, out8x256Half(out, h))
	}
}
...
// Kernels (blake3_fusedchain256_<shape>x4_neon_arm64.s and the
// single-lane kernels blake3_fusedchain256_<shape>x1_gpr_arm64.s).
//
//go:noescape
func blake3FusedChain13x4NeonAsm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[4]*byte, out *[4][4]uint64)

//
//go:noescape
func blake3FusedChain20x4NeonAsm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[4]*byte, out *[4][4]uint64)

//
//go:noescape
func blake3FusedChain36x4NeonAsm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[4]*byte, out *[4][4]uint64)

//
//go:noescape
func blake3FusedChain68x4NeonAsm(fixedKey *[32]byte, comps *uint64, nGroups int, dataPtrs *[4]*byte, out *[4][4]uint64)
