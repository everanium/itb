...
func applyHashTier() {
	...
	case "aesni", "vaesavx2":
		forcetier.Warnf("blake3asm: no %s fused arm; keeping auto-dispatch", forcetier.HashTier())
	case "neon", "sve2", "sve":
...
func applyInterlockPRFFillTier() {
	...
	case "avx2", "vex":
		...
		if forcetier.InterlockPRFFillTier() == "vex" {
			forcetier.Warnf("blake3asm: no vex batch-16 arm; selecting the AVX2 arm")
		}
		HasAVX512X16, HasAVX2X16 = false, true
	case "aesni", "vaesavx2":
		forcetier.Warnf("blake3asm: no %s batch-16 arm; keeping auto-dispatch", forcetier.InterlockPRFFillTier())
	case "neon":
