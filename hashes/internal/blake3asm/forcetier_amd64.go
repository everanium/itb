...
// to both BLAKE3 dispatch families — the fused cascade flags and the
...
//	aesni  — BLAKE3 has no AES-based arm; the token forces scalar with
...
func applyHashTier() {
	...
	case "avx512":
		if !cpu.X86.HasAVX512F {
			forcetier.Warnf("blake3asm: avx512 fused tier needs AVX-512F; keeping auto-dispatch")
			return
		}
	...
	case "avx2", "vex":
		if !cpu.X86.HasAVX2 {
			forcetier.Warnf("blake3asm: %s fused tier needs AVX2; keeping auto-dispatch", forcetier.HashTier())
			return
		}
		if forcetier.HashTier() == "vex" {
			forcetier.Warnf("blake3asm: no vex fused arm; selecting the AVX2 kernels")
		}
		FusedHasAVX512, FusedHasAVX2 = false, true
		HasAVX512X16, HasAVX2X16 = false, true
	case "aesni":
		forcetier.Warnf("blake3asm: no aesni fused arm; forcing scalar")
		FusedHasAVX512, FusedHasAVX2 = false, false
		HasAVX512X16, HasAVX2X16 = false, false
	case "vaesavx2":
		forcetier.Warnf("blake3asm: no vaesavx2 fused arm; keeping auto-dispatch")
	case "neon", "sve2", "sve":
		forcetier.Warnf("blake3asm: %s tier is arm64-only; keeping auto-dispatch", forcetier.HashTier())
	case "scalar":
		FusedHasAVX512, FusedHasAVX2 = false, false
		HasAVX512X16, HasAVX2X16 = false, false
	}
...
func applyInterlockPRFFillTier() {
	...
	case "avx512":
		if !cpu.X86.HasAVX512F {
			forcetier.Warnf("blake3asm: avx512 batch-16 tier needs AVX-512F; keeping auto-dispatch")
			return
		}
		HasAVX512X16, HasAVX2X16 = true, false
	case "avx2", "vex", "vaesavx2":
		if !cpu.X86.HasAVX2 {
			forcetier.Warnf("blake3asm: %s batch-16 tier needs AVX2; keeping auto-dispatch", forcetier.InterlockPRFFillTier())
			return
		}
		if forcetier.InterlockPRFFillTier() != "avx2" {
			forcetier.Warnf("blake3asm: no %s batch-16 arm; selecting the AVX2 arm", forcetier.InterlockPRFFillTier())
		}
		HasAVX512X16, HasAVX2X16 = false, true
	case "aesni":
		forcetier.Warnf("blake3asm: no aesni batch-16 arm; forcing scalar")
		HasAVX512X16, HasAVX2X16 = false, false
	case "neon":
		forcetier.Warnf("blake3asm: neon batch-16 tier is arm64-only; keeping auto-dispatch")
	case "scalar":
		HasAVX512X16, HasAVX2X16 = false, false
	}
}
