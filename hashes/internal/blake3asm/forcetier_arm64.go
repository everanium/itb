...
func applyFusedHashTier() {
	...
	default:
		forcetier.Warnf("blake3asm: %s tier is amd64-only; keeping auto-dispatch", forcetier.HashTier())
	}
}
...
func applyFusedInterlockPRFFillTier() {
	...
	case "scalar":
		HasNEONX16 = false
	default:
		forcetier.Warnf("blake3asm: %s batch-16 tier is amd64-only; keeping auto-dispatch", forcetier.InterlockPRFFillTier())
	}
}
