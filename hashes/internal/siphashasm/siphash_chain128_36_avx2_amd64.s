...
// with 36-byte per-lane data input. AVX2-only fallback for hosts lacking the AVX-512
...
DATA rol16qb<>+0(SB)/8,  $0x0504030201000706
DATA rol16qb<>+8(SB)/8,  $0x0d0c0b0a09080f0e
DATA rol16qb<>+16(SB)/8, $0x0504030201000706
DATA rol16qb<>+24(SB)/8, $0x0d0c0b0a09080f0e
GLOBL rol16qb<>(SB), RODATA|NOPTR, $32

#define ROLQ32(r) VPSHUFD $0xB1, r, r
#define ROLQ16(r) VPSHUFB rol16qb<>(SB), r, r
...
TEXT ·sipHash24Chain128Absorb36x4Avx2Asm(SB), NOSPLIT, $0-24
	MOVQ seeds+0(FP),    BX
	...
	// ===== final padded block: data[32:36] | (36<<56) =====
	MOVL 32(R8),  R12; VMOVQ R12, X4
	MOVL 32(R9),  R12; VPINSRQ $1, R12, X4, X4
	MOVL 32(R10), R12; VMOVQ R12, X6
	MOVL 32(R11), R12; VPINSRQ $1, R12, X6, X6
	VINSERTI128 $1, X6, Y4, Y4
	MOVQ $0x2400000000000000, R12; VMOVQ R12, X6; VPBROADCASTQ X6, Y6; VPXOR Y6, Y4, Y4
	SIP_ABSORB
