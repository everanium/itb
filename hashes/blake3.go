...
// hashes/internal/blake3asm, installed by the blake3 entry's
...
//
// The single arm is identical to BLAKE3WithKey(key); the batched arm
// evaluates the four lanes through it under their per-lane seeds and
// is bit-exact with four single calls on every input. Every shipped
// constructor path attaches the fused cascade hooks of the blake3
// registry entry, which the batched ChainHash256 entry points consult
// first, so the batched arm is the fallback for seeds built without
// those hooks.
