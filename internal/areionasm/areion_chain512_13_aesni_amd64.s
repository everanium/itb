...
// message shape. 13 ≤ 56-byte chunkSize, so the absorb is a single
// SoEM-512 round. AES-NI-only fallback; identical to the 20-byte kernel
// (areion_chain512_20_aesni_amd64.s) except the state staging loads
// exactly data[8..13] (MOVL 4B + MOVBLZX 1B) so an exactly-13-byte lane
// buffer is never over-read.
//
// Per-lane 64-byte state:
//   b0: [lengthTag(13) | data[0..8]]
//   b1: [data[8..12] | data[12] | 0]
//   b2, b3: 0
...
// STAGE13 stages the 13-byte lane state (data ptr in reg dp) into SP.
#define STAGE13(dp) \
	MOVQ $13, R12;   MOVQ R12, 0(SP); \
	MOVQ 0(dp), R12; MOVQ R12, 8(SP); \
	MOVQ $0, 16(SP); MOVQ $0, 24(SP); \
	MOVL 8(dp), R12; MOVL R12, 16(SP); \
	MOVBLZX 12(dp), R12; MOVB R12, 20(SP); \
...
TEXT ·Areion512ChainAbsorb13x4AesNi(SB), NOSPLIT, $64-32
