...
// TestAreion256ChainAbsorb13x4_Parity verifies the Interlocked Barrier
// fill kernel against the reference at the 13-byte data shape.
func TestAreion256ChainAbsorb13x4_Parity(t *testing.T) {
	testChainAbsorbParity(t, 13, Areion256ChainAbsorb13x4)
}

// TestAreion256ChainAbsorb13x4_BitSensitivity flips every one of the
// 104 bits of the 13-byte input (per lane) and requires that lane's
// digest to change.
func TestAreion256ChainAbsorb13x4_BitSensitivity(t *testing.T) {
	if !HasVAESAVX512 {
		t.Skip("requires VAES + AVX-512")
	}
	var fixedKey [32]byte
	...
	run := func(d [][]byte) [4][4]uint64 {
		...
		var out [4][4]uint64
		Areion256ChainAbsorb13x4(&fixedKey, &seeds, &dp, &out)
		return out
	}
	baseOut := run(base)
	for lane := 0; lane < 4; lane++ {
...
func BenchmarkAreion256ChainAbsorb13x4_Reference(b *testing.B) {
	benchChainAbsorbReference(b, 13, nil)
}
func BenchmarkAreion256ChainAbsorb13x4_Fused(b *testing.B) {
	benchChainAbsorbReference(b, 13, Areion256ChainAbsorb13x4)
}
