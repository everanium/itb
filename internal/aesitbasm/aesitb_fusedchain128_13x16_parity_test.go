...
type fusedX16Fn func(key *[16]byte, comps []uint64, groupIdxBase uint64, out *[16][2]uint64)
...
func checkFusedX16Parity(t *testing.T, label string, kernel fusedX16Fn) {
	...
	check := func(what string, key *[16]byte, comps []uint64, base uint64) {
		...
		kernel(key, comps, base, &got)
		scalarFusedX16(key, comps, base, &want)
		for i := 0; i < 16; i++ {
			if got[i] != want[i] {
		...
		if len(comps) == 2 {
			scalarBatchX16(key, base, comps[0], comps[1], &want)
			for i := 0; i < 16; i++ {
				if got[i] != want[i] {
...
func TestScalarFusedX16MatchesSequential(t *testing.T) {
	...
	for _, pairs := range fusedX16PairCounts {
		for iter := 0; iter < 16; iter++ {
			...
			base := rng.Uint64()
			var got [16][2]uint64
			scalarFusedX16(&key, comps, base, &got)
			for i := 0; i < 16; i++ {
				var buf [13]byte
				buf[0] = 0x03
				binary.LittleEndian.PutUint64(buf[1:9], base+uint64(i))
				lo, hi := ChainAbsorb(&key, buf[:], comps[0], comps[1])
				for k := 2; k < len(comps); k += 2 {
					lo, hi = ChainAbsorb(&key, buf[:], comps[k]^lo, comps[k+1]^hi)
				}
	...
	for _, pairs := range fusedX16PairCounts {
		for _, tc := range fusedX16FixedCases {
			...
			for _, base := range append([]uint64{rng.Uint64()}, tc.bases...) {
				key := tc.key
				var got [16][2]uint64
				FusedChain13x16(&key, comps, base, &got)
				for i := 0; i < 16; i++ {
					var buf [13]byte
					buf[0] = 0x03
					binary.LittleEndian.PutUint64(buf[1:9], base+uint64(i))
					var one [2]uint64
					FusedChain13x1(&key, comps, &buf[0], &one)
					if got[i] != one {
...
func TestFusedChain13x16RejectsShortComponents(t *testing.T) {
	...
	comps := []uint64{1, 2, 3}
	var got, want [16][2]uint64
	FusedChain13x16(&key, comps, 5, &got)
	scalarFusedX16(&key, comps, 5, &want)
	if got != want {
