...
// aesitbasm_entropy_test.go — the input-entropy differential audit
...
func refLanes(in *kernelaudit.Inputs) [][2]uint64 {
	...
	for l := range in.Lanes {
		out[l][0], out[l][1] = ScalarFusedChain(&key, in.Comps, in.Lanes[l])
	}
	return out
}
...
func refFill(in *kernelaudit.Inputs) [][2]uint64 {
	...
	copy(key[:], in.Key)
	var out [16][2]uint64
	scalarFusedX16(&key, in.Comps, in.Base, &out)
	return out[:]
}
...
func auditX1(t *testing.T, label string, n int, f fusedX1Fn) {
	...
	for _, pairs := range auditPairCounts {
		...
		reportAudit(t, kernelaudit.DifferentialRef(cfg, func(in *kernelaudit.Inputs) [][2]uint64 {
			...
			copy(key[:], in.Key)
			var out [2]uint64
			f(&key, in.Comps, &in.Lanes[0][0], &out)
			return [][2]uint64{out}
		}, refLanes))
	}
}
