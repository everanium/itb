...
type fusedAsmX16 func(*[16]byte, *uint64, int, uint64, *[16][2]uint64)

func wrapX16(f fusedAsmX16) fusedX16Fn {
	return func(key *[16]byte, comps []uint64, groupIdxBase uint64, out *[16][2]uint64) {
		f(key, &comps[0], len(comps)/2, groupIdxBase, out)
	}
}
...
func amd64FusedX16Tiers() []fusedX16Tier {
	return []fusedX16Tier{
		{"avx512", func() bool { return aes.CPU.HasVAES && aes.CPU.HasAVX512 }, wrapX16(aesITB128FusedChain13x16Avx512Asm), true, false, false, false},
		{"vaesavx2", func() bool { return aes.CPU.HasVAES && aes.CPU.HasAVX2 }, wrapX16(aesITB128FusedChain13x16VaesAvx2Asm), false, true, false, false},
		{"vex", func() bool { return aes.CPU.HasAESNI && aes.CPU.HasAVX2 }, wrapX16(aesITB128FusedChain13x16VexAsm), false, false, true, false},
		{"aesni", func() bool { return aes.CPU.HasAESNI }, wrapX16(aesITB128FusedChain13x16AesNiAsm), false, false, false, true},
		...
		for _, tc := range fusedX16FixedCases {
			...
			for _, base := range tc.bases {
				...
				for _, tier := range amd64FusedX16Tiers() {
					...
					key := tc.key
					var out [16][2]uint64
					FusedChain13x16(&key, comps, base, &out)
