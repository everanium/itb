...
func FusedChain20x8(key *[16]byte, components []uint64, dataPtrs *[8]*byte, out *[8][2]uint64) {
	if FusedX8Active() && validComponents(components) {
		aesITB128FusedChain20x8Avx512Asm(key, &components[0], len(components)/2, dataPtrs, out)
		return
	}
...
func FusedChain36x8(key *[16]byte, components []uint64, dataPtrs *[8]*byte, out *[8][2]uint64) {
	if FusedX8Active() && validComponents(components) {
		aesITB128FusedChain36x8Avx512Asm(key, &components[0], len(components)/2, dataPtrs, out)
		return
	}
...
func FusedChain68x8(key *[16]byte, components []uint64, dataPtrs *[8]*byte, out *[8][2]uint64) {
	if FusedX8Active() && validComponents(components) {
		aesITB128FusedChain68x8Avx512Asm(key, &components[0], len(components)/2, dataPtrs, out)
		return
	}
	fusedChainX8ViaX4(FusedChain68x4, key, components, dataPtrs, out)
}

// Eight-lane fused kernels (aesitb_fusedchain128_<shape>x8_avx512_amd64.s).
//
//go:noescape
