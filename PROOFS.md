...
> **Disclaimer.** These proofs are self-analysis by the authors and have not been independently verified. ITB is an experimental construction. The information-theoretic barrier is a software-level property — it provides no guarantees against hardware-level attacks (DPA/SPA, Spectre, Meltdown, cache timing). Non-cryptographic hash functions are intended solely for research purposes; using them in any real-world application is potentially dangerous. No warranty is provided.
...
Proofs are stated for the 64-bit variant (HashFunc). They apply equally to 128-bit (HashFunc128, ChainHash128), 256-bit (HashFunc256, ChainHash256), and 512-bit (HashFunc512, ChainHash512) variants — the pixel processing and barrier properties are width-independent.
...
**With non-invertible hash (requirement 5):** To verify a candidate (r', m'), the attacker would need to check if m' equals bits 3-58 of ChainHash(counter||nonce, dataSeed). This requires evaluating ChainHash with the correct dataSeed — but dataSeed is unknown, and ChainHash cannot be inverted.
...
For P = 81 (minimum container, 512-bit key): 7^81 ≈ 10^68 ≈ 2^227 — below the Landauer thermodynamic limit (~2^306 ≈ 10^92), but each of the 10^68 rotation configurations requires independent ChainHash evaluation to verify, making exhaustive search computationally infeasible with any foreseeable technology. The noise barrier (2^648 for 81 pixels, Proof 5) independently exceeds the Landauer limit. ∎
...
**Proof.** For keyBits = 512:

```
MinPixels = ⌈512 / 7⌉ = 74
```

Square container: side = ⌈√74⌉ = 9, P = 81.
...
The barrier strictly exceeds the key space by a factor of 2^136.
...
**Theorem.** For any container C encrypted with seeds (nS, dS, sS) and any candidate seed tuple (nS', dS', sS') ≠ (nS, dS, sS), the output of Decrypt(nS', dS', sS', C) is computationally indistinguishable from uniform random bytes.

**Proof.** Decrypt extracts a byte sequence by:
...
**Theorem.** `EncryptAuthenticated` with MAC over full capacity (COBS + null + fill) is designed to prevent CCA spatial patterns and false null-terminator attacks (given a secure MAC function).
...
**Part B: False null-terminator prevention.** MAC is verified BEFORE null-terminator search in DecryptAuthenticated:

```
1. Decrypt entire capacity → decoded[]
...
**Mitigation:** The nonce is mandatory and internally generated from crypto/rand on every Encrypt call. The caller cannot reuse nonces by design. ∎
