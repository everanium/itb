#!/usr/bin/env bash
#
# One-step test runner for the Java binding. Builds libitb3.so + the
# JNI shim + jars via build.sh, then invokes the JUnit 5 suite through
# Gradle. Positional arguments are forwarded straight to Gradle (e.g.
# `./run_tests.sh --tests '*SmokeTest'`).
#
