...
A long-run stress harness under `bindings/java/loop/` holds one
...
the binding holds no thread-affine state behind it, so `--goroutines`
is the thread count verbatim.
