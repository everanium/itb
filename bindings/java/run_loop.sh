#!/usr/bin/env bash
#
# Fleet entry point for the loop stress harness of the Java binding:
# builds the utility jar with Gradle (a no-op when it is up to date;
# libitb3.so, the JNI shim and the library jar are assumed built by
...
JNI="$PWD/build/jni/libitb3_jni.so"
if [ ! -f "$JNI" ]; then
