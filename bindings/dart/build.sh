...
# reach the shared dist/linux-amd64/libitb3.so, the pub cache, or
...
if [[ "${ITB_SKIP_CLEAN:-0}" == "1" ]]; then
    ...
    # .dart_tool/ holds both the resolved package config and the kernel
    # snapshots `dart run` reuses, so it goes on every build: a kept
    # snapshot is what lets an entry point run from an older source.
    clean_target '.dart_tool'
    clean_target 'build'
    clean_target 'doc/api'
    clean_tree '*.dill'
    clean_tree '*.snapshot'
    if [[ "${ITB_KEEP_DOWNLOADS:-0}" == "1" ]]; then
        echo "[clean] ITB_KEEP_DOWNLOADS=1: keeping pubspec.lock (weaker guarantee)"
    else
        clean_target 'pubspec.lock'
    fi
fi
