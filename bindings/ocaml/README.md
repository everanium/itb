...
The convenience driver builds `libitb3.so` and the dune project
...
```ocaml
module Itb = Itb3   (* the only line that names the library version *)
...
```ocaml
Itb.max_workers receiver 4   (* clamped by libitb3; <= 0 selects auto *)
```
...
keys are rejected by libitb3 with a diagnostic.
...
- Byte buffers cross the FFI boundary zero-copy (`ocaml_bytes`); the
  OCaml runtime lock is held for the duration of each libitb3 call, so
