...
the `Itb` module: `create` / `open_` / `blob`, Single Message
`encrypt_message` / `decrypt_message`, `rekey` / `close`, incremental
stream sessions (`encrypt_stream` / `decrypt_stream` with `write` /
`end_` / `read` / `drain_all`), the `hashes` / `profiles` / `version`
...
The `?opts` association list overrides the profile default per call
(chunk size, outer cipher, parallax on/off, wrapper on/off, MAC
name, palette) — rendered into the URL-query opts string the Go
side validates:

```ocaml
let opts = [("chunkSize", "65536"); ("withWrapper", "false")] in
let sender = Itb.create "singlemsg-triple-mac-v1" ~opts () in
let receiver = Itb.open_ "singlemsg-triple-mac-v1" (Itb.blob sender) ~opts
```
...
```ocaml
Itb.rekey ~perm:(Bytes.make 32 '\x11') ~wrap:(Bytes.make 32 '\x22') sender;
let receiver = Itb.open_ "singlemsg-triple-mac-v1" (Itb.blob sender)
```
...
the alcotest suite. The suite covers the library version, the hash
roster order, the shipped profile list, Single Message and stream
round trips, incremental-read loops, tampered-wire rejection,
closed-handle mapping, large-payload buffer sizing, rekey, and error
...
./bindings/ocaml/eitb/eitb hashes
