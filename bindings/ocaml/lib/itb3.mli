(** Thin OCaml proxy over the libitb3 shared library's Triple Pipeline
    surface.

    The library wraps the [ITB_Triple_*] C ABI exported by
    [cmd/cshared] (libitb3.so / .dylib / .dll) through ocaml-ctypes —
    ...
    {[
      ...
      let wire = Itb3.encrypt_message sender (Bytes.of_string "hello") in
      assert (Itb3.decrypt_message receiver wire = Bytes.of_string "hello")
    ]} *)

(** Raised on every failed libitb3 call. The [int] is the libitb3
...
(** [create profile ?opts ()] constructs a fresh pipeline against the
    ...
    unknown keys are rejected by libitb3). An unregistered name raises
...
(** [load ?perm ?wrap blob] reconstructs a pipeline from a blob
    ...
    validated by libitb3). The profile shape travels inside the blob —
...
(** [load_f ?perm ?wrap path] is [load] for a blob stored at [path];
    the file is read inside libitb3 (a missing or unreadable file
...
(** [save_f p path] writes the current blob to [path] inside libitb3
...
(** [read_into sess dst cap] is the caller-buffer drain primitive:
    ...
    exceeds the real length of [dst] — libitb3 honours [cap] as the
