...
type Profile = Everanium.Itb3.Profile

/// A Triple Pipeline session.
///
/// The type is the C# binding's <c>Everanium.Itb3.Pipeline</c> — it implements
/// <c>IDisposable</c>, so <c>use</c> / <c>use!</c> bindings give the
/// deterministic release path (libitb3 zeroes key material
...
type Pipeline = Everanium.Itb3.Pipeline
...
module Pipeline =
    ...
    let init (profile: string) (opts: Opts) : Result<Pipeline, ItbError> =
        ItbError.attempt (fun () -> Everanium.Itb3.Pipeline.Init(profile, Opts.toNative opts))
    ...
    let load (blob: byte[]) : Result<Pipeline, ItbError> =
        ItbError.attempt (fun () -> Everanium.Itb3.Pipeline.Load(ReadOnlySpan blob))
    ...
    let loadWithMasters (blob: byte[]) (permMaster: byte[]) (wrapMaster: byte[]) : Result<Pipeline, ItbError> =
        ItbError.attempt (fun () -> Everanium.Itb3.Pipeline.Load(ReadOnlySpan blob, permMaster, wrapMaster))

    /// <c>load</c> for a blob stored in a file; the file is read
    /// inside the library.
    let loadF (path: string) : Result<Pipeline, ItbError> =
        ItbError.attempt (fun () -> Everanium.Itb3.Pipeline.LoadF path)
    ...
    let loadFWithMasters (path: string) (permMaster: byte[]) (wrapMaster: byte[]) : Result<Pipeline, ItbError> =
        ItbError.attempt (fun () -> Everanium.Itb3.Pipeline.LoadF(path, permMaster, wrapMaster))
    ...
    let inspect (blob: byte[]) : Result<Profile, ItbError> =
        ItbError.attempt (fun () -> Everanium.Itb3.Pipeline.Inspect(ReadOnlySpan blob))
    ...
    let register (name: string) (profile: Profile) : Result<unit, ItbError> =
        ItbError.attempt (fun () -> Everanium.Itb3.Pipeline.Register(name, profile))
    ...
    let lookup (name: string) : Result<Profile, ItbError> =
        ItbError.attempt (fun () -> Everanium.Itb3.Pipeline.Lookup name)
