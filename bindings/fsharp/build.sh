...
# freshly built F# code. The C# binding's own build.sh is deliberately
# not invoked instead: it would additionally clean and rebuild the C#
# test, bench and eitb projects, which this binding neither references
# nor owns.
#
...
if [ "$SKIP_CLEAN" = "1" ]; then
    ...
    echo "==> cleaning F# binding artefacts"
    clean_under "$BINDING_DIR" \
        src/Everanium.LibItb3.FSharp/bin       src/Everanium.LibItb3.FSharp/obj \
        tests/Everanium.LibItb3.FSharp.Tests/bin \
        tests/Everanium.LibItb3.FSharp.Tests/obj \
        tests/Everanium.LibItb3.FSharp.Tests/TestResults \
        bench/Everanium.LibItb3.FSharp.Bench/bin \
        bench/Everanium.LibItb3.FSharp.Bench/obj \
        eitb/Everanium.LibItb3.FSharp.Eitb/bin \
        eitb/Everanium.LibItb3.FSharp.Eitb/obj \
        loop/bin                               loop/obj \
        TestResults
...
cd "$BINDING_DIR"
# The solution carries the C# library, the F# library, and the test,
# bench and eitb projects, so one build covers every artefact the
# sibling scripts run.
echo "==> building F# binding (dotnet build -c Release)"
dotnet build Everanium.LibItb3.FSharp.sln -c Release

require_built "$CSHARP_LIB_DIR/bin/Release/net*/Everanium.LibItb3.dll"
require_built "$BINDING_DIR/src/Everanium.LibItb3.FSharp/bin/Release/net*/Everanium.LibItb3.FSharp.dll"
require_built "$BINDING_DIR/eitb/Everanium.LibItb3.FSharp.Eitb/bin/Release/net*/Everanium.LibItb3.FSharp.Eitb.dll"
require_built "$BINDING_DIR/bench/Everanium.LibItb3.FSharp.Bench/bin/Release/net*/Everanium.LibItb3.FSharp.Bench.dll"
require_built "$BINDING_DIR/loop/bin/Release/net*/Everanium.LibItb3.FSharp.Loop.dll"
