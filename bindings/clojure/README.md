...
A long-run stress harness under `bindings/clojure/loop/` holds one
...
neither the Clojure record around it nor the Java layer beneath holds
thread-affine state, so `--goroutines` is the thread count verbatim.
