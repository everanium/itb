(ns io.github.everanium.itb3.clojure.error
  "Error surface shared by every fallible call in the binding.

  A non-OK libitb3 status surfaces as a `clojure.lang.ExceptionInfo`
  (`ex-info`) whose data map carries:

    :type   ::itb
    :status status keyword (see io.github.everanium.itb3.clojure.status)
  ...
  (:require [io.github.everanium.itb3.clojure.status :as status])
