(ns io.github.everanium.itb3.clojure.core
  ...
  letting `with-open` do it) frees the handle — libitb3 zeroes key
...
(defn ^java.lang.AutoCloseable init
  ...
  `opts-map` follows the io.github.everanium.itb3.clojure.opts roster.
  (The return tag lets `with-open` resolve `.close` statically; the
  value is a Pipeline record.)"
  ([profile] (init profile nil))
  ([profile opts-map]
   (->Pipeline
    (itb-call (io.github.everanium.itb3.Pipeline/init ^String profile (opts/build opts-map))))))
...
(defn ^java.lang.AutoCloseable load
  ...
  ([blob perm-master wrap-master]
   (->Pipeline
    (itb-call (io.github.everanium.itb3.Pipeline/load ^bytes blob
...
(defn ^java.lang.AutoCloseable load-f
  ...
  ([path perm-master wrap-master]
   (->Pipeline
    (itb-call (io.github.everanium.itb3.Pipeline/loadF ^String (str path)
                                                ^bytes perm-master ^bytes wrap-master)))))

(defn inspect
  "Decodes the blob's embedded profile record (as a profile map, see
  io.github.everanium.itb3.clojure.profile) without opening a Pipeline. No
  registry read, no primitive probe."
  [^bytes blob]
  (profile/profile->map (itb-call (io.github.everanium.itb3.Pipeline/inspect blob))))
...
(defn register!
  ...
  (itb-call (io.github.everanium.itb3.Pipeline/register
...
(defn lookup
  ...
  [profile-name]
  (profile/profile->map
   (itb-call (io.github.everanium.itb3.Pipeline/lookup ^String profile-name))))
