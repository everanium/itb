 ...
 * Bonus 1b in the design notes. The cache grows on demand with the
...
function emitChunkAuthSingle(
  ...
  // Uint8Array per chunk (Bonus 1b in the design notes). When
...
export class StreamEncryptorAuthTriple {
   ...
   * encrypt dispatch (the design notes).
   *
   * @internal
   */
  private _outBuf: StreamAuthCache = { buf: null };
...
export class StreamDecryptorAuthTriple {
   ...
   * decrypt dispatch (the design notes).
   *
   * @internal
   */
  private _outBuf: StreamAuthCache = { buf: null };
