...
#   3. The OS default loader path (-litb3 only).
#
...
emit() {
    printf '%s' "-L$1 -Wl,-rpath,$1 -litb3"
}

if [ -n "${ITB_LIBITB3_PATH:-}" ] && [ -f "$ITB_LIBITB3_PATH" ]; then
    emit "$(CDPATH= cd -- "$(dirname -- "$ITB_LIBITB3_PATH")" && pwd)"
    exit 0
fi
