...
--
-- The C core carries its own basename (libitb3_lua.so) so it can never
-- be confused with the native libitb3.so it links against; it is loaded
-- by explicit path below rather than through LUA_CPATH. `require
-- "itb3"` therefore always returns the merged table below.

local function core_path()
    local src = debug.getinfo(1, "S").source
    local path = src:match("^@(.*)$") or src
    local dir = path:match("^(.*)[/\\][^/\\]*$") or "."
    return dir .. "/libitb3_lua.so"
end

local loader = assert(package.loadlib(core_path(), "luaopen_itb3"))
local M = loader()
...
-- passes through to Go verbatim (percent-encoded); libitb3 rejects
