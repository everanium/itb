...
```kotlin
import io.github.everanium.itb3.kotlin.*
...
A long-run stress harness under `bindings/kotlin/loop/` holds one
...
**shared-handle** — JVM platform threads call into one Pipeline handle
concurrently, since the handle is an opaque Go-side registry key and
no layer between the utility and it holds thread-affine state, so
`--goroutines` is the thread count verbatim.
