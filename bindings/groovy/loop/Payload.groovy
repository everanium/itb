 ...
 *   <li>ROTATING: the buffer is regenerated before every iteration,
 *       so no two encrypt calls see the same plaintext.</li>
 *   <li>PATTERN_ZERO / PATTERN_FF: degenerate constant fills (all
 *       0x00 / all 0xFF) probing minimum-entropy plaintext
 *       handling.</li>
...
/**
 * The deterministic per-worker generator's state. Groovy has no
...
final class Payload {
    ...
    private static final ThreadLocal<SecureRandom> CSPRNG =
            ThreadLocal.withInitial({ new SecureRandom() } as java.util.function.Supplier<SecureRandom>)
     ...
     *
     * <p>Groovy-specific. The JDK's default {@code SecureRandom}
     ...
     * way for a JVM source set to reach that entry — a native
    ...
    static boolean fill(PayloadMode mode, boolean seeded, Rng rng, byte[] buf) {
        switch (mode) {
            ...
            case PayloadMode.ROTATING:
                ...
                return true
            case PayloadMode.PATTERN_ZERO:
                Arrays.fill(buf, (byte) 0)
                return true
            case PayloadMode.PATTERN_FF:
                Arrays.fill(buf, (byte) 0xFF)
                return true
            default:
                for (int i = 0; i < buf.length; i++) {
                    buf[i] = (byte) (((int) ('A' as char)) + (i % 26))
                }
                return true
        }
    }
