...
final class Ops {
    ...
    private static boolean rekeyPipes(RunState r, int id, long iter) {
        ...
        if (r.cfg.parallax) {
            ...
            if (!Payload.fillRandom(perm)) {
                Worker.fail(r, id, "g${id} iter ${iter}: csprng: parallax master".toString())
                return false
            }
        }
        if (r.cfg.wrapper) {
            ...
            if (!Payload.fillRandom(wrap)) {
                Worker.fail(r, id, "g${id} iter ${iter}: csprng: wrapper master".toString())
                return false
            }
        }
        ...
        try {
            if (r.pipes.stream != null) {
                try {
                    ...
                    Worker.fail(r, id, "g${id} iter ${iter}: Rekey(${r.streamProfile}): " +
                            "${Worker.detail(e)}".toString())
                    return false
                }
            }
            if (r.pipes.msg != null) {
                try {
                    ...
                    Worker.fail(r, id, "g${id} iter ${iter}: Rekey(${r.msgProfile}): " +
                            "${Worker.detail(e)}".toString())
                    return false
                }
            }
        ...
        Main.logLine("rekey: g${id} iter ${iter} rotated parallax + wrapper masters " +
                "(rekey #${n})".toString())
        return true
    }
    ...
    private static boolean blobCyclePipes(RunState r, int id, long iter) {
        ...
        try {
            if (r.pipes.stream != null) {
                ...
                try {
                    ...
                    Worker.fail(r, id, "g${id} iter ${iter}: Load(${r.streamProfile}): " +
                            "${Worker.detail(e)}".toString())
                    return false
                }
            ...
            if (r.pipes.msg != null) {
                ...
                try {
                    ...
                    Worker.fail(r, id, "g${id} iter ${iter}: Load(${r.msgProfile}): " +
                            "${Worker.detail(e)}".toString())
                    return false
                }
                r.pipes.msg.close()
        ...
        Main.logLine("blob-cycle: g${id} iter ${iter} reopened from session blob " +
                "(cycle #${n})".toString())
        return true
    }
    ...
    static boolean maintenance(RunState r, int id, long iter) {
        ...
        if (cfg.blobCycleEvery > 0 && iter % cfg.blobCycleEvery == 0L &&
