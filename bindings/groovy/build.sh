...
# so no output can survive from an earlier invocation.
#
...
# means the shim is in place before any Groovy code loads it.
#
...
if [ "$SKIP_CLEAN" = "1" ]; then
    ...
    echo "==> cleaning Groovy binding artefacts"
    clean_under "$BINDING_DIR" build .gradle out
fi
