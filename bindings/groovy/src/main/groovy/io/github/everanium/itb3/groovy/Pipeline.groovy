...
import io.github.everanium.itb3.Pipeline as JPipeline
 ...
 * refreshes it. {@code close()} releases the native handle (libitb3
 ...
 *
 * <p>Every failing libitb3 call surfaces as {@link ItbException}
...
final class Pipeline implements AutoCloseable {
    ...
    static <T> T withPipeline(String profile, Opts opts = new Opts(),
            @ClosureParams(value = SimpleType, options = 'io.github.everanium.itb3.groovy.Pipeline')
            Closure<T> body) {
    ...
    <T> T encrypting(
            @ClosureParams(value = SimpleType, options = 'io.github.everanium.itb3.groovy.EncryptStream')
            Closure<T> body) {
    ...
    <T> T decrypting(
            @ClosureParams(value = SimpleType, options = 'io.github.everanium.itb3.groovy.DecryptStream')
            Closure<T> body) {
    ...
    /** Releases the native handle (libitb3 closes it first, zeroing
