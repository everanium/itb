#!/usr/bin/env bash
#
# One-step test runner for the Python binding. Builds libitb3.so via
# build.sh, points ITB_LIBITB3_PATH at the freshly-built shared
# library, then invokes `python -m unittest discover` against the
# tests/ tree. Forwards any positional arguments through to unittest
# (e.g. a specific test module via dotted path).
#
# Usage:
