...
The convenience driver builds `libitb3.so` and compile-checks the
...
`inspect` returns the record as a `dict` decoded from the JSON libitb3
emits; absent keys are optional fields at their zero value.
