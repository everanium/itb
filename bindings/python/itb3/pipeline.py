...
class Pipeline:
    ...
    handle — libitb3 zeroes key material internally.
    ...
    def init(cls, profile: str, opts: Opts | None = None) -> Pipeline:
        ...
        (the undersized attempt is closed by libitb3 before
        returning)."""
        s = _ffi.syms()
...
# Profile record: the JSON object libitb3 emits from inspect / lookup
...
def register(name: str, profile: Profile | str) -> None:
    ...
    libitb3; a duplicate name fails with
