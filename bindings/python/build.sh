...
if [[ "${ITB_SKIP_CLEAN:-0}" == "1" ]]; then
    ...
    clean_tree '__pycache__'
    clean_tree '*.pyc'
    clean_tree '*.pyo'
    clean_tree '*.egg-info'
    clean_target 'build'
    clean_target 'dist'
    clean_target '.pytest_cache'
    clean_target '.mypy_cache'
    clean_target '.ruff_cache'
    clean_target '.tox'
    clean_target 'htmlcov'
    clean_target '.coverage'
    # The binding declares no third-party runtime dependency, so there
    # is no fetched dependency tree to preserve under
    # ITB_KEEP_DOWNLOADS; the wipe above is already the full guarantee.
fi
