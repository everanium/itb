...
import itb3_gleam.{type ItbError}
...
@external(erlang, "itb3_gleam_ffi", "encrypt_stream")
fn ffi_encrypt_stream(pipeline: Pipeline) -> Result(SessionHandle, ItbError)

@external(erlang, "itb3_gleam_ffi", "decrypt_stream")
fn ffi_decrypt_stream(pipeline: Pipeline) -> Result(SessionHandle, ItbError)

@external(erlang, "itb3_gleam_ffi", "stream_write")
fn ffi_stream_write(
  handle: SessionHandle,
  data: BitArray,
) -> Result(Nil, ItbError)

@external(erlang, "itb3_gleam_ffi", "stream_end")
fn ffi_stream_end(handle: SessionHandle) -> Result(Nil, ItbError)

@external(erlang, "itb3_gleam_ffi", "stream_read")
fn ffi_stream_read(
  handle: SessionHandle,
  max_bytes: Int,
) -> Result(#(BitArray, Bool), ItbError)

@external(erlang, "itb3_gleam_ffi", "stream_free")
