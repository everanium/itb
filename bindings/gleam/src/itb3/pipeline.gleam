...
//// release the Go-side state (libitb3 zeroes key material
...
import itb3_gleam.{type ItbError, type Opts}
...
@external(erlang, "itb3_gleam_ffi", "init")
...
@external(erlang, "itb3_gleam_ffi", "load")
...
@external(erlang, "itb3_gleam_ffi", "load_f")
...
/// `rekey`.
@external(erlang, "itb3_gleam_ffi", "save")
...
@external(erlang, "itb3_gleam_ffi", "save_f")
...
@external(erlang, "itb3_gleam_ffi", "max_workers")
...
/// same Pipeline.
@external(erlang, "itb3_gleam_ffi", "rekey")
pub fn rekey(
  pipeline: Pipeline,
  perm_master: BitArray,
  wrap_master: BitArray,
...
@external(erlang, "itb3_gleam_ffi", "free")
pub fn free(pipeline: Pipeline) -> Nil

/// One call, one self-contained wire.
@external(erlang, "itb3_gleam_ffi", "encrypt_message")
...
@external(erlang, "itb3_gleam_ffi", "decrypt_message")
...
/// bounded-memory streaming use the incremental `itb3/stream` session.
@external(erlang, "itb3_gleam_ffi", "encrypt_stream_one_shot")
...
@external(erlang, "itb3_gleam_ffi", "decrypt_stream_one_shot")
