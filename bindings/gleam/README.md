...
public surface is `itb3_gleam` (version, the profile catalogue
...
regular BEAM schedulers. The top-level module is named `itb3_gleam`
...
27+, rebar3, and Gleam 1.11+. macOS: the same via Homebrew; libitb3
...
(`bindings/erlang/priv/libitb3_nif.so`) resolves `libitb3.so` through
...
let assert Ok(Nil) = pipeline.save_f(sender, "session.blob") // written by libitb3, mode 0600
...
let assert Ok(record) = itb.inspect(blob)                 // profile record, no Pipeline
```

`itb3_gleam.inspect` returns the record as JSON text; absent keys are
optional fields at their zero value. The Gleam stdlib carries no JSON
codec, so the text is handed over verbatim for the caller's own
decoder (e.g. `gleam_json`).
...
libitb3; a duplicate name returns `Error(ItbError("profile_exists", _))`.
...
- The Erlang backend joins the code path at the first libitb3 call,
