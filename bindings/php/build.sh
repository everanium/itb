#!/usr/bin/env bash
#
# One-step build for the PHP binding's libitb3.so dependency.
# Prerequisites (Go, PHP 7.4+ with the FFI extension available) must be
# installed separately; see README.md "Prerequisites" section.
#
# Every artefact this binding owns is removed before the build, so
# nothing in the tree predates the invocation.
#
# Usage:
#   ./build.sh             # default build (full asm stack)
#   ./build.sh --noitbasm  # opt out of ITB's SIMD asm kernels
#                          # (use on hosts without AVX-512+VL)
#
# Environment:
#   ITB_SKIP_CLEAN=1       # keep existing artefacts (fast iteration)
#   ITB_KEEP_DOWNLOADS=1   # keep fetched dependency trees
#
# The PHPUnit runner lives in one of the fetched trees (vendor/ from
# composer, or tools/phpunit.phar). On a host with neither composer nor
# a PATH-visible phpunit, restoring it needs network, so
# ITB_KEEP_DOWNLOADS=1 is the practical setting for offline work here.

set -eu
set -o pipefail

cd "$(dirname "$0")"
REPO_ROOT="$(cd ../.. && pwd)"

TAGS=()
case "${1:-}" in
    --noitbasm) TAGS=(-tags=noitbasm); shift;;
    -h|--help)  echo "usage: $0 [--noitbasm]"; exit 0;;
    "")         ;;
    *)          echo "unknown option: $1" >&2; exit 2;;
esac

# ---- artefact wipe ---------------------------------------------------
# The build starts from nothing: every artefact this binding owns is
# removed before anything is rebuilt, so no output can predate this
# invocation. ITB_SKIP_CLEAN=1 skips the wipe for fast iteration.
#
# Fetched dependency trees and registry-resolved lock files need network
# to restore, so ITB_KEEP_DOWNLOADS=1 preserves them. That is the weaker
# guarantee: a stale dependency can still mask breakage, and only the
# artefacts this binding compiles itself are then known to be fresh.
#
# Deletion safety: clean_target takes a path relative to this binding's
# own directory. An empty path, an absolute path, or one containing ".."
# is refused outright, and the resolved target is re-checked to lie
# inside the binding directory before removal -- so the wipe cannot
# reach the shared dist/linux-amd64/libitb3.so, another binding, or
# anything else outside this directory. Every removal is logged first.
BINDING_DIR="$(pwd -P)"

# Subtrees, relative to this binding, that a pattern sweep must not
# descend into. A dependency tree preserved by ITB_KEEP_DOWNLOADS sits
# inside this directory, so without this the sweep would reach into the
# very tree the flag is there to protect.
# tools/ is deliberately absent from this list: PHPUnit writes its
# result cache there, and that is test state rather than a download.
CLEAN_PRUNE=('vendor')

clean_target() {
    local rel="$1" abs res
    case "$rel" in
        ""|/*|*..*)
            echo "[clean] refusing unsafe target: '$rel'" >&2
            exit 1
            ;;
    esac
    abs="$BINDING_DIR/$rel"
    [ -e "$abs" ] || [ -L "$abs" ] || return 0
    res="$(readlink -f "$abs")"
    case "$res" in
        "$BINDING_DIR"/*) ;;
        *)
            echo "[clean] refusing target outside $BINDING_DIR: $res" >&2
            exit 1
            ;;
    esac
    echo "[clean] rm -rf $abs"
    rm -rf "$abs"
}

# Remove every entry matching a name pattern anywhere below this
# binding's directory, skipping the CLEAN_PRUNE subtrees. Matches are
# collected before the first removal so the walk is not racing the
# deletions.
clean_tree() {
    local pattern="$1" hit prune
    local -a args=("$BINDING_DIR") hits=()
    for prune in ${CLEAN_PRUNE+"${CLEAN_PRUNE[@]}"}; do
        args+=(-path "$BINDING_DIR/$prune" -prune -o)
    done
    args+=(-name "$pattern" -print0)
    while IFS= read -r -d '' hit; do
        hits+=("$hit")
    done < <(find "${args[@]}")
    for hit in "${hits[@]}"; do
        clean_target "${hit#"$BINDING_DIR"/}"
    done
}

if [[ "${ITB_SKIP_CLEAN:-0}" == "1" ]]; then
    echo "==> ITB_SKIP_CLEAN=1: keeping the existing artefacts"
else
    echo "==> removing the artefacts owned by this binding"
    # PHPUnit writes its result cache beside whichever runner was used,
    # so the caches are hunted across the tree rather than at the root
    # only -- a preserved tools/ must not keep test state alive.
    clean_tree '.phpunit.cache'
    clean_tree '.phpunit.result.cache'
    if [[ "${ITB_KEEP_DOWNLOADS:-0}" == "1" ]]; then
        echo "[clean] ITB_KEEP_DOWNLOADS=1: keeping vendor/, tools/ and composer.lock (weaker guarantee)"
    else
        clean_target 'vendor'
        clean_target 'tools'
        clean_target 'composer.lock'
    fi
fi

# The PHP binding has no runtime dependency -- it reaches libitb3 through
# FFI -- so the only thing a fetched tree provides is the test runner.
# Restore it when the wipe removed it and the host has no other source,
# which keeps the strong guarantee (a full clean stays buildable) without
# needing ITB_KEEP_DOWNLOADS. The URL pins the major; PHPUnit serves the
# current release within it, so no checksum is asserted here.
PHPUNIT_PHAR_URL="https://phar.phpunit.de/phpunit-12.phar"
if [[ -z "${ITB_PHPUNIT:-}" ]] \
   && ! command -v phpunit >/dev/null 2>&1 \
   && [[ ! -x vendor/bin/phpunit && ! -f tools/phpunit.phar ]]; then
    if command -v composer >/dev/null 2>&1; then
        echo "==> restoring the test runner via composer"
        composer install --no-interaction --quiet
    else
        echo "==> fetching the test runner: $PHPUNIT_PHAR_URL"
        mkdir -p tools
        curl -fsSL -o tools/phpunit.phar "$PHPUNIT_PHAR_URL"
        chmod +x tools/phpunit.phar
        php tools/phpunit.phar --version >/dev/null \
            || { echo "build.sh: fetched phpunit.phar does not run" >&2; exit 1; }
    fi
fi

cd "$REPO_ROOT"
echo "==> building libitb3.so${TAGS:+ (with ${TAGS[*]})}"
go build -trimpath "${TAGS[@]}" -buildmode=c-shared \
    -o dist/linux-amd64/libitb3.so ./cmd/cshared

cd "$REPO_ROOT/bindings/php"
echo "==> lint-checking the PHP sources, the tests, the bench and eitb"
for d in src tests bench eitb; do
    if [[ ! -d "$d" ]]; then
        echo "build.sh: expected directory '$d' is missing" >&2
        exit 1
    fi
done
find src tests bench eitb -name '*.php' -print0 | while IFS= read -r -d '' f; do
    php -l "$f" > /dev/null
done
php -l autoload.php > /dev/null

echo "==> PHP binding loads libitb3.so at runtime via FFI; no further build step."
echo "==> ready: ./run_tests.sh"
