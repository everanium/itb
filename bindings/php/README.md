...
extension available. Windows: the same; libitb3 builds as
`libitb3.dll`.
...
The convenience driver builds `libitb3.so` and lint-checks the PHP
sources in one step:
...
same PSR-4 mapping (`Everanium\Itb3\` → `src/`) from `composer.json`.

## Library lookup order

1. `ITB_LIBITB3_PATH` environment variable (path to the shared
   library file).
2. `<repo>/dist/<os>-<arch>/libitb3.<ext>` resolved from the package
   directory (in-repo builds).
3. The OS default loader path (`LD_LIBRARY_PATH`, `ld.so.cache`,
   `DYLD_LIBRARY_PATH`, `PATH`).

The FFI declarations are read from `include/itb3.h` — a hand-cleaned,
preprocessor-free copy of the generated `dist/<os>-<arch>/libitb3.h`
...
the JSON libitb3 emits; absent keys are optional fields at their zero
value.
...
The harness builds `libitb3.so`, exports `ITB_LIBITB3_PATH`, resolves
