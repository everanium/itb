...
Note that `trainHash`'s combined criterion failures — narrow 8-bit lane output (7-of-8 external-Barrier unmask leak), 2¹⁶ rank-space collapse, T-function class — create a theoretically SAT-attackable surface. The naive 2¹⁶ synthetic-space brute empirically blocks under the compound cascade + primer + unrank stack (Step 2 result above); the **full-solver question remains open** — no dedicated SAT / algebraic solver was run against `trainHash`, so whether it breaks under one is **untested**, not proven closed. Every shipped primitive breaks at least one criterion of this failure combination (≥ 128-bit lo-lane closes 7-of-8, full rank space closes 2¹⁶, AES-family / ARX cascades close the T-function bit-plane path), so the untested `trainHash` full-solver question does not shift the shipped-security envelope — but it is a genuinely open question about the primitive, not a closed one.
...
**Independence — 8-seed isolation ([Theorem 3](PROOFS.md#proof-3-8-seed-isolation)).** For any `i ≠ j`, `I(seed_i; seed_j) = 0`. Cross-seed algebraic leverage is absent — if the attacker knows some bits of `noiseSeed` via CCA leak, this gives zero bits of information about `dataSeed / lockSeed / startSeed`. This blocks MITM «split by seed»: impossible to split the problem into «recover `noiseSeed` half» + «recover `dataSeed` half» at cheaper cost, because the halves do not share state space that MITM exploits.
...
|---|---|---|
| Slide attack | Repeating structure across rounds | ChainHash rounds are independent per component ([Theorem 3b](PROOFS.md#proof-3b-chainhash-full-component-utilization)) — no slide equivalence |
...
[Proof 4a](PROOFS.md#proof-4a-multi-factor-full-kpa-resistance) codifies this in one sentence:
