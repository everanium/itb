...
func DecryptAuthenticated128(noiseSeed, dataSeed, startSeed *Seed128, fileData []byte, macFunc MACFunc) ([]byte, error) {
	...
	return interleaveForSingle(original, buildPermutePRF128(noiseSeed, nonce), buildPermuteBatchPRF128(noiseSeed, nonce)), nil
}
...
// EncryptAuthenticated128Cfg is the Cfg variant of
// [EncryptAuthenticated128]: threads cfg through every Cfg-aware
// accessor in the Single Ouroboros authenticated pipeline. Body
// otherwise identical to EncryptAuthenticated128, including the MAC-
...
func EncryptAuthenticated128Cfg(cfg *Config, noiseSeed, dataSeed, startSeed *Seed128, data []byte, macFunc MACFunc) ([]byte, error) {
	...
	width, height := containerSizeAuth128Cfg(cfg, noiseSeed, dataSeed, startSeed, len(encoded)+tagSize)
	totalPixels := width * height
	capacity := (totalPixels * DataBitsPerPixel) / 8
	...
	process128Cfg(cfg, noiseSeed, dataSeed, startSeed, nonce, container, width, height, full, true, 0)
...
// DecryptAuthenticated128Cfg is the Cfg variant of
// [DecryptAuthenticated128].
func DecryptAuthenticated128Cfg(cfg *Config, noiseSeed, dataSeed, startSeed *Seed128, fileData []byte, macFunc MACFunc) ([]byte, error) {
	...
	decodedPtr, decoded := acquireBuffer(capacity)
	defer releaseBuffer(decodedPtr, decoded)
	process128Cfg(cfg, noiseSeed, dataSeed, startSeed, nonce, container, width, height, decoded, false, 0)
	...
	return interleaveForSingleCfg(cfg, original, buildPermutePRF128Cfg(cfg, noiseSeed, nonce), buildPermuteBatchPRF128Cfg(cfg, noiseSeed, nonce)), nil
}

// EncryptAuthenticated3x128Cfg is the Cfg variant of
// [EncryptAuthenticated3x128]: threads cfg through every Cfg-aware
// accessor in the Triple Ouroboros authenticated pipeline. Body
// otherwise identical to EncryptAuthenticated3x128, including the
...
// DecryptAuthenticated3x128Cfg is the Cfg variant of
// [DecryptAuthenticated3x128].
...
// EncryptStreamAuthenticated128 encrypts a single Streaming AEAD chunk
// with integrity protection using 128-bit hash seeds.
//
// Pipeline: data → COBS encode → build payload [COBS][0x00][fill]
...
func EncryptStreamAuthenticated128(noiseSeed, dataSeed, startSeed *Seed128, data []byte, macFunc MACFunc, streamID [32]byte, cumulativePixelOffset uint64, finalFlag bool) ([]byte, error) {
	...
	width, height := containerSizeAuth128(noiseSeed, dataSeed, startSeed, len(encoded)+tagSize+1)
	totalPixels := width * height
	capacity := (totalPixels * DataBitsPerPixel) / 8
...
// DecryptStreamAuthenticated128 decrypts and verifies a single
// Streaming AEAD chunk using 128-bit hash seeds. Returns the recovered
...
func DecryptStreamAuthenticated128(noiseSeed, dataSeed, startSeed *Seed128, chunkData []byte, macFunc MACFunc, streamID [32]byte, cumulativePixelOffset uint64) ([]byte, bool, error) {
	...
	return interleaveForSingle(original, buildPermutePRF128(noiseSeed, nonce), buildPermuteBatchPRF128(noiseSeed, nonce)), finalFlag, nil
}

// EncryptStreamAuthenticated3x128 encrypts a single Streaming AEAD
// chunk using Triple Ouroboros (128-bit variant). Streaming-binding
// components mirror [EncryptStreamAuthenticated128]: streamID,
...
// DecryptStreamAuthenticated3x128 decrypts and verifies a single
// Streaming AEAD chunk using Triple Ouroboros (128-bit variant).
...
func DecryptStreamAuthenticated3x128(noiseSeed, dataSeed1, dataSeed2, dataSeed3, startSeed1, startSeed2, startSeed3 *Seed128, chunkData []byte, macFunc MACFunc, streamID [32]byte, cumulativePixelOffset uint64) ([]byte, bool, error) {
	...
	return interleaveForTripleParallelLocked(parts[0], parts[1], parts[2], buildLockPRF128(noiseSeed, nonce), buildLockBatchPRF128(noiseSeed, nonce)), finalFlag, nil
}

// EncryptStreamAuthenticated128Cfg is the Cfg variant of
// [EncryptStreamAuthenticated128]: threads cfg through every Cfg-aware
...
func EncryptStreamAuthenticated128Cfg(cfg *Config, noiseSeed, dataSeed, startSeed *Seed128, data []byte, macFunc MACFunc, streamID [32]byte, cumulativePixelOffset uint64, finalFlag bool) ([]byte, error) {
	...
	width, height := containerSizeAuth128Cfg(cfg, noiseSeed, dataSeed, startSeed, len(encoded)+tagSize+1)
	totalPixels := width * height
	capacity := (totalPixels * DataBitsPerPixel) / 8
...
// DecryptStreamAuthenticated128Cfg is the Cfg variant of
// [DecryptStreamAuthenticated128].
func DecryptStreamAuthenticated128Cfg(cfg *Config, noiseSeed, dataSeed, startSeed *Seed128, chunkData []byte, macFunc MACFunc, streamID [32]byte, cumulativePixelOffset uint64) ([]byte, bool, error) {
	...
	return interleaveForSingleCfg(cfg, original, buildPermutePRF128Cfg(cfg, noiseSeed, nonce), buildPermuteBatchPRF128Cfg(cfg, noiseSeed, nonce)), finalFlag, nil
}

// EncryptStreamAuthenticated3x128Cfg is the Cfg variant of
// [EncryptStreamAuthenticated3x128]: threads cfg through every
...
// DecryptStreamAuthenticated3x128Cfg is the Cfg variant of
// [DecryptStreamAuthenticated3x128].
func DecryptStreamAuthenticated3x128Cfg(cfg *Config, noiseSeed, dataSeed1, dataSeed2, dataSeed3, startSeed1, startSeed2, startSeed3 *Seed128, chunkData []byte, macFunc MACFunc, streamID [32]byte, cumulativePixelOffset uint64) ([]byte, bool, error) {
	...
	return interleaveForTripleParallelLockedCfg(cfg, parts[0], parts[1], parts[2], buildLockPRF128Cfg(cfg, noiseSeed, nonce), buildLockBatchPRF128Cfg(cfg, noiseSeed, nonce)), finalFlag, nil
}
