...
// points. Most bench names carry a "Triple" infix
// (BenchmarkExtTriple<Primitive>…); the KMAC, production-shape and
// fleet cohorts are named for what they measure instead.
//
...
// 8 MiB per region) and every hashPool starter tier (512 and 262144
...
// --- KMAC256 Triple facade benches: canonical cipher, MAC dimension ---
//
// Where the ExtTriple benches sweep the inner-hash primitive
// dimension, this cohort pins one canonical cipher configuration
// (Areion-SoEM-512 inner hash, 1024-bit keys, 512-bit nonce, parallax
// off, wrapper off) and turns the MAC dimension to KMAC256, so the
// AVX-512 Keccak-f[1600] tier in macs/internal/keccakasm is measured
// end-to-end through both wire shapes (Single Message MAC and
// Streaming AEAD MAC), both directions, at 1 / 16 / 64 MB. Build with
// -tags noitbasm for the scalar-tier baseline of the same twelve
// cells.
