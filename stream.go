...
// EncryptStream encrypts data in chunks, calling emit for each encrypted chunk.
// Each chunk is a self-contained ITB message: [nonce][width][height][container].
// Chunks can be concatenated for storage and later decrypted with DecryptStream.
//
// The emit callback receives each encrypted chunk sequentially. The caller is
// responsible for writing chunks to the destination (file, network, buffer).
//
...
//	    return err
//	})
func EncryptStream(noiseSeed, dataSeed, startSeed *Seed, data []byte, chunkSize int, emit func(chunk []byte) error) error {
	...
	for off := 0; off < len(data); off += chunkSize {
		...
		chunk, err := Encrypt(noiseSeed, dataSeed, startSeed, data[off:end])
		if err != nil {
...
// DecryptStream decrypts concatenated chunks produced by EncryptStream.
// Calls emit for each decrypted plaintext chunk sequentially.
//
// Parses [nonce][width][height] headers to determine chunk boundaries,
// then decrypts each chunk independently.
//
...
//	    return nil
//	})
func DecryptStream(noiseSeed, dataSeed, startSeed *Seed, data []byte, emit func(chunk []byte) error) error {
	if noiseSeed == dataSeed || noiseSeed == startSeed || dataSeed == startSeed {
		return fmt.Errorf("itb: all three seeds must be different (triple-seed isolation)")
	}
	...
	for off := 0; off < len(data); {
		...
		decrypted, err := Decrypt(noiseSeed, dataSeed, startSeed, data[off:off+chunkLen])
		if err != nil {
			return fmt.Errorf("itb: chunk at offset %d: %w", off, err)
		}
