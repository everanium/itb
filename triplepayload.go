...
func buildTripleWire3(cfg *Config, data []byte, bp lockBatchPRF48, reserve int, fillReserve bool, nonce, ilNonce []byte, sizeFn func(cobsLens [3]int) (width, height int)) (tp *triplePayloads, out, container []byte, width, height int, err error) {
	...
	// Each bound covers that lane's interlock-nonce fragment plus the
	// barrier output, which is exactly the byte count the matching COBS
	// goroutine encodes below.
	bounds := [3]int{
		cobsEncodeBound(lens[0] + laneLen),
		cobsEncodeBound(lens[1] + laneLen),
		cobsEncodeBound(lens[2] + laneLen),
	}
	width, height = sizeFn([3]int{bounds[0], bounds[1], bounds[2] + reserve})
	...
	//
	// Each lane holds its interlock-nonce fragment followed by the
	// laneLen bytes the barrier writes. acquireBuffer hands back a
	// zeroed slice, so the fragment copy is the only write ahead of the
	// barrier's own sub-slice.
	var lanePtrs [3]*[]byte
	var lanes [3][]byte
	for i := range lanes {
		lanePtrs[i], lanes[i] = acquireBuffer(lens[i] + laneLen)
		copy(lanes[i][:lens[i]], ilNonce[offs[i]:offs[i]+lens[i]])
	}
	releaseLanes := func() {
		for i := range lanes {
	...
	splitForTriple48LockedInto(cfg, data, bp,
		lanes[0][lens[0]:], lanes[1][lens[1]:], lanes[2][lens[2]:])
