...
func process128(noiseSeed, dataSeed, startSeed *Seed128, nonce []byte, container []byte, width, height int, data []byte, encode bool, maxW int) {
	...
	if numWorkers == 1 {
		processChunk128(nil, noiseSeed, dataSeed, nonce, container, data, startPixel, totalPixels, 0, dataPixels, totalBits, encode)
		return
	}
	...
	for w := 0; w < numWorkers; w++ {
		...
		go func(startP, endP int) {
			defer wg.Done()
			processChunk128(nil, noiseSeed, dataSeed, nonce, container, data, startPixel, totalPixels, startP, endP, totalBits, encode)
		}(startP, endP)
	}
	wg.Wait()
}
...
// to [processChunk128] so a non-nil cfg with an explicit NonceBits
// override is honoured at the per-pixel buffer-allocation site. Body
// otherwise identical to process128.
func process128Cfg(cfg *Config, noiseSeed, dataSeed, startSeed *Seed128, nonce []byte, container []byte, width, height int, data []byte, encode bool, maxW int) {
	if encode {
		// Lock the AttachLockSeed safeguard: any subsequent
		// re-attach attempt on noiseSeed will panic with
		// ErrLockSeedAfterEncrypt to prevent breaking
	...
	if numWorkers == 1 {
		processChunk128(cfg, noiseSeed, dataSeed, nonce, container, data, startPixel, totalPixels, 0, dataPixels, totalBits, encode)
		return
	}
	...
	for w := 0; w < numWorkers; w++ {
		...
		go func(startP, endP int) {
			defer wg.Done()
			processChunk128(cfg, noiseSeed, dataSeed, nonce, container, data, startPixel, totalPixels, startP, endP, totalBits, encode)
		}(startP, endP)
	}
	wg.Wait()
}

// containerSize128Cfg is the Cfg variant of [containerSize128]: threads
...
func containerSize128Cfg(cfg *Config, noiseSeed, dataSeed, startSeed *Seed128, payloadCOBSLen int) (width, height int) {
	return calcContainerSizeCfg(cfg, payloadCOBSLen,
		noiseSeed.MinPixels(), dataSeed.MinPixels(), startSeed.MinPixels())
}

// containerSizeAuth128Cfg is the Cfg variant of [containerSizeAuth128].
func containerSizeAuth128Cfg(cfg *Config, noiseSeed, dataSeed, startSeed *Seed128, payloadCOBSLen int) (width, height int) {
	return calcContainerSizeCfg(cfg, payloadCOBSLen,
		noiseSeed.MinPixelsAuth(), dataSeed.MinPixelsAuth(), startSeed.MinPixelsAuth())
}

// containerSize3_128Cfg is the Cfg variant of [containerSize3_128]:
// threads cfg through [calcContainerSize3Cfg].
func containerSize3_128Cfg(cfg *Config, noiseSeed *Seed128, dataSeed1, dataSeed2, dataSeed3 *Seed128, startSeed1, startSeed2, startSeed3 *Seed128, cobsLens [3]int) (width, height int) {
...
// containerSizeAuth3_128Cfg is the Cfg variant of
// [containerSizeAuth3_128].
func containerSizeAuth3_128Cfg(cfg *Config, noiseSeed *Seed128, dataSeed1, dataSeed2, dataSeed3 *Seed128, startSeed1, startSeed2, startSeed3 *Seed128, cobsLens [3]int) (width, height int) {
	return calcContainerSize3Cfg(cfg, cobsLens,
		noiseSeed.MinPixelsAuth(),
		[3]int{dataSeed1.MinPixelsAuth(), dataSeed2.MinPixelsAuth(), dataSeed3.MinPixelsAuth()},
		[3]int{startSeed1.MinPixelsAuth(), startSeed2.MinPixelsAuth(), startSeed3.MinPixelsAuth()})
...
func Encrypt128Cfg(cfg *Config, noiseSeed, dataSeed, startSeed *Seed128, data []byte) ([]byte, error) {
	...
	process128Cfg(cfg, noiseSeed, dataSeed, startSeed, nonce, container, width, height, payload, true, 0)
...
// Decrypt128Cfg is the Cfg variant of [Decrypt128]: threads cfg through
// currentNonceSizeCfg, headerSizeCfg, process128Cfg, the PRF builder,
// and interleaveForSingleCfg. Body otherwise identical to Decrypt128,
...
// Body otherwise identical to Encrypt3x128, including the
...
func Encrypt3x128Cfg(cfg *Config, noiseSeed, dataSeed1, dataSeed2, dataSeed3, startSeed1, startSeed2, startSeed3 *Seed128, data []byte) ([]byte, error) {
	...
	p0, p1, p2 := splitForTriple48LockedCfg(cfg, data, buildLockPRF48_128Cfg(cfg, noiseSeed, nonce), buildLockBatchPRF48_128Cfg(cfg, noiseSeed, nonce))
	...
	width, height := containerSize3_128Cfg(cfg, noiseSeed, dataSeed1, dataSeed2, dataSeed3, startSeed1, startSeed2, startSeed3, [3]int{len(encs[0]), len(encs[1]), len(encs[2])})
	totalPixels := width * height
	third := totalPixels / 3
	thirdPixels2 := totalPixels - 2*third
	...
	go func() {
		process128Cfg(cfg, noiseSeed, dataSeed3, startSeed3, nonce, container[offset2:totalPixels*Channels], thirdPixels2, 1, payloads[2], true, perThird)
		wg.Done()
	}()
	wg.Wait()
...
func Decrypt3x128Cfg(cfg *Config, noiseSeed, dataSeed1, dataSeed2, dataSeed3, startSeed1, startSeed2, startSeed3 *Seed128, fileData []byte) ([]byte, error) {
	...
	return interleaveForTriple48LockedCfg(cfg, parts[0], parts[1], parts[2], buildLockPRF48_128Cfg(cfg, noiseSeed, nonce), buildLockBatchPRF48_128Cfg(cfg, noiseSeed, nonce)), nil
}
