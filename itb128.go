...
func process128Cfg(cfg *Config, noiseSeed, dataSeed, startSeed *Seed128, nonce []byte, container []byte, width, height int, data []byte, encode bool, maxW int) {
	...
	if numWorkers == 1 {
		processChunk128(cfg, noiseSeed, dataSeed, nonce, container, data, startPixel, totalPixels, 0, dataPixels, totalBits, microBatch, encode)
		return
	}
	...
	for w := 0; w < numWorkers; w++ {
		...
		go func(startP, endP int) {
			defer wg.Done()
			processChunk128(cfg, noiseSeed, dataSeed, nonce, container, data, startPixel, totalPixels, startP, endP, totalBits, microBatch, encode)
		}(startP, endP)
	}
	wg.Wait()
}
...
func checkEightSeeds128(ns, ls, ds1, ds2, ds3, ss1, ss2, ss3 *Seed128) error {
	...
	for i := 0; i < len(seeds); i++ {
		...
		// otherwise invoke a nil function later in ChainHash128, so
...
func Decrypt3x128Cfg(cfg *Config, noiseSeed, lockSeed, dataSeed1, dataSeed2, dataSeed3, startSeed1, startSeed2, startSeed3 *Seed128, fileData []byte) ([]byte, error) {
	...
	return interleaveForTriple48LockedCfg(cfg, lanes[0], lanes[1], lanes[2], buildLockBatchPRF48_128Cfg(cfg, lockSeed, ilNonce)), nil
}
