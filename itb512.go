...
// checkEightSeeds512 verifies all 8 seeds are value-distinct
...
func Encrypt3x512Cfg(cfg *Config, noiseSeed, lockSeed, dataSeed1, dataSeed2, dataSeed3, startSeed1, startSeed2, startSeed3 *Seed512, data []byte) ([]byte, error) {
	...
	tp, width, height, err := buildTriplePayloads(cfg, data, buildLockBatchPRF48_512Cfg(cfg, lockSeed, ilNonce), nomacTagStubSizeCfg(cfg), true,
