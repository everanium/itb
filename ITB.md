...
The shipped barrier composes two mechanisms — a per-chunk 48-bit keyed bit permutation (Rank Barrier) and a per-pixel absorption stage (Pixel Barrier), both described together in §12. This section narrates the absorption intuition, which is the Pixel Barrier's contribution.
...
- **Container**: filled by `internal/drbg` (AES-CTR or ChaCha20, seeded per call from crypto/rand CSPRNG) — external, independent of the construction.
- **Config**: ChainHash(seed + nonce) — internal, PRF-based.
- **Nonces**: crypto/rand — two independent per-message draws (`main_nonce` for the per-pixel and per-region derivations, `interlock_nonce` for the barrier's per-chunk mask draws), each wire-configurable up to 512 bits and enforced pairwise-distinct inside the pair.

Two independent random processes. The DRBG does not know about PRF, PRF does not know about the DRBG. Their only point of contact is the moment of embedding, after which the observer sees only the result, not the components.
...
- A different random container (fresh DRBG draw).
- A different config map (nonce feeds into every ChainHash).
- Three different startPixels (main nonce feeds into each region's `deriveStartPixel`).
...
- **dataSeed1, dataSeed2, dataSeed3** → per-region rotation + XOR configurations.
- **startSeed1, startSeed2, startSeed3** → per-region pixel offsets.
...
The lockSeed keys the barrier's permutation channel independently of the noiseSeed that keys the noise-position channel, so a structural shortcut against one channel's primitive cannot leak into another channel's derivation. The three-way per-region `dataSeed`/`startSeed` split localises any per-region weakness to that region alone — recovery of one region's dataSeed or startSeed reveals nothing about the other two.

## 5. Triple Ouroboros: Three Interleaved Regions

Plaintext does not reach the regions as bytes. Every message first passes through the Interlocked Barrier's Rank Barrier (§12) — a per-chunk PRF-keyed 48-bit permutation — which is also the mechanism that produces the three region payloads. What each region actually carries is a stream of PRF-permuted 16-bit lane fragments, not plaintext at any offset.

**How the plaintext becomes three region payloads.**
...
5. Region 0's payload receives `l0` (2 bytes), Region 1 receives `l1`, Region 2 receives `l2`. The three payloads grow in lockstep — one 16-bit lane fragment per 48-bit input chunk.
6. Each region's lane buffer opens with that region's fragment of the interlock nonce; the lane values of steps 2–5 accumulate after it, and the whole buffer enters the COBS stage as one payload. The nonce is split `base, rem := N/3, N%3`, with remainder bytes going to the lowest-indexed lanes; both sides derive the split from the configured nonce size alone, so no length information travels on the wire. The fragments therefore sit inside the Pixel Barrier's coverage and inside the MAC's input on the authenticated shapes.
...
Plaintext + 4-byte BE length prefix
  ...
  └─────────────────────────────────────────────────┘
   ...
   (l0, l1, l2)      (l0, l1, l2)
        │                │
        ▼                ▼
   Region 0 payload:  il_frag0 ‖ l0_chunk0 ‖ l0_chunk1 ‖ ...  (16 bits per chunk)
   Region 1 payload:  il_frag1 ‖ l1_chunk0 ‖ l1_chunk1 ‖ ...
   Region 2 payload:  il_frag2 ‖ l2_chunk0 ‖ l2_chunk1 ‖ ...
```

No region carries a plaintext byte at any position. Without the lockSeed and interlock_nonce, the mapping from plaintext bits to lane positions is a per-chunk secret drawn from ≈ 2^70.20 balanced partitions; a known-plaintext crib fixes no bit-position-to-lane anchor a solver could rest on. The three per-region payloads are what feeds every stage below; the plaintext byte sequence never appears intact anywhere past this point.

**Three parallel DRBG regions.** One container is allocated. Three parallel goroutines fill it via `internal/drbg` — each writes to its own region:
...
**Three parallel encrypt pipelines.** Each per-region lane payload (16-bit fragments from the Rank Barrier's chunk permutation, not plaintext bytes) is COBS-encoded, null-terminated, filled with DRBG residue, then encrypted into its region with its own seeds:

```
Region 0 payload (l0 stream) → COBS → [cobs|0x00|fill] → dataSeed1 + startSeed1 + noiseSeed → Region 0
Region 1 payload (l1 stream) → COBS → [cobs|0x00|fill] → dataSeed2 + startSeed2 + noiseSeed → Region 1
Region 2 payload (l2 stream) → COBS → [cobs|0x00|fill] → dataSeed3 + startSeed3 + noiseSeed → Region 2
                                                        ─────────────────────────────────
                                                        3 goroutines, each with numCPU/3 workers
```

Each pipeline is the per-pixel absorption stage: per-pixel ChainHash, rotation, XOR mask, noise bit at unknown position, startPixel wrap-around — all within its region. The Rank Barrier of the Interlocked Barrier (§12) already ran before this stage produced the lane payloads; the Pixel Barrier (per-pixel absorption, §12) is what these three pipelines apply. The two Barriers always run together — this is the shipped indivisible barrier composition.

**Per-pixel bit-level processing.** Inside each region, the standard ITB per-pixel processing applies. For a lane byte `b` (part of Region 0's `l0` stream after COBS framing):
...
```
Output: [main_nonce][W][H][uniform random pixels]
...
**Decrypt.** The three regions are decoded in parallel: read `[main_nonce][W][H]`, split the pixel data into thirds (integer division; last third absorbs remainder), three parallel goroutines run the Pixel Barrier inverse (noise-strip, inverse rotate, XOR-unmask) on each region with the respective seeds, and COBS-decode each region back to its lane payload. Each lane's leading interlock-nonce fragment is stripped and the three fragments are reassembled into the interlock nonce. Then the Rank Barrier inverse (`interleaveTriple48LockedBatch`) reassembles the three lane streams into the 48-bit chunks under the same per-chunk mask triples derived from lockSeed + interlock_nonce, and the 4-byte length prefix is stripped to recover the original plaintext at its exact extent.
...
- **Rank Barrier — per-chunk mask permutation** — a per-chunk PRF-keyed mask triple drawn from ≈ 2^70.20 balanced partitions sits between the plaintext and the pixel layer; without the lockSeed the mapping from plaintext bits to observed lane positions is a per-chunk secret.
- **Pixel Barrier — per-pixel 1:1 signal/noise ambiguity** — 7-rotation × 8-noisePos encoding preserves 56 per-pixel candidates at 1:1 signal/noise from a DRBG noise bit at unknown position.

Alongside the barrier, three independent per-region startPixels — each derived from a separate startSeed and not transmitted on the wire — force the attacker to jointly enumerate (sp0, sp1, sp2) candidate triples without feedback.
...
- **dataSeed configs** (rotation + 56 XOR bits, per region) — completely independent, zero leak.
- **startSeed configs** (per-region pixel offsets) — completely independent, zero leak.
...
**What the attacker gets after removing noise bits.** CCA lets the attacker strip noise bits and see 7 data bits per channel. But "clean encrypted data" is a misleading description. These data bits contain a mixture of encrypted plaintext (COBS-encoded) **and** encrypted DRBG fill — both encrypted identically by the per-region dataSeed (rotation + XOR). The container is always over-sized relative to the payload; the `side++` construction guarantees a positive DRBG-fill gap. Perfect fill — zero DRBG bytes — is mathematically impossible ([Proof 10](PROOFS.md#proof-10-guaranteed-drbg-residue-no-perfect-fill)).

This means CCA does not give the attacker a clean plaintext-only ciphertext. A portion of the data bits carry random DRBG fill that is indistinguishable from encrypted plaintext. The attacker does not know where the plaintext ends and the fill begins, does not know the fill content, and cannot separate one from the other without the correct dataSeed. The DRBG-residue barrier layer is partially preserved within the data channel itself: ambiguity from DRBG residue persists even after noise removal.
...
See [SECURITY.md § 2 CCA Oracle Leak Comparison](SECURITY.md#2-cca-oracle-leak-comparison), [SCIENCE.md § 2.8 CCA Leak Upper Bound](SCIENCE.md#28-cca-leak-upper-bound-theorem-6), [Proof 10](PROOFS.md#proof-10-guaranteed-drbg-residue-no-perfect-fill).

## 11. startPixels: Not Transmitted, Not Recoverable

Each region's startPixel is computed from its own startSeed + main nonce via ChainHash. None of the three startPixels is transmitted or stored; each is computed in a register once. The only theoretical way to learn a startPixel is a cache side-channel (Flush+Reload, Prime+Probe) by a co-located attacker on the same CPU.

Even in the worst case (Full KPA + CCA + cache side-channel on one region), the attacker gets: noisePos (from CCA) + one startPixel (from cache) + 7 rotation candidates per pixel of that region + the per-chunk barrier permutation still unobservable. Without invertible hash — brute-force 2^keyBits on that region's dataSeed. The other two regions remain isolated.
...
The Interlocked Barrier is a mandatory, always-on, non-disableable layer that sits between the plaintext-interleaved payload and the pixel-embedding pipeline. It composes two mechanisms in series and never runs either in isolation. **Rank Barrier** is a per-chunk 48-bit keyed bit permutation over the interleaved payload, drawn from a ≈ 2^70.20 balanced-partition space keyed by the lockSeed and the interlock nonce. **Pixel Barrier** is the per-pixel absorption stage that whitens each channel byte through channelXOR + rotate7 + noise-bit insertion, keyed by the dataSeed and noiseSeed against the main nonce. Rank Barrier denies the attacker a stable bit-position-to-lane anchor; Pixel Barrier denies a per-byte observation channel. Neither Barrier alone is sufficient — Rank Barrier alone is invertible once a primitive is inverted, and Pixel Barrier alone leaves a fixed bit-position map. The shipped construction always engages both.

**Rank Barrier — 48-bit chunk permutation.** Each 48-bit (6-byte) chunk of the interleaved payload is partitioned into three disjoint 16-of-48 lanes by a mask triple `(m0, m1, m2)`, each of popcount 16, with `m0 ∪ m1 ∪ m2` covering all 48 bits. The mask triple is drawn per chunk by a PRF-keyed unrank of the balanced-partition space, keyed by the lockSeed and the nonce, and unobservable without the lockSeed.
...
**Pixel Barrier — per-pixel absorption.** Each pixel packs 56 payload bits into 8 channel bytes of 7 data bits each. The 8 channels carry the **RGBWYOPA** mnemonic — Red, Green, Blue, White, Yellow, Orange, Purple, Alpha — labelling the eight positions of the byte-unit; the mnemonic is a naming convention, not a tie to image processing. Per pixel, keyed by the `dataSeed` and `noiseSeed` ChainHash outputs, the Pixel Barrier applies per 7-bit field: a **channelXOR** with 7 PRF-mask bits, a **rotate7** by a per-pixel amount in [0, 6], and insertion around a per-pixel **noise bit** taken from the original DRBG container byte at a per-pixel `noisePos`. The decode path inverts exactly (noise-strip, inverse rotate, XOR).

**The absorption guarantee** ([Proof 1](PROOFS.md#proof-1-information-theoretic-barrier)): for any observed byte value v and any hash output h, the probability P(v | h) = 1/2. This holds even under Full KPA — because the noise bit comes from the original container (DRBG), which is random and independent of everything. The observation does not uniquely determine the hash output. This is information theory, not computational assumption.
...
**Without hash inversion (PRF):** 56 candidates per pixel × P pixels per region × 3 regions = a combinatorial space that no attainable throughput enumerates. The attacker cannot verify any candidate without inverting ChainHash. PRF makes inversion infeasible. The Pixel Barrier's per-pixel ambiguity is preserved, and the Rank Barrier stacks a per-chunk ≈ 2^70.20 mask-space enumeration on top that must be resolved jointly.

**With hash inversion (invertible hash):** the attacker takes each candidate, inverts ChainHash → gets candidate seed → verifies on another pixel. Inversion **bypasses** the Pixel Barrier's per-pixel ambiguity. The Rank Barrier's per-chunk permutation remains — the mapping from plaintext bits to observed lane positions is a hidden per-chunk secret and no inversion of the primitive produces it — so the instance-formulation under-determination survives. The barrier is not broken; ChainHash is inverted.

The barrier absorbs the hash output through both mechanisms: (1) the Pixel Barrier's noise absorption — DRBG noise bit at unknown position makes the byte ambiguous, and 7 rotation candidates per pixel create 7^P unverifiable combinations; (2) the Rank Barrier's permutation — a hidden per-chunk mask triple denies a bit-position-to-lane anchor. DRBG residue — guaranteed fill bytes encrypted by dataSeed within the data channel, indistinguishable from encrypted plaintext ([Proof 10](PROOFS.md#proof-10-guaranteed-drbg-residue-no-perfect-fill)) — is a structural property of mechanism (1): even after CCA reveals noise bit positions, DRBG fill remains in the data channel. CCA (MAC + Reveal) can bypass the Pixel Barrier's noise-position uncertainty, but the Pixel Barrier's rotation ambiguity, the DRBG residue, and the Rank Barrier's per-chunk permutation remain intact through 8-seed isolation. The noise bits are removed, but the data bits still contain DRBG fill that the attacker cannot separate from plaintext — the information-theoretic barrier is never fully eliminated. KPA candidates are ambiguity, not leakage. PRF preserves the Pixel Barrier's ambiguity; the Rank Barrier's under-determination survives even an invertible-hash case.

**Map space vs key space.** Direct guessing of the per-pixel configuration map is architecturally more expensive than seed brute-force: 2^(62·P) attempts (62 config bits per pixel — 3 noise-position + 3 data-rotation + 56 per-bit XOR), which at the shipped minimum container P = 1225 for 1024-bit keys gives ≈ 2^75950, dwarfing the ≤ 2^2048 key space. Brute-forcing the seed remains the attacker's most efficient known path, and even that residual enumeration exceeds the Landauer bound on irreversible enumeration at all shipped key sizes (see §10).
...
- **Grover** — requires a verification oracle. Core ITB and MAC + Silent Drop have no external oracle; the attacker must jointly search noiseSeed and per-region dataSeeds (without dataSeed, noiseSeed output is indistinguishable from random), while three independent startSeeds contribute per-region startPixel candidates (enumerated, not brute-forced), and the per-chunk barrier permutation lifts each Grover iteration by a factor tied to the ≈ 2^70.20 mask space. With MAC + Reveal: CCA reveals noisePos but not startPixels or lockSeed.
...
In AES or ChaCha20, testing one candidate key takes on the order of a nanosecond — a single block operation. In ITB, testing one candidate requires decrypting the **entire container** — all P pixels across all three regions, each with ChainHash evaluation, plus the per-chunk barrier unrank. The larger the message, the more expensive each attempt.
...
- **Barrier protects the PRF:** hash collisions are the only theoretical weakness of a non-invertible hash function — two different inputs producing the same output. In a traditional cipher, collisions may be exploitable because the attacker observes the output directly. In ITB, collisions are invisible: two pixels with the same dataHash have different original container bytes (DRBG), so the observed bytes are different. The collision is absorbed. The 48-bit Interlocked Barrier compounds this by re-mapping every chunk through a hidden per-chunk permutation — any hypothetical collision in the underlying dataHash lands in a chunk-specific lane assignment the attacker cannot recover.

- **ChainHash architecture resists MITM:** the N independent `uint64` seed components are processed sequentially — each round feeds the previous output XOR'd with the next component — so compromising one component reveals nothing about the others (there are no key-schedule relationships between them). Three independent barriers stack against meet-in-the-middle attacks: unobservable hash output (Pixel Barrier absorption), non-invertibility (PRF assumption), and multi-call key discrimination through the chained output.

Together: non-invertibility blocks inversion, and absorption hides collisions. Each property closes the other's theoretical weakness. In Core ITB and MAC + Silent Drop (no oracle, passive observation only), the barrier makes a non-invertible hash function indistinguishable from an ideal random function — collisions absorbed, statistical patterns absorbed, no known attack surface remains. With MAC + Reveal (CCA): noiseSeed config is leaked via oracle interaction, but per-region dataSeeds and the lockSeed remain protected by PRF non-invertibility and 8-seed isolation. Even after noise removal, the data channel retains DRBG fill bytes encrypted by dataSeed — perfect fill is impossible ([Proof 10](PROOFS.md#proof-10-guaranteed-drbg-residue-no-perfect-fill)) — so information-theoretic ambiguity persists within the data bits themselves.
...
- **No magic bytes or file-format signatures.** The wire header carries only `[main_nonce][W][H]` — dimensions plus the main per-message nonce — and the container. Nothing on the wire identifies the payload as ITB output.
...
- **Null terminator encrypted.** The COBS terminator is embedded inside each region's encrypted payload, so decoding with a wrong seed does not surface a distinguishing boundary.
