...
**Note (dual-container variant — Double Ouroboros).** An alternative approach to doubling brute-force space without modifying pixel processing. Split plaintext into two parts (even/odd bytes), encrypt each into a separate CSPRNG container sharing a single header. 5 seeds: 2 × dataSeed + 2 × startSeed + 1 × noiseSeed (shared). Output format: `[nonce][W][H][pixel_data: W×H×8]` — identical to standard ITB. Internally, pixel data is split in half: pixels 0..W×H/2-1 = container1, pixels W×H/2..W×H-1 = container2. Each container has its own startPixel (wrapping within its half), its own dataSeed (independent rotation + XOR), and independent CSPRNG fill. The boundary is computed deterministically from W×H — not stored. The attacker sees a standard ITB message: same header, same size, same uniform random pixel data. Cannot distinguish Double from Single Ouroboros. One dataSeed yields every other byte — useless without the second. Under CCA: P × 2^(2×keyBits) classical, √P × 2^keyBits Grover. Separate Encrypt2/Decrypt2 functions with 5 seed parameters. Double Ouroboros.

**Note (triple-container variant — Triple Ouroboros).** Same principle with three containers. 7 seeds: 3 × dataSeed + 3 × startSeed + 1 × noiseSeed. Pixel data split into thirds by integer division (last third absorbs remainder pixels). Three independent rings, three startPixels within their respective thirds, three rotation/XOR configurations. One dataSeed yields every third byte — useless without the other two. Same header, same file size, indistinguishable from Single or Double. Under CCA: P × 2^(3×keyBits) classical, √P × 2^(3×keyBits/2) Grover.

**General pattern (N-tuple Ouroboros).** N containers within one message = (2N+1) seeds (N × dataSeed + N × startSeed + 1 × noiseSeed). Pixel data split into N parts by integer division. Under CCA: P × 2^(N×keyBits) classical. Same header, same file size, externally indistinguishable. The three-seed architecture (N=1) remains the proven optimum for practical use; N>1 is theoretical and architecturally trivial to implement when needed.
