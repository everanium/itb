...
**Three parallel encrypt pipelines.** Each per-snake lane payload (16-bit fragments from Part 1's chunk permutation, not plaintext bytes) is COBS-encoded, null-terminated, filled with DRBG residue, then encrypted into its region with its own seeds:
...
- **Part 2 — per-pixel 1:1 signal/noise ambiguity** — 7-rotation × 8-noisePos encoding preserves 56 per-pixel candidates at 1:1 signal/noise from a DRBG noise bit at unknown position.
