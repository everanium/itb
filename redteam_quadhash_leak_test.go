...
func TestRedTeamQuadHashLeak7of8(t *testing.T) {
	...
	// travels split across the three interlocked lanes. Under quadHash it
	// never reaches the Rank Barrier: the squaring fold drives the 13-byte
	// group block of the cascade fill into its attractor set, which erases
	// the derived interlock key the fill prepends to the lockSeed
	// components. The probe measures that on attacker-chosen material —
	// the derived key spreads widely across nonces while the barrier lanes
	// it feeds do not move at all — and the lanes below are then built
	// under an attacker-chosen zero nonce.
	lens, offs := nonceSplit(currentNonceSizeCfg(cfg))
	inv := probeInterlockNonceInvariance128(cfg, thLeak_quadHash, 512, 8, 8)
	...
	if !inv.invariant {
		t.Fatalf("interlock nonce reaches the barrier lanes under quadHash; the nonce-free reconstruction does not apply")
	}
	...
	// The lockSeed is the lab reference this measurement is stated
	...
	// attack decision. Step 2 then measures how much of that reference
	// the attacker needs at all.
	lockSeed, _ := SeedFromComponents128(thLeak_quadHash, meta.Debug.Lock...)
	bp := buildLockBatchPRF48_128Cfg(cfg, lockSeed, attackerNonce)
	...
	// process128Cfg reads COBS-encoded lane bytes + 0x00 terminator, then
	// DRBG tail. The encoder COBS-encodes `fragment || barrier` as one
	// buffer, so the stream carries an unknown prefix of code byte plus
	// fragment and the attacker-computable tail from offset fragLen+1 on.
	barriers := [3][]byte{lane0, lane1, lane2}
	var lanes [3][]byte
	for i := 0; i < 3; i++ {
		lanes[i], _ = laneStreamWithUnknownFragment(barriers[i], lens[i])
	}
	...
	// Read the interlock-nonce fragments off the lane fronts. With the
	// barrier lanes in hand the first walked pixel of each region carries
	// that region's fragment; enumerating the 2048 per-pixel (noisePos,
	// dataHash) configurations and keeping the readings whose COBS decode
	// reproduces the barrier lane past the fragment yields the fragment
	// itself. Where the pixel window leaves no spare byte past the
	// fragment the reading is under-constrained and several survive.
	...
	t.Logf("interlock-nonce fragments off the lane fronts: candidates per region = %v (lens %v); %d of %d nonce bytes pinned uniquely, %d whole-nonce candidates remain",
	...
	// [0]=lo, [1]=hi, [2..7]=0 — under a nonce of his own choosing. Per
	// candidate: compute expected lane bytes via
	// splitForTriple48LockedInto, splice them behind the unknown fragment
	// prefix, and run the same 7-of-8 leak measurement per pixel over the
	// pixels past that prefix. Record match count.
	//
	// Hypothesis under cascade defence: real lockSeed's cascade path
	// depends on Components[2..7] non-trivially. Attacker's synthetic
	// space (Components[2..7]=0) fundamentally does not reach real cascade
	...
	// Histogram summary: sorted ascending by score, worst-to-best.
	var histKeys []int
	...
	// fragment recovery produced its answer from the observed pixels; this
	// line only says whether the true fragment is among the survivors.
	...
	// legitimate measurement outcome elsewhere. The Pixel Barrier still
	// carries the fragments here even though the Rank Barrier ignores the
	// nonce, so the true fragment must be reachable; its absence means the
	// mirror broke. This consumes ground truth after every decision.
	for i := 0; i < 3; i++ {
		if len(fragSets[i]) == 0 {
