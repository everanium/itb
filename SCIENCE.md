...
In both cases, **the barrier itself is never broken**. The barrier consists of three mechanisms: (1) noise absorption — CSPRNG noise bit at unknown position makes the byte ambiguous ([Theorem 1](PROOFS.md#proof-1-information-theoretic-barrier)); (2) encoding ambiguity — 7 rotation candidates per pixel from dataSeed create 7^P unverifiable combinations ([Theorem 4](PROOFS.md#proof-4-rotation-barrier)); (3) CSPRNG residue — guaranteed fill bytes in data bit positions are indistinguishable from encrypted plaintext ([Proof 10](PROOFS.md#proof-10-guaranteed-csprng-residue-no-perfect-fill)). CCA can bypass mechanism (1) by revealing noise positions, but mechanisms (2) and (3) remain intact through triple-seed isolation and the `side += barrierFill` construction (`SetBarrierFill`, default 1). Hash inversion bypasses mechanisms (1) and (2) via a side path (hash function property), but does not break them — the observation still contains the ambiguity.
