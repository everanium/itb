...
The nonce is mandatory. The shipped wire carries two independent nonces — `main_nonce` (feeding the per-pixel and per-snake derivations) and `interlock_nonce` (feeding the barrier's per-chunk mask draws through `deriveInterLockSeed` under the disjoint `0x04` domain tag) — drawn per encrypt from crypto/rand and enforced pairwise-distinct at the API. Without per-message freshness on both slots, two messages encrypted with the same seeds produce identical pixel configuration maps AND identical per-chunk barrier permutations, and an attacker observing two containers can XOR corresponding extracted bits to obtain a lane-permuted `data1 ⊕ data2` through the shared barrier mask (§1.2.1) — the classical two-time-pad exposure a fresh nonce is designed to preclude. The per-message pair ensures with overwhelming probability that each message receives a unique configuration map. Birthday collision bounds depend on nonce size (per slot, independent): ~2^256 messages (default 512-bit — mathematically unreachable), ~2^128 (256-bit), ~2^64 (128-bit); joint dual-slot collision requires simultaneous collision on both slots.
